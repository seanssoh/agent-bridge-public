...
bridge_daemon_global_auth_sync_tick() {
  ...
  # fix: an env-ONLY bash gate silently skipped an opt-in enabled through the
  # headless `global-auth-sync enable` verb (which PERSISTS the bool but sets no
  # env), so the feature was dead at the daemon level. The probe reads only the
  # runtime config + env (no registry/token/credential work, no audit write), so
  # the cheap not-opted-in skip is preserved and the bash skip can never drift
  # LOOSER or TIGHTER than the python gate. A check failure/timeout fails safe to
