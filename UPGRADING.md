...
`upgrade --apply` 이 operator 가 직접 수정한 파일을 만나면 새 upstream source 를 live 에 적용하고, `git merge-file --diff3` 의 3-way merge 결과(`<<<<<<<` / `|||||||` / `=======` / `>>>>>>>` marker 가 포함될 수 있음)를 `*.upgrade-conflict` sidecar 에 기록한다. 즉 sidecar 는 operator 의 깨끗한 원본 복사본이 아니라 **resolve 가 필요한 merge 결과물**일 수 있다. operator 가 직접 수정한 내용을 보려면 conflict file 을 비교 후 필요한 변경을 main file 에 다시 적용한다 (또는 backup 에서 복원).

매 `upgrade --apply` run 은 `state/upgrade-conflicts/<run-id>.json` 에 conflict 목록과 write 시점의 live target sha256 을 기록한다. 다음 run 의 시작 단계에서 reconcile 이 실행되어 sha256 이 변하지 않은 (operator 가 이미 처리했거나 손대지 않은) conflict 는 `backups/upgrade-conflict-archive/<date>/` 로 자동 archive 된다. 변경된 conflict 는 그대로 두어 mid-reconcile 작업을 방해하지 않는다. lifecycle 도구:

```bash
agent-bridge upgrade conflicts list                    # pending 목록 + 변경 여부
agent-bridge upgrade conflicts diff   <conflict-path>  # live vs conflict diff
agent-bridge upgrade conflicts adopt  --yes <path>     # sidecar 내용을 live 로 채택 (marker/구문 검사 통과 시)
agent-bridge upgrade conflicts discard --yes <path>    # conflict 폐기, live 유지
agent-bridge upgrade conflicts archive --yes <path>    # backups/upgrade-conflict-archive/<date>/ 로 이동
agent-bridge upgrade conflicts reconcile --auto-archive
```

`adopt` 은 live 에 복사하기 전에 sidecar 를 검증한다: 미해결 conflict marker(`<<<<<<<` 등)가 남아 있거나, live 대상이 `.py`/`.sh` 인데 구문 오류가 있으면 **복사를 거부하고 sidecar(복구 artifact)를 그대로 남긴다** — `bridge-upgrade.py` 처럼 upgrader 자신을 marker 로 덮어써 복구 도구를 brick 하는 것을 막기 위함이다. 직접 hand-merge 를 끝낸 operator 가 검사를 우회하려면 `--force` 를 쓴다. 성공한 adopt 는 덮어쓰기 전 live 본을 `<path>.pre-adopt` 로 백업한다.

`agent-bridge status` 는 pending count > 0 일 때 `WARNING: N pending upgrade-conflict file(s)` 를 출력한다.
