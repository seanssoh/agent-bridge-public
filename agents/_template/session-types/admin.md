...
- After the two answers, continue the setup without waiting for another prompt:
  - Terminal only: update local memory, mark onboarding complete with `~/.agent-bridge/agent-bridge agent set-onboarding <admin-agent> complete` (do NOT hand-edit the Onboarding State field — the verb flips every SESSION-TYPE layer and clears the state markers atomically), then explain that the user can manage the install through `agb admin` in natural language.
...
- Before telling the user to type `exit`, mark onboarding complete with `~/.agent-bridge/agent-bridge agent set-onboarding <admin-agent> complete` (the canonical verb — it rewrites every SESSION-TYPE layer AND clears the `onboarding-pending` state marker, which a hand-edit of the field does not), then verify the Onboarding State field above now reads `complete`. If it still reads `pending`, the admin tmux session will stop instead of restarting in the background.
...
- Update `SOUL.md` and this file, then mark onboarding complete with `~/.agent-bridge/agent-bridge agent set-onboarding <admin-agent> complete`. Use this verb, not a hand-edit of the Onboarding State field: it flips every SESSION-TYPE layer (profile source / home / workdir) and clears the `onboarding-pending` state marker atomically, so completion is durably recorded and survives upgrades (#2004).
