...
This is the Agent Bridge build of the Discord channel. On top of the base
channel it adds:

- **Reply references** — inbound Discord replies carry `referenced_message_id`
  (and a best-effort `referenced_message_excerpt`), and `fetch_messages` marks
  reply rows, so the assistant knows exactly which earlier message a reply
  targets instead of guessing from context.
- **Thread-as-session** (opt-in, per agent) — set
  `DISCORD_THREAD_AUTO_SESSION_CHANNEL_ID` to a parent channel's snowflake and
  every thread under it auto-spawns a persistent isolated Claude sub-session
  for that channel-owning agent. Disabled by default. The bundled dispatcher
  (`thread-session/`) derives the owning agent from `BRIDGE_AGENT_ID`, so it
  works for any Discord agent with no per-agent path configuration.

Derived from Anthropic's Apache-2.0 `discord` plugin; see [LICENSE](./LICENSE).
...
Install the plugin:
```
/plugin install discord@agent-bridge
/reload-plugins
```
...
```sh
claude --channels plugin:discord@agent-bridge
```
