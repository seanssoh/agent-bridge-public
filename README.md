...
**Latest pre-release**: v0.16.0-rc1 (2026-06-05) — the **release candidate** for `0.16.0`, capping the `beta1..4` line with a 6-item stabilization wave. Headline: the **daemon supervision redesign (#1563)** — the daemon is now **crash-only and self-supervising** instead of self-healing. Each scheduler tick runs as a child in its own process group under a runner-process **T1 backstop**: a genuinely wedged tick self-aborts within `(max-step-budget + grace)` so the OS init layer (launchd `KeepAlive` / systemd `Restart=always`, plus an optional `Type=notify` watchdog as a **T0** outer ring) restarts a fresh daemon — while a *healthy* long step that keeps stamping progress is **never** punished (the flapping-monitor irony is the #1 risk this redesign is built to avoid). A hardened **singleton** (start-time-proof eviction so a recycled pid is never killed), an **escalate-don't-self-heal** admin-liveness path (a down admin routes a durable task to its codex pair, a busy/long-turn admin is never escalated), and an **A2A receiver** with bounded exponential backoff + a per-`(config-fingerprint, error_class)` circuit breaker (a transient tailnet/bind blip backs off instead of crash-looping; a real auth/config error is held immediately; the fail-closed bind/HMAC boundary is unchanged) round out the supervision contract. See `OPERATIONS.md` §"Daemon supervision (#1563)" + `docs/developer-handover.md` for the behavior + env knobs (`BRIDGE_DAEMON_ADMIN_DOWN_STALE_SECS`, the A2A backoff/breaker ceilings, etc.). The rc also lands the rc1 stabilization items: **#1455** settings single-tree drift doctor, **#1459** cron-dispatch backlog recovery + run/queue reconcile, **#1417** lifecycle identity sync-on-start (+ `agb agent set-onboarding`), **#9773** surgical codex app-server subtree reap on teardown, **#9780** Stop/turn-end inbox auto-drain, and **#1367 auth sealed-paste** — an operator-terminal `agb auth claude-token receive` echo-off `/dev/tty` read so a Claude token is never captured into an agent transcript/argv/env/queue/audit (fails closed with no controlling tty). **A2A Rooms** are now **cross-node (P4) wired and multi-node-verified** on a live 2- and 3-node Tailscale mesh — `agb room create/join/approve/show`, the `agbroom://` invite, per-room epoch bumps, and the leader roster-broadcast all reach across nodes (Cloudflare ZT transport + whole-room `agb a2a send` fan-out remain the maturing edges); see [docs/a2a-rooms.md](./docs/a2a-rooms.md). This is a **release candidate** for live testing — `v0.15.4` remains the recommended stable.
...
- **Recommended upgrade target.** `v0.15.4` (current stable) supersedes `v0.15.3` / `v0.15.2` / `v0.15.1` / `v0.15.0` and all the `v0.15.0-rc1` / `v0.15.0-betaN` / `v0.14.5-betaN` prereleases — treat the latest stable tag as current for production. `v0.16.0-rc1` is the latest **pre-release** (the `0.16.0` release candidate — the daemon supervision redesign #1563 + the rc1 stabilization wave; A2A Rooms now cross-node-verified) — pull it only to live-test the rc features; it does not demote the `v0.15.4` stable.
- v0.7.x → v0.15.4 leap path: **direct `agent-bridge upgrade --apply` to `v0.15.4`** is the current recommended (stable) path; pull `v0.16.0-rc1` on top only to live-test the `0.16.0` features — the v0.13.7-v0.13.10 cycle fixed the Bash 5.3.9 heredoc-stdin deadlock chain by extracting leap-path bodies to `lib/upgrade-helpers/`, so the leap is single-atomic from any v0.7.x/v0.8.x/v0.9.x/v0.10.x/v0.11.x/v0.12.x/v0.13.x/v0.14.x source. v0.13.10 is the minimum-safe fallback if you're pinned below v0.14.x or troubleshooting an in-flight leap. See [OPERATIONS.md §"Upgrade"](./OPERATIONS.md#upgrade) for recipe + follow-up notes.
