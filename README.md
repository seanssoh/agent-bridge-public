...
**Current stable / LTS**: v0.16.4 (2026-06-09) — the current head of the **v0.16 Long-Term Support (LTS) line** and its recommended install. The LTS designation is on the `v0.16` *line*; every v0.16.x release is an LTS release — `v0.16.4` is a v0.16.x LTS-line patch, superseding `v0.16.3` as the line head (same line, not a different LTS; v0.16.3's LTS designation is not revoked). Headline: **guard over-block hardening wave** — five targeted relaxations that reduce false-deny friction in `hooks/tool-policy.py` while keeping all containment intact. The three guard relaxations: **`state/tasks.db` read carve-out** — Bash reads of the queue DB are now allowed (writes/mutations stay denied; no admin bypass); the read-intent classifier is hardened fail-closed: `LD_PRELOAD`/`LD_AUDIT` and similar dynamic-loader env prefixes are treated as exec surface, Bash 5.3 `${ }` funsubs fail closed, awk is scanned in program-word only, and an unresolved path-expansion blocks the carve-out (**#1690**); **admin Bash read of a peer agent's home no longer falsely denied** — admin + read-intent + peer-home path only; writes/mutations and shared secret/private subtrees remain denied even for admin (least-privilege) (**#1692**); **read-only viewer commands (`strings`, `hexdump`, `comm`, `fold`, `expand`, `paste`, `csvlook`) are classified read-intent**, and a prose-mention false-fire is fixed — a protected-path name appearing inside an unbalanced-quote command body is no longer treated as a real file reference; real redirect/assignment to a protected path still denied (**#1693**). Plus: **WARP-mesh A2A receiver no longer crash-loops a healthy `/32`-bound receiver** — `cmd_healthz` falls back, for the cloudflare-warp-mesh transport only, to a dependency-free socket-held liveness check (a plain `bind()` returning `EADDRINUSE` means the socket is held ⇒ healthy); Tailscale/LAN keep full HTTP wedge-detection (**#1701**); and **new read-only verb `agb a2a net-status`** (alias `agb a2a status`) reporting bridge_id / transport / listen addr / receiver healthz / substrate health / peers / room membership — active-transport-only, zero state mutation, no secrets, stable JSON; receiver liveness reuses the #1701-fixed healthz path (**#1697**). Note: **#1691** (intent-aware credential/shared-alias mention gate) was **descoped** from this release — dual adversarial review found two distinct real bypasses; those gates stay conservatively over-blocking until redone bundled with #1709. The `v0.16.3` headline (`lts` upgrade channel + 9-lane stabilization wave #1687/#1662/#1689/#1685/#1667/#1677/#1670/#1672/#1671/#1683), the `v0.16.2` headline (four robustness/perf fixes #1659/#1660/#1661/#1663, flock-backend VM-verified), the `v0.16.1` headline (A2A audit follow-up, iso-upgrade robustness #1635/#1636, MS365 `get_valid_token` #1650, `set -u` roster-map hardening #1627), and earlier `v0.16.0` / `v0.15.x` headlines carry forward. See [CHANGELOG.md](./CHANGELOG.md#0164--2026-06-09) for the full list.

**Previous stable**: v0.16.3 (2026-06-08) — superseded by `v0.16.4` on the same v0.16 LTS line; the official `0.16.3` release. Headline: `lts` upgrade channel + a 9-lane stabilization wave over `0.16.2` (gated by per-PR + full-branch integration codex review and orbstack Linux iso v2 VM-verify) — marquee feature `agb upgrade --channel lts` pins an install to the v0.16 LTS line (sticky, fail-closed, **#1687**); three marquee fixes: upgrade self-restart exit-code (**#1662**), `statusLine` HUD preservation through rerender (**#1689**), A2A receiver self-heal on first post-v0.15.x upgrade (**#1685**). The `v0.16.2` headline (four robustness/perf fixes #1659/#1660/#1661/#1663, flock-backend VM-verified) and `v0.15.x` / `v0.16.0`/`v0.16.1` headlines carry forward. See [CHANGELOG.md](./CHANGELOG.md#0163--2026-06-08) for the full list.
- **Recommended upgrade target.** `v0.16.4` (current stable / head of the v0.16 LTS line) supersedes `v0.16.3` / `v0.16.2` / `v0.16.1` / `v0.16.0` / `v0.15.4` / `v0.15.3` / `v0.15.2` / `v0.15.1` / `v0.15.0`, all the `v0.16.0-rc1..rc3` / `v0.15.0-rc1` / `v0.15.0-betaN` / `v0.14.5-betaN` prereleases — treat the latest stable tag as current for production. The `v0.16.0-rc1..rc3` candidates are folded into the `v0.16.0`→`v0.16.1`→`v0.16.2`→`v0.16.3`→`v0.16.4` stable line. Conservative installs that want to stay on this LTS line across future minors can pin with `agb upgrade --channel lts` (see [OPERATIONS.md §"Upgrade"](./OPERATIONS.md#upgrade)).
- v0.7.x → v0.16.4 leap path: **direct `agent-bridge upgrade --apply` to `v0.16.4`** is the current recommended (stable) path — the v0.13.7-v0.13.10 cycle fixed the Bash 5.3.9 heredoc-stdin deadlock chain by extracting leap-path bodies to `lib/upgrade-helpers/`, so the leap is single-atomic from any v0.7.x/v0.8.x/v0.9.x/v0.10.x/v0.11.x/v0.12.x/v0.13.x/v0.14.x/v0.15.x/v0.16.x source. v0.13.10 is the minimum-safe fallback if you're pinned below v0.14.x or troubleshooting an in-flight leap. See [OPERATIONS.md §"Upgrade"](./OPERATIONS.md#upgrade) for recipe + follow-up notes.
...
`v0.16.4` (stable) 이 현재 권장 target 이다. v0.7.x / v0.8.x / v0.9.x / v0.10.x / v0.11.x / v0.12.x / v0.13.x / v0.14.x / v0.15.x / v0.16.x 어디서든 단일 `upgrade --apply` 한 번으로 `v0.16.4` 까지 올라간다 (v0.13.7-v0.13.9 가 Bash 5.3.9 heredoc-chain 을 풀어놨기 때문에 중간 hop 불필요).
...
이 host 를 현재 권장 target (`v0.16.4` 이상) 까지 올린다. 끝나면 done note 한 줄 요약으로 보고.
