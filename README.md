...
**Current stable / LTS**: v0.16.6 (2026-06-10) — the current head of the **v0.16 Long-Term Support (LTS) line** and its recommended install. The LTS designation is on the `v0.16` *line*; every v0.16.x release is an LTS release — `v0.16.6` supersedes `v0.16.5` as the line head (same line, not a different LTS; v0.16.5's LTS designation is not revoked). Headline: **mesh-robustness + read-guard hardening + reliability sweep** — a 10-lane wave. Mesh/guard half: the A2A mesh rides out transient peer churn without drops or tunnel bounces (**#1732** quiet-pend / durable-no-drop / auto-resume, **#1733** WARP tunnel-bounce gated on *real peer-loss* instead of handshake-idle — verified on a live Cloudflare-WARP mesh host — and the **#1742** cross-lane fix that excludes transient peers from the bounce proof), the shared-secret / peer-home read-guard **comprehensively closes the static-decodable bypass class** (`cd`/`pushd` cwd grammar, `&&`/`||`/`;` chains, glob enumeration, ANSI-C decode, quote-aware brace expansion — **#1709**), admins get an audited durable install env-override (`config set-env`, **#1734**), and a test-bind state-path guard stops a test mesh from clobbering the live `rooms.db` (**#1728**). Reliability half: the queue-gateway socket-listener crash-loop is fixed (**#1652**), upgrades reap leaked codex-broker / queue-gateway orphans one-shot + dry-run-default (**#1567**), Homebrew interpreter / rendered-abs-path cosmetic diffs no longer raise spurious `settings.json` upgrade conflicts (**#1675**/**#1694**), shared static agents inherit the operator's global `~/.claude/settings.json` as the bottom render layer behind a credential-safe denylist (**#11901**), the A2A receiver supervisor no longer floods false crash-loop tasks on macOS tailnet self-route flaps and auto-rebinds after tailnet-IP loss (**#1679**/**#1680**), and a fresh-install admin's workdir identity is no longer clobbered by the codex-pair template (**#1750**). The `v0.16.5` headline (**zero-touch A2A Rooms mesh** — reconcile control-loop self-healing the multi-node mesh via per-transport adapters #1705/#1706/#1707, room-token join + leader relay + roster anti-entropy #1695, `agb a2a net-status` v2 status window #1708), the `v0.16.4` headline (guard over-block hardening wave #1690/#1692/#1693/#1701/#1697), the `v0.16.3` headline (`lts` upgrade channel + 9-lane stabilization wave), and earlier `v0.16.x` / `v0.15.x` headlines carry forward. See [CHANGELOG.md](./CHANGELOG.md#0166--2026-06-10) for the full list, and [docs/a2a-rooms.md](./docs/a2a-rooms.md) for operator usage of the mesh.

**Previous stable**: v0.16.5 (2026-06-09) — superseded by `v0.16.6` on the same v0.16 LTS line; the official `0.16.5` release. Headline: **zero-touch A2A Rooms mesh** — a k8s-controller-style reconcile control-loop continuously self-heals the multi-node mesh (stable-address **#1705**, tunnel-health probe+bounce **#1706**, peer-reachability **#1707**), room-token join + leader relay + roster anti-entropy (**#1695**), and the `agb a2a net-status` v2 status window (**#1708**); transport-agnostic across `cloudflare-warp-mesh` / `tailscale` with every receiver security invariant preserved. The `v0.16.4` headline (guard over-block hardening wave #1690/#1692/#1693/#1701/#1697) and earlier headlines carry forward. See [CHANGELOG.md](./CHANGELOG.md#0165--2026-06-09) for the full list.
- **Recommended upgrade target.** `v0.16.6` (current stable / head of the v0.16 LTS line) supersedes `v0.16.5` / `v0.16.4` / `v0.16.3` / `v0.16.2` / `v0.16.1` / `v0.16.0` / `v0.15.4` / `v0.15.3` / `v0.15.2` / `v0.15.1` / `v0.15.0`, all the `v0.16.0-rc1..rc3` / `v0.15.0-rc1` / `v0.15.0-betaN` / `v0.14.5-betaN` prereleases — treat the latest stable tag as current for production. The `v0.16.0-rc1..rc3` candidates are folded into the `v0.16.0`→`v0.16.1`→`v0.16.2`→`v0.16.3`→`v0.16.4`→`v0.16.5`→`v0.16.6` stable line. Conservative installs that want to stay on this LTS line across future minors can pin with `agb upgrade --channel lts` (see [OPERATIONS.md §"Upgrade"](./OPERATIONS.md#upgrade)).
- v0.7.x → v0.16.6 leap path: **direct `agent-bridge upgrade --apply` to `v0.16.6`** is the current recommended (stable) path — the v0.13.7-v0.13.10 cycle fixed the Bash 5.3.9 heredoc-stdin deadlock chain by extracting leap-path bodies to `lib/upgrade-helpers/`, so the leap is single-atomic from any v0.7.x/v0.8.x/v0.9.x/v0.10.x/v0.11.x/v0.12.x/v0.13.x/v0.14.x/v0.15.x/v0.16.x source. v0.13.10 is the minimum-safe fallback if you're pinned below v0.14.x or troubleshooting an in-flight leap. See [OPERATIONS.md §"Upgrade"](./OPERATIONS.md#upgrade) for recipe + follow-up notes.
