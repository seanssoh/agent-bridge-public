...
**Current stable / LTS**: v0.16.8 (2026-06-11) — the current head of the **v0.16 Long-Term Support (LTS) line** and its recommended install. The LTS designation is on the `v0.16` *line*; every v0.16.x release is an LTS release — `v0.16.8` supersedes `v0.16.7` as the line head (same line, not a different LTS). Headline: **session-resume + settings/CI reliability sweep, plus no-LLM picker auto-resolve** — seven live-fleet issues closed: agent restart no longer silently discards a live session via **both** mechanisms of **#1769** (a one-shot trusted-resume marker repairing the post-kill resolver gate, quarantine-aware on the #827 shortcut; and the `FORCE_FRESH_SESSION` setup-freshness gate now logging every named trip and keeping `--continue` when all trips are re-ensurable — the actual cause of an 18-session mass-fresh event); `agent update --model/--effort` now actually reaches static Claude agents (**#1763** — was a silent no-op behind "changed: yes"); iso v2 agents can read their own `workdir/.claude/settings.json` (**#1766** — 0640/0750 group-publish + canonical-symlink walker acceptance + the Settings Error picker cataloged, closing a production startup-blocking picker); the operator-global settings self-reference loop is broken with fail-safe classification (**#1759**); fresh agents get the operator's HUD display config seeded if-absent (**#1753**, allowlist + traversal-hardened); the CI iso-helper-ratchet is boundary-anchored, ending the `os.environ` false-red class (**#1764**, baseline 1561→871); and the daemon now auto-resolves interactive pickers (codex update / resume-compact / trust prompts) from a shipped fingerprint catalog with policy keystrokes and three safety rails — unknown pickers escalate to the admin with a verbatim capture (**#1762**, no LLM sweep required). Operational note: the us↔cm-prod production A2A pair was restored bidirectionally on 2026-06-11 using v0.16.7's `trusted-routed` transport (root cause: Cloudflare WARP corporate forwarding blackhole). The `v0.16.7` headline (`trusted-routed` A2A transport #1758 + the #1755/#1756 hooks/settings pair), the `v0.16.6` headline (mesh-robustness + read-guard hardening, 10 lanes), the `v0.16.5` headline (zero-touch A2A Rooms mesh), and earlier headlines carry forward. See [CHANGELOG.md](./CHANGELOG.md#0168--2026-06-11) for the full list and [docs/a2a-cross-bridge.md](./docs/a2a-cross-bridge.md) for the trusted-routed trust model + rollout recipe.

**Previous stable**: v0.16.7 (2026-06-10) — superseded by `v0.16.8` on the same v0.16 LTS line. Headline: **`trusted-routed` A2A transport** (**#1758**) — direct private-IP bind on a corporate-routed trusted private network, no WARP/Tailscale enrollment required, with a per-peer `transport.kind` override and per-destination egress source; plus the dynamic-agent every-prompt "UserPromptSubmit hook error" fix (**#1755**) and operator-pinned session preferences surviving settings re-renders (**#1756**). See [CHANGELOG.md](./CHANGELOG.md#0167--2026-06-10) for the full list.
- **Recommended upgrade target.** `v0.16.8` (current stable / head of the v0.16 LTS line) supersedes `v0.16.7` / `v0.16.6` / `v0.16.5` / `v0.16.4` / `v0.16.3` / `v0.16.2` / `v0.16.1` / `v0.16.0` / `v0.15.4` / `v0.15.3` / `v0.15.2` / `v0.15.1` / `v0.15.0`, all the `v0.16.0-rc1..rc3` / `v0.15.0-rc1` / `v0.15.0-betaN` / `v0.14.5-betaN` prereleases — treat the latest stable tag as current for production. The `v0.16.0-rc1..rc3` candidates are folded into the `v0.16.0`→`v0.16.1`→`v0.16.2`→`v0.16.3`→`v0.16.4`→`v0.16.5`→`v0.16.6`→`v0.16.7`→`v0.16.8` stable line. Conservative installs that want to stay on this LTS line across future minors can pin with `agb upgrade --channel lts` (see [OPERATIONS.md §"Upgrade"](./OPERATIONS.md#upgrade)).
- v0.7.x → v0.16.8 leap path: **direct `agent-bridge upgrade --apply` to `v0.16.8`** is the current recommended (stable) path — the v0.13.7-v0.13.10 cycle fixed the Bash 5.3.9 heredoc-stdin deadlock chain by extracting leap-path bodies to `lib/upgrade-helpers/`, so the leap is single-atomic from any v0.7.x/v0.8.x/v0.9.x/v0.10.x/v0.11.x/v0.12.x/v0.13.x/v0.14.x/v0.15.x/v0.16.x source. v0.13.10 is the minimum-safe fallback if you're pinned below v0.14.x or troubleshooting an in-flight leap. See [OPERATIONS.md §"Upgrade"](./OPERATIONS.md#upgrade) for recipe + follow-up notes.
