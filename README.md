...
**Latest pre-release**: v0.16.0-beta4 (2026-06-04) — the **create→isolate→onboard completeness** beta on the `0.16.0` line. Headline: a fresh `agent create --isolate` for a linux-user Claude agent now lands clean on the **first** create — (1) the six identity profile files (SOUL/CLAUDE/SESSION-TYPE/MEMORY/MEMORY-SCHEMA/TOOLS.md) are published to `ab-agent-<a>:0660` via a narrow **root-forced, TOCTOU-safe** helper (workdir `O_DIRECTORY|O_NOFOLLOW` dir-fd + per-basename `openat O_NOFOLLOW` + `fstat`-verify regular/iso-owner + `fchown`/`fchmod` the open fd; a planted symlink is refused and audited), independent of the controller's stale supplementary-group cache that previously left them `0600`/unreadable (**#1520c**, PR #1532); and (2) the agent is held from authless auto-start until its per-agent credential is seeded (**#1520b**, PR #1531). The watchdog classifier anchors the expected group to the workdir directory's own group (so long agent names are not misclassified), and `isolate --reapply` shares the same publish for native recovery. The fresh-create publish path is covered by the merged create→isolate smoke (`scripts/smoke/1520c-create-isolate-profile-publish.sh`); beta4 is the live-validation cut for the full create→onboard flow. **Builds on beta3**: **per-agent Claude config isolation for shared-mode agents (#1520)** — a shared `agent create` Claude agent now launches with its own `CLAUDE_CONFIG_DIR`, so it reads its pre-seeded `<agent-home>/.claude` config/MCP/credentials instead of the operator-global `~/.claude.json`; hardened across a 7-round adversarial review down to the `BASH_ENV`/allexport process-startup boundary (auth-continuity preserved on the switch; the residual `builtin`-keyword shadow is the pre-existing same-UID #1454 boundary, not a new regression) — and a **generic plugin `requires` resolver (#1528)**: a plugin's `.claude-plugin/plugin.json` may declare a top-level `"requires"` array of channel specs, and `agent create --channels plugin:X@mkt` now transitively pulls in the channels `X` declares it needs (ordered dedupe, cycle/depth-safe, warn-and-continue on an unresolvable dep, `--dry-run` previews the expansion; zero domain hardcoding in core). Also lands the multi-user **Teams/M365 OAuth `/auth/callback` demux (#1524)** — one Azure bot app fronting N per-agent listeners, with the agent id embedded in the OAuth `state` so a front router can demux the callback (marketplace teams+ms365 `0.1.1` via #1526) — three isolation/upgrade fixes (**#1516** `upgrade --apply` no longer silently downgrades a beta install to stable, **#1517** first `agb room create` on a fresh iso host auto-bootstraps the canonical `rooms.db`, **#1518** legacy-mirror chmod dead-code cleanup), and the CI fix that ends the recurring `unit/static` false-red (**#1527**, root-caused to a too-tight 10m job timeout → 15m + hermetic source snapshot). Carries forward the **A2A Rooms (beta)** foundation from beta1 + the #1513 launch fix from beta2; still **single-node** (multi-node A2A transport — Tailscale / Cloudflare Zero Trust + cross-node relay — remains upcoming). See [docs/a2a-rooms.md](./docs/a2a-rooms.md) for the rooms feature. This is a **beta** for live testing — `v0.15.4` remains the recommended stable.

**Current stable**: v0.15.4 (2026-06-03) — a new-items wave (eleven codex-pair-reviewed fixes) on top of `v0.15.3`. Headline: the **invalid `PermissionDenied` hook-key fix (#1495)** — current Claude Code rejects that key and skips the *entire* `settings.json`, so a stale legacy hook block was silently taking plugins/MCP servers and channel bots offline; the fix removes it from the tracked base and prunes any non-bridge-managed hook key on render (warning to stderr) while preserving every event the bridge actually wires. Alongside it: the **v2-split handoff delivery fix (#1497)** — the Python hook layer was v2-blind, so `NEXT-SESSION.md` was silently dropped on v2-split installs; now `bridge-run` exports a `BRIDGE_AGENT_WORKDIR_RESOLVED` scalar (initial + roster-refresh relaunch) and the hook resolvers honour `BRIDGE_AGENT_ROOT_V2` (mirroring bash) while keeping the roster fallback reachable for dynamic agents. Plus the **usage-endpoint-429-as-rotation-signal (#1468, credential roadmap Phase 0)** — a genuine `429 rate_limit_error` now persists a near-limit probe cache so proactive OAT rotation fires (breaking the near-limit catch-22, idempotent per token/window); the **bridge-lib re-exec hardening (#1454)** — blocks a shadowed `source`/`builtin`/function from running while the inherited env is still present (a bounded threat model: the ambient secret env is inherited by design, not scrubbed) — a credential Phase-0 security gate; and a cluster of v2/path + daemon fixes (#1492 admin-pair v2 workdir, #1407 `set -u` map guard, #1464 cron-aware stale health, #1472 tmux stale-spool drop, #1474 admin cron-exemption wrapper regression). The `v0.15.3` headline (the **Codex provisioning upgrade wave #8945** + the **incident #8807 process-explosion fail-safes**), `v0.15.2` (headless Claude OAT self-rotation + Darwin-gated macOS `apiKeyHelper`) and `v0.15.1` (`agb setup template-sync`) carry forward. See [CHANGELOG.md](./CHANGELOG.md#0154--2026-06-03) for the full list.
- **Recommended upgrade target.** `v0.15.4` (current stable) supersedes `v0.15.3` / `v0.15.2` / `v0.15.1` / `v0.15.0` and all the `v0.15.0-rc1` / `v0.15.0-betaN` / `v0.14.5-betaN` prereleases — treat the latest stable tag as current for production. `v0.16.0-beta4` is the latest **pre-release** (the `0.16.0` beta line — A2A Rooms + per-agent config isolation #1520 + plugin `requires` resolver #1528, single-node) — pull it only to live-test the beta features; it does not demote the `v0.15.4` stable.
- v0.7.x → v0.15.4 leap path: **direct `agent-bridge upgrade --apply` to `v0.15.4`** is the current recommended (stable) path; pull `v0.16.0-beta4` on top only to live-test the `0.16.0` beta features — the v0.13.7-v0.13.10 cycle fixed the Bash 5.3.9 heredoc-stdin deadlock chain by extracting leap-path bodies to `lib/upgrade-helpers/`, so the leap is single-atomic from any v0.7.x/v0.8.x/v0.9.x/v0.10.x/v0.11.x/v0.12.x/v0.13.x/v0.14.x source. v0.13.10 is the minimum-safe fallback if you're pinned below v0.14.x or troubleshooting an in-flight leap. See [OPERATIONS.md §"Upgrade"](./OPERATIONS.md#upgrade) for recipe + follow-up notes.
...
- 운영자 확인: `agb skills list [--agent <name>] [--json]` 으로 shared + per-agent installed skills 점검.
...
`v0.15.4` (stable) 이 현재 권장 target 이다. v0.7.x / v0.8.x / v0.9.x / v0.10.x / v0.11.x / v0.12.x / v0.13.x / v0.14.x 어디서든 단일 `upgrade --apply` 한 번으로 `v0.15.4` 까지 올라간다 (v0.13.7-v0.13.9 가 Bash 5.3.9 heredoc-chain 을 풀어놨기 때문에 중간 hop 불필요).
...
이 host 를 현재 권장 target (`v0.15.4` 이상) 까지 올린다. 끝나면 done note 한 줄 요약으로 보고.
