...
**Current stable**: v0.16.0 (2026-06-07) — the official `0.16.0` release. Headline: **A2A rooms whole-room fan-out + a Cloudflare One/WARP-Mesh transport, plus bounded AskUserQuestion for autonomous agents** — on the rc1-rc3 foundation (A2A Rooms P1-P4, the daemon-singleton supervision redesign **#1563**, and A2A inbound backpressure + the **B1-B8** receiver security audit). `agent-bridge a2a send --room` fans one message to every eligible room member (sender membership OS-actor anchored; the receiver fail-closed `room_scoped_check` untouched — **#1594**); a transport-pluggable receiver adds a **`cloudflare-warp-mesh`** kind that binds only after proving a real local WARP-Mesh interface **and** connected/enrolled WARP, fail-closed, with Tailscale + raw-IP back-compat unchanged (**#1595**); an autonomous agent that calls `AskUserQuestion` no longer hangs on the interactive picker — a PreToolUse intercept routes it to the channel with a bounded wait + autonomous fallback (**#1569**). Plus upgrade-path hardening — `agb upgrade conflicts adopt` refuses marker / unparseable / backup-less sidecars (**#1601**) and `agb upgrade --ref --dry-run` previews the requested ref without mutating the tree (**#1602**) — the Stop-hook drain no longer wastes an LLM turn on daemon-owned `[cron-dispatch]` work (**#1596**), migrate re-materializes stale workdir identity copies (**#1598**), and an **operating-manual index skill** gives every agent one pointer-only entry to the operating canon (**#1604**). Each PR codex pair-reviewed (four caught real blockers, all fixed + re-reviewed); the wave was **Linux-VM verified** (`agb upgrade` dry-run clean + the full per-feature smoke battery green). No `agb upgrade` schema/contract change; wire protocol `a2a-enqueue-v1` unchanged. The 2-node A2A rooms fan-out + 2-node WARP live validations are **deferred** to multi-node / WARP infra (smoke + single-node + adversarial review done). The `v0.15.4` line (#1495 hook-key fix, #1497 v2 handoff, #1468 usage-429 rotation, #1454 re-exec hardening) and earlier `v0.15.x` headlines carry forward. See [CHANGELOG.md](./CHANGELOG.md#0160--2026-06-07) for the full list.

**Previous stable**: v0.15.4 (2026-06-03) — superseded by `v0.16.0`; a new-items wave (eleven codex-pair-reviewed fixes) on top of `v0.15.3`. Headline: the **invalid `PermissionDenied` hook-key fix (#1495)** — current Claude Code rejects that key and skips the *entire* `settings.json`, so a stale legacy hook block was silently taking plugins/MCP servers and channel bots offline; the fix removes it from the tracked base and prunes any non-bridge-managed hook key on render (warning to stderr) while preserving every event the bridge actually wires. Alongside it: the **v2-split handoff delivery fix (#1497)** — the Python hook layer was v2-blind, so `NEXT-SESSION.md` was silently dropped on v2-split installs; now `bridge-run` exports a `BRIDGE_AGENT_WORKDIR_RESOLVED` scalar (initial + roster-refresh relaunch) and the hook resolvers honour `BRIDGE_AGENT_ROOT_V2` (mirroring bash) while keeping the roster fallback reachable for dynamic agents. Plus the **usage-endpoint-429-as-rotation-signal (#1468, credential roadmap Phase 0)** — a genuine `429 rate_limit_error` now persists a near-limit probe cache so proactive OAT rotation fires (breaking the near-limit catch-22, idempotent per token/window); the **bridge-lib re-exec hardening (#1454)** — blocks a shadowed `source`/`builtin`/function from running while the inherited env is still present (a bounded threat model: the ambient secret env is inherited by design, not scrubbed) — a credential Phase-0 security gate; and a cluster of v2/path + daemon fixes (#1492 admin-pair v2 workdir, #1407 `set -u` map guard, #1464 cron-aware stale health, #1472 tmux stale-spool drop, #1474 admin cron-exemption wrapper regression). The `v0.15.3` headline (the **Codex provisioning upgrade wave #8945** + the **incident #8807 process-explosion fail-safes**), `v0.15.2` (headless Claude OAT self-rotation + Darwin-gated macOS `apiKeyHelper`) and `v0.15.1` (`agb setup template-sync`) carry forward. See [CHANGELOG.md](./CHANGELOG.md#0154--2026-06-03) for the full list.
- **Recommended upgrade target.** `v0.16.0` (current stable) supersedes `v0.15.4` / `v0.15.3` / `v0.15.2` / `v0.15.1` / `v0.15.0`, all the `v0.16.0-rc1..rc3` / `v0.15.0-rc1` / `v0.15.0-betaN` / `v0.14.5-betaN` prereleases — treat the latest stable tag as current for production. The `v0.16.0-rc1..rc3` candidates are now folded into the `v0.16.0` stable.
- v0.7.x → v0.16.0 leap path: **direct `agent-bridge upgrade --apply` to `v0.16.0`** is the current recommended (stable) path — the v0.13.7-v0.13.10 cycle fixed the Bash 5.3.9 heredoc-stdin deadlock chain by extracting leap-path bodies to `lib/upgrade-helpers/`, so the leap is single-atomic from any v0.7.x/v0.8.x/v0.9.x/v0.10.x/v0.11.x/v0.12.x/v0.13.x/v0.14.x/v0.15.x source. v0.13.10 is the minimum-safe fallback if you're pinned below v0.14.x or troubleshooting an in-flight leap. See [OPERATIONS.md §"Upgrade"](./OPERATIONS.md#upgrade) for recipe + follow-up notes.
...
`v0.16.0` (stable) 이 현재 권장 target 이다. v0.7.x / v0.8.x / v0.9.x / v0.10.x / v0.11.x / v0.12.x / v0.13.x / v0.14.x / v0.15.x 어디서든 단일 `upgrade --apply` 한 번으로 `v0.16.0` 까지 올라간다 (v0.13.7-v0.13.9 가 Bash 5.3.9 heredoc-chain 을 풀어놨기 때문에 중간 hop 불필요).
...
이 host 를 현재 권장 target (`v0.16.0` 이상) 까지 올린다. 끝나면 done note 한 줄 요약으로 보고.
