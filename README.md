...
**Current stable**: v0.14.0 (2026-05-16) — stabilization milestone batching S0-S3 + S5 Track A1/A2.
- Recommended upgrade target. v0.14.0 silences the macOS `ensure_matrix_path failed` stop-hook noise, demotes the `BRIDGE_LAYOUT=legacy` env-leak hard-die to a warning when a valid v2 marker exists on disk, and introduces the `BRIDGE_ISOLATION_REQUIRED=auto|yes|no` env knob.
- v0.7.x → v0.14.x leap path: **direct `agent-bridge upgrade --apply` to v0.14.0** is the current recommended path — the v0.13.7-v0.13.10 cycle fixed the Bash 5.3.9 heredoc-stdin deadlock chain by extracting leap-path bodies to `lib/upgrade-helpers/`, so the leap is single-atomic from any v0.7.x/v0.8.x/v0.9.x/v0.10.x/v0.11.x/v0.12.x/v0.13.x source. v0.13.10 is the minimum-safe fallback if you're pinned below v0.14.0 or troubleshooting an in-flight leap. See [OPERATIONS.md §"Upgrade"](./OPERATIONS.md#upgrade) for recipe + follow-up notes.
- macOS: works in shared-mode (default). Linux-user isolation is Linux-only — v0.14.0's platform discriminator makes the gate explicit (`BRIDGE_ISOLATION_REQUIRED`).
- Stabilization roadmap: [`docs/stabilization-plan-2026-05-15.md`](./docs/stabilization-plan-2026-05-15.md). v0.14.0 cleared S0-S3 + S5 Track A1/A2; S4 (Bash 3.2 cleanup), S5 B/C/D, S6+ are deferred to next cycle.
...
> **플랫폼 지원 범위**: macOS는 개발자용입니다. 멀티 테넌트 격리(`BRIDGE_AGENT_ISOLATION_MODE=linux-user`, 차후 `agent-bridge isolate`)는 Linux에서만 동작합니다. macOS는 기본 `shared` 모드 + hook 계층 격리(tool policy, prompt guard)까지만 지원합니다. 자세한 지원 매트릭스는 [docs/platform-support.md](./docs/platform-support.md)를 참고하세요.
...
~/.agent-bridge/agent-bridge upgrade             # apply (default subcommand, 모든 버전 호환)
```

각 release 의 후속 행동은 [`OPERATOR_ACTIONS_PENDING.md`](./OPERATOR_ACTIONS_PENDING.md) 의 release section 으로 surface 되며, `[upgrade-complete]` admin task body 가 자동 reference 한다.

### 다른 server 의 admin (patch) 한테 보낼 prompt template

queue task 또는 외부 채널로 그쪽 admin 한테 그대로 보내면 됨:

````markdown
# [upgrade-action] v0.6.39 — agent-bridge 업그레이드 + 자동 회복

이 host 를 v0.6.39 까지 올리고 알려진 두 regression (#488 patch source, #491 autoCompactWindow propagate) 을 회복한다. 끝나면 done note 한 줄 요약으로 보고.

## 1단계 — 업그레이드

```bash
~/.agent-bridge/agent-bridge upgrade --dry-run
~/.agent-bridge/agent-bridge upgrade
```

(인자 없는 `upgrade` 가 default apply. 모든 버전 호환. v0.6.39 부터는 `upgrade --apply` alias 도 동작.)

검증:
```bash
cat ~/.agent-bridge/VERSION                # 0.6.39
~/.agent-bridge/agent-bridge daemon status
```

## 2단계 — patch source 회복 (#488)

```bash
~/.agent-bridge/agent-bridge agent reclassify --dry-run
# candidate 1+ 이면 apply, 0 이면 skip
~/.agent-bridge/agent-bridge agent reclassify --apply
```

## 3단계 — settings rerender (#491)

```bash
~/.agent-bridge/agent-bridge agent rerender-settings --dry-run
# needs-rerender 1+ 이면 apply, 0 이면 skip
~/.agent-bridge/agent-bridge agent rerender-settings --apply
```
...
```
final_version=0.6.39, reclassify=<count|no-op>, rerender=<count|no-op>, operator-actions=<summary or none>
```

문제 시 (`upgrade` 실패, daemon 안 뜸, conflict file) `agb update --status blocked --note "<reason>"` + 외부 채널로 사람 operator 보고. 절대 `daemon stop` 분리 실행 금지 (#314, #315).

## Reference
- 표준 절차: source root `UPGRADING.md`
- Release notes: https://github.com/SYRS-AI/agent-bridge-public/releases/tag/v0.6.39
````

매 release 마다 prompt 의 버전 번호와 #2/#3 의 issue reference 만 갱신하면 된다. v0.6.40 이상에서는 `OPERATOR_ACTIONS_PENDING.md` 의 신규 section 만 추가로 보고.
