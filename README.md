...
**Current stable**: v0.16.1 (2026-06-08) — the official `0.16.1` release. Headline: **stabilization + hardening over `0.16.0` — an A2A audit follow-up, iso-upgrade robustness, an MS365 cross-plugin token-refresh, and a complete `set -u` hardening of the roster-map accessors.** The A2A wave hardens inbound backpressure (fail-**OPEN** on a count-read error so a peer isn't silently 503-blocked, **#1623**), the sender deliver-loop (per-row guard **#1628**, outbox retry/backoff reset **#1618**), fresh-arrival nudges + read-only nudge DB (**#1633/#1634**), receiver cycle-on-`--restart-daemon` (**#1614**), and a `GET /healthz` semaphore exemption (**#1645**). Upgrade-path: **iso-agent upgrades no longer abort on the backup scan** (graceful-skip of controller-unreadable per-UID files, **#1635**) and the non-identity `_template` scaffolding (`.claude/commands/`, capture gitkeeps, `session-type-files/`, `codex/`) now **propagates to the agent workdir** with skip-existing + per-agent `target_dir` cross-agent-write containment (**#1636**); settings.json cosmetic-only diffs (hook order / `python3` vs `/usr/bin/python3`) stop forcing spurious `.upgrade-conflict`s (**#1638**). **MS365 `get_valid_token`** — an MCP tool **and** a `bun server.ts get-valid-token` CLI — hands trusted in-fleet callers (e.g. the CRM proxy) a guaranteed-valid Graph token, refreshing via the stored `refresh_token` and never exposing it (**#1650**, live-verified on real M365). A **complete nounset sweep** guards every `BRIDGE_AGENT_*` roster-map accessor — per-agent reads, `[[ -v ]]` predicates, key-iterations, and existence probes — so a scalar/undeclared map can't abort a fresh agent launch/restart under `set -u` (**#1627**). Plus a **post-restart auto-wake** so an auto-restarted session doesn't sit idle (**#1639**), a unified deferred-notification staleness gate that drops a done-task replay (**#1619**), and a daemon socket-restart CI-flake fix (**#1644**). Each PR codex pair-reviewed (the gate caught a cross-agent scaffold-write escape, a CLI no-upn exit-0, and the full nounset accessor set — all fixed + re-reviewed); **Linux-VM verified on iso v2** (`agb upgrade --channel current` clean — 12 agents incl an iso agent migrated no-abort, daemon healthy, per-fix Linux smokes green). Same `agb upgrade` path; wire protocol `a2a-enqueue-v1` unchanged. The `v0.16.0` headline (A2A rooms whole-room fan-out **#1594** + Cloudflare One/WARP-Mesh transport **#1595** + bounded AskUserQuestion **#1569**, on the rc1-rc3 foundation; 2-node rooms/WARP live validations still deferred) and earlier `v0.15.x` headlines carry forward. See [CHANGELOG.md](./CHANGELOG.md#0161--2026-06-08) for the full list.

**Previous stable**: v0.16.0 (2026-06-07) — superseded by `v0.16.1`; the official `0.16.0` stable. Headline: **A2A rooms whole-room fan-out (#1594) + a Cloudflare One/WARP-Mesh transport (#1595) + bounded AskUserQuestion for autonomous agents (#1569)** — on the rc1-rc3 foundation (A2A Rooms P1-P4, the daemon-singleton supervision redesign #1563, A2A inbound backpressure + the B1-B8 receiver security audit), plus upgrade-path hardening (`conflicts adopt` sidecar guards #1601, `--ref --dry-run` no-mutate preview #1602, Stop-hook cron-dispatch skip #1596, migrate workdir re-materialize #1598) and an operating-manual index skill (#1604). The 2-node A2A rooms fan-out + 2-node WARP live validations remain deferred to multi-node/WARP infra. The `v0.15.4` line (#1495 hook-key fix, #1497 v2 handoff, #1468 usage-429 rotation, #1454 re-exec hardening) and earlier `v0.15.x` headlines carry forward. See [CHANGELOG.md](./CHANGELOG.md#0160--2026-06-07) for the full list.
- **Recommended upgrade target.** `v0.16.1` (current stable) supersedes `v0.16.0` / `v0.15.4` / `v0.15.3` / `v0.15.2` / `v0.15.1` / `v0.15.0`, all the `v0.16.0-rc1..rc3` / `v0.15.0-rc1` / `v0.15.0-betaN` / `v0.14.5-betaN` prereleases — treat the latest stable tag as current for production. The `v0.16.0-rc1..rc3` candidates are folded into the `v0.16.0`→`v0.16.1` stable line.
- v0.7.x → v0.16.1 leap path: **direct `agent-bridge upgrade --apply` to `v0.16.1`** is the current recommended (stable) path — the v0.13.7-v0.13.10 cycle fixed the Bash 5.3.9 heredoc-stdin deadlock chain by extracting leap-path bodies to `lib/upgrade-helpers/`, so the leap is single-atomic from any v0.7.x/v0.8.x/v0.9.x/v0.10.x/v0.11.x/v0.12.x/v0.13.x/v0.14.x/v0.15.x/v0.16.x source. v0.13.10 is the minimum-safe fallback if you're pinned below v0.14.x or troubleshooting an in-flight leap. See [OPERATIONS.md §"Upgrade"](./OPERATIONS.md#upgrade) for recipe + follow-up notes.
...
`v0.16.1` (stable) 이 현재 권장 target 이다. v0.7.x / v0.8.x / v0.9.x / v0.10.x / v0.11.x / v0.12.x / v0.13.x / v0.14.x / v0.15.x / v0.16.x 어디서든 단일 `upgrade --apply` 한 번으로 `v0.16.1` 까지 올라간다 (v0.13.7-v0.13.9 가 Bash 5.3.9 heredoc-chain 을 풀어놨기 때문에 중간 hop 불필요).
...
이 host 를 현재 권장 target (`v0.16.1` 이상) 까지 올린다. 끝나면 done note 한 줄 요약으로 보고.
