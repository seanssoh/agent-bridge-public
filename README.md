...
**Current version**: v0.15.1 (2026-05-31) — a stabilization batch on top of `v0.15.0` plus one new opt-in feature. Headline: **`agb setup template-sync`** — seed a freshly-created agent's model / effort / permission-mode / skills / plugins / channels from a reference agent so new agents no longer come up bare (Sonnet / low-effort / no plugins); reference read is roster-only and **never copies credentials / tokens / MCP secrets**, the whole command + the roster-write verbs are gated (operator-tui / operator-trusted-id) + audited (see [`docs/template-sync-design.md`](./docs/template-sync-design.md) and [OPERATIONS.md](./OPERATIONS.md)). Also: the launch default model bumps to `claude-opus-4-8`; cron text-route + `--kind shell` macOS / non-iso fixes; and daemon nudge/alert hygiene from external-contributor reports (#1408/#1409/#1411). The `v0.15.0` headline (A2A cross-bridge self-heal, handoffd receiver supervision, the `agb a2a setup` wizard, the iso-v2 hardening campaign) carries forward. See [CHANGELOG.md](./CHANGELOG.md#0151--2026-05-31) for the full list.
- **Recommended upgrade target.** `v0.15.1` supersedes `v0.15.0` and all the `v0.15.0-rc1` / `v0.15.0-betaN` / `v0.14.5-betaN` prereleases — treat the latest stable tag as current.
- v0.7.x → v0.15.1 leap path: **direct `agent-bridge upgrade --apply` to `v0.15.1`** is the current recommended path — the v0.13.7-v0.13.10 cycle fixed the Bash 5.3.9 heredoc-stdin deadlock chain by extracting leap-path bodies to `lib/upgrade-helpers/`, so the leap is single-atomic from any v0.7.x/v0.8.x/v0.9.x/v0.10.x/v0.11.x/v0.12.x/v0.13.x/v0.14.x source. v0.13.10 is the minimum-safe fallback if you're pinned below v0.14.x or troubleshooting an in-flight leap. See [OPERATIONS.md §"Upgrade"](./OPERATIONS.md#upgrade) for recipe + follow-up notes.
