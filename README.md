...
**Current stable / LTS**: v0.16.9 (2026-06-12) — the current head of the **v0.16 Long-Term Support (LTS) line** and its recommended install. The LTS designation is on the `v0.16` *line*; every v0.16.x release is an LTS release — `v0.16.9` supersedes `v0.16.8` as the line head (same line, not a different LTS). Headline: **fleet-reliability sweep from a day of live v0.16.8 field reports — headlined by the idle-reaper policy fix that was silently killing operator-created agents.** Seven issues closed, every one surfaced by running v0.16.8 on real multi-agent installs the day it shipped: the daemon idle dynamic-agent reaper conflated "dynamic" (creation method) with "disposable" and was reaping operator-created long-lived agents incl. `loop=1` ones — confirmed root cause of an operator codex pair destroyed four separate times (**#1795** — reap now requires `ephemeral=="1"` AND `loop!=1`, operator/loop dynamics never reaped, migration fail-safe); upgrade doc-migration no longer silently rolls back agent `MEMORY.md` (**#1781** — 13/22 agents on one host); the no-LLM picker stopped false-escalating every idle composer screen (**#1783** — 19/22 sessions within minutes); post-upgrade `verify-tasks-db` works from the admin agent + the "unable to open database file" mystery is root-caused to a WAL `mode=ro`/`-shm` open (**#1786**, plus a #1709-class env-assignment guard tightening); `doctor` no longer guides quarantining a live agent on case-insensitive filesystems and stops false-flagging the intended settings layout (**#1787**/**#1788**); and cron-dispatched children are fenced to their job with attributable, spoof-resistant task origin (**#1792**). Operational note: the us↔cm-prod production A2A pair was restored on 2026-06-11 using v0.16.7's `trusted-routed` transport (root cause: Cloudflare WARP corporate forwarding blackhole). The `v0.16.8` headline (session-resume + settings/CI sweep + picker auto-resolve, 7 issues), the `v0.16.7` headline (`trusted-routed` A2A transport), and earlier headlines carry forward. See [CHANGELOG.md](./CHANGELOG.md#0169--2026-06-12) for the full list.

**Previous stable**: v0.16.8 (2026-06-11) — superseded by `v0.16.9` on the same v0.16 LTS line. Headline: **session-resume + settings/CI reliability sweep, plus no-LLM picker auto-resolve** (**#1769** both restart-resume mechanisms, **#1763** static model/effort, **#1766** iso own-settings, **#1759** self-ref settings loop, **#1753** HUD seed, **#1764** ratchet anchoring, **#1762** picker auto-resolve). See [CHANGELOG.md](./CHANGELOG.md#0168--2026-06-11).
- **Recommended upgrade target.** `v0.16.9` (current stable / head of the v0.16 LTS line) supersedes `v0.16.8` / `v0.16.7` / `v0.16.6` / `v0.16.5` / `v0.16.4` / `v0.16.3` / `v0.16.2` / `v0.16.1` / `v0.16.0` / `v0.15.4` / `v0.15.3` / `v0.15.2` / `v0.15.1` / `v0.15.0`, all the `v0.16.0-rc1..rc3` / `v0.15.0-rc1` / `v0.15.0-betaN` / `v0.14.5-betaN` prereleases — treat the latest stable tag as current for production. The `v0.16.0-rc1..rc3` candidates are folded into the `v0.16.0`→`v0.16.1`→`v0.16.2`→`v0.16.3`→`v0.16.4`→`v0.16.5`→`v0.16.6`→`v0.16.7`→`v0.16.8`→`v0.16.9` stable line. Conservative installs that want to stay on this LTS line across future minors can pin with `agb upgrade --channel lts` (see [OPERATIONS.md §"Upgrade"](./OPERATIONS.md#upgrade)).
- v0.7.x → v0.16.9 leap path: **direct `agent-bridge upgrade --apply` to `v0.16.9`** is the current recommended (stable) path — the v0.13.7-v0.13.10 cycle fixed the Bash 5.3.9 heredoc-stdin deadlock chain by extracting leap-path bodies to `lib/upgrade-helpers/`, so the leap is single-atomic from any v0.7.x/v0.8.x/v0.9.x/v0.10.x/v0.11.x/v0.12.x/v0.13.x/v0.14.x/v0.15.x/v0.16.x source. v0.13.10 is the minimum-safe fallback if you're pinned below v0.14.x or troubleshooting an in-flight leap. See [OPERATIONS.md §"Upgrade"](./OPERATIONS.md#upgrade) for recipe + follow-up notes.
