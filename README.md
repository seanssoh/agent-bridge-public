...
**Current stable / LTS**: v0.16.5 (2026-06-09) — the current head of the **v0.16 Long-Term Support (LTS) line** and its recommended install. The LTS designation is on the `v0.16` *line*; every v0.16.x release is an LTS release — `v0.16.5` is a v0.16.x LTS-line release, superseding `v0.16.4` as the line head (same line, not a different LTS; v0.16.4's LTS designation is not revoked). Headline: **zero-touch A2A Rooms mesh** — a k8s-controller-style **reconcile control-loop** runs on the handoff-daemon tick and continuously self-heals the multi-node mesh, converging each node's *actual* network state toward the only human input (*desired* room membership + transport config). Three per-transport **self-heal adapters** keep the mesh live without operator intervention: stable-address acquisition (**#1705**), tunnel-health probe + bounce (**#1706**), and peer-reachability (**#1707**). New nodes join from a **room token** (sha256(token) on the wire + HKDF per-pair key + the existing two-factor admission), a **leader relays** member↔member traffic, and the **roster anti-entropies** itself (**#1695** P1/P2). `agb a2a net-status` becomes the control-loop's **status window** — a v2 read-only view of bridge_id / transport / listen addr / receiver healthz / substrate health / reconcile FSM step state / peers / room membership (**#1708**, alias `agb a2a status`). The mesh is **transport-agnostic** — identical on `cloudflare-warp-mesh` and `tailscale`; only stable-address acquisition and tunnel-health probing live behind per-transport adapters — and preserves every receiver security invariant (fail-closed bind proof, HMAC, allowlist, dedupe, `remote_addr`, two-factor admission). The `v0.16.4` headline (guard over-block hardening wave #1690/#1692/#1693/#1701/#1697), the `v0.16.3` headline (`lts` upgrade channel + 9-lane stabilization wave #1687/#1662/#1689/#1685/#1667/#1677/#1670/#1672/#1671/#1683), the `v0.16.2` headline (four robustness/perf fixes #1659/#1660/#1661/#1663, flock-backend VM-verified), the `v0.16.1` headline (A2A audit follow-up, iso-upgrade robustness #1635/#1636, MS365 `get_valid_token` #1650, `set -u` roster-map hardening #1627), and earlier `v0.16.0` / `v0.15.x` headlines carry forward. See [CHANGELOG.md](./CHANGELOG.md#0165--2026-06-09) for the full list, and [docs/a2a-rooms.md](./docs/a2a-rooms.md) for operator usage of the mesh.

**Previous stable**: v0.16.4 (2026-06-09) — superseded by `v0.16.5` on the same v0.16 LTS line; the official `0.16.4` release. Headline: **guard over-block hardening wave** — five targeted relaxations that reduce false-deny friction in `hooks/tool-policy.py` while keeping all containment intact: `state/tasks.db` read carve-out with a fail-closed read-intent classifier (**#1690**), admin Bash read of a peer agent's home no longer falsely denied (**#1692**), read-only viewer commands classified read-intent + a prose-mention false-fire fix (**#1693**), WARP-mesh A2A receiver no longer crash-loops a healthy `/32`-bound receiver (**#1701**), and the read-only `agb a2a net-status` verb (**#1697**). The `v0.16.3` headline (`lts` upgrade channel + 9-lane stabilization wave) and `v0.15.x` / `v0.16.0`/`v0.16.1`/`v0.16.2` headlines carry forward. See [CHANGELOG.md](./CHANGELOG.md#0164--2026-06-09) for the full list.
- **Recommended upgrade target.** `v0.16.5` (current stable / head of the v0.16 LTS line) supersedes `v0.16.4` / `v0.16.3` / `v0.16.2` / `v0.16.1` / `v0.16.0` / `v0.15.4` / `v0.15.3` / `v0.15.2` / `v0.15.1` / `v0.15.0`, all the `v0.16.0-rc1..rc3` / `v0.15.0-rc1` / `v0.15.0-betaN` / `v0.14.5-betaN` prereleases — treat the latest stable tag as current for production. The `v0.16.0-rc1..rc3` candidates are folded into the `v0.16.0`→`v0.16.1`→`v0.16.2`→`v0.16.3`→`v0.16.4`→`v0.16.5` stable line. Conservative installs that want to stay on this LTS line across future minors can pin with `agb upgrade --channel lts` (see [OPERATIONS.md §"Upgrade"](./OPERATIONS.md#upgrade)).
- v0.7.x → v0.16.5 leap path: **direct `agent-bridge upgrade --apply` to `v0.16.5`** is the current recommended (stable) path — the v0.13.7-v0.13.10 cycle fixed the Bash 5.3.9 heredoc-stdin deadlock chain by extracting leap-path bodies to `lib/upgrade-helpers/`, so the leap is single-atomic from any v0.7.x/v0.8.x/v0.9.x/v0.10.x/v0.11.x/v0.12.x/v0.13.x/v0.14.x/v0.15.x/v0.16.x source. v0.13.10 is the minimum-safe fallback if you're pinned below v0.14.x or troubleshooting an in-flight leap. See [OPERATIONS.md §"Upgrade"](./OPERATIONS.md#upgrade) for recipe + follow-up notes.
