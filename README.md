...
**Current stable / LTS**: v0.16.19 (2026-06-28) — the current head of the **v0.16 Long-Term Support (LTS) line** and its recommended install. The LTS designation is on the `v0.16` *line*; every v0.16.x release is an LTS release — `v0.16.19` supersedes `v0.16.18` as the line head (same line, not a different LTS). Headline: **the #2171 / #19460 M4 fleet-down token-rotation regression is closed on the LTS line.** The M4 incident: reactive Claude-token rotation re-picked a stale / limited candidate when the usage probe degraded (403 / edge-blocked) and the whole fleet went dark. This backports the **core** of the hybrid fix (the same four PRs shipped on `v0.17.0-beta7`): the daemon now **live-probes a rotation candidate before committing it** (`cmd_rotate --preflight` — locked snapshot → unlocked probe → locked revalidate → commit, never under `registry_lock`; daemon usage-monitor rotate runs it under a **global preflight budget**, fail-closed on exhaustion → existing `skipped:all_tokens_limited` + #1789 envelope), **prefers a measured-fresh candidate** (measured-usage prefilter keyed by `_token_signal_digest`, additive / fail-open), and **stops a macOS keychain shadow from silently re-authing onto a dead personal token** (`reconcile-keychain` self-heal — default fail-closed diagnostic, `--apply` operator-gated). A small `classify_token_kind` / `active_registry_token_kind` classifier (#18696 / #2141) was backported to the v0.16 line so the cherry-picked preflight could gate OAT-native probing. **The remaining PR-B2 Part 2 (daemon `active_dead_recover` for the 401/403 auth-dead marker + blind-nudge suppression) is a tracked fast-follow.** `--preflight` OFF = byte-identical legacy. Lineage: **v0.16.18 (2026-06-27)** made fleet token auto-rotation *enable-able* on the LTS line (the **`global-auth-sync enable|disable|status` verb**, #19140 / #2155 backport, + **#18849 Part 2** daemon threshold-gate / cascade / warn, default-OFF via two gates + the **#19146 PreToolUse Bash guard** #2157); **v0.16.17 (2026-06-27)** was a security + auth/A2A reliability LTS bundle (the **#2137 keychain-free auth-hijack root-fix** promoted to official, the **#2146 macOS-NAT64 A2A receiver-crash fleet fix**, and **#18849 Part 1** seamless-rotation machinery gated default-OFF / INERT); **v0.16.16 (2026-06-26)** was a Teams reply silent-non-delivery fix (**#2112**) + the **LTS endpoint at which mainline development moved to `v0.17`**, and earlier v0.16.x headlines carry forward. See [CHANGELOG.md](./CHANGELOG.md#01619--2026-06-28-lts) for the full list.

**Previous stable**: v0.16.18 (2026-06-27) — superseded by `v0.16.19` on the same v0.16 LTS line. Headline: **fleet token auto-rotation becomes *enable-able* on the LTS line — the fleet-delivery cut.** It landed the **`global-auth-sync enable|disable|status` verb (#19140 / #2155 backport)** — a first-class, persisted, headless-safe operator switch for operator-global seamless credential propagation (so a rotated token reaches a dynamic-vanilla Claude agent without a restart) — plus **#18849 Part 2** (daemon threshold-gate at the **95%** mark + **cascade-to-next-available** + **warn-on-exhaustion**, #2147 / #2148 / #17927-P2 backport), backported **DEFAULT-OFF for fresh installs via two independent gates** (per-token `auto_rotate` + `global-auth-sync`), bundled with the **#19146 PreToolUse Bash guard (#2157 backport, security)**. See [CHANGELOG.md](./CHANGELOG.md#01618--2026-06-27-lts). Before it, **v0.16.17 (2026-06-27)** was a security + auth/A2A reliability LTS bundle (the **#2137 keychain-free auth-hijack root-fix**, the **#2146 macOS-NAT64 A2A receiver-crash fleet fix**, and **#18849 Part 1** INERT), **v0.16.16 (2026-06-26)** was a Teams reply silent-non-delivery fix (**#2112**) + the **LTS endpoint at which mainline development moved to `v0.17`**, and **v0.16.15 (2026-06-17)** was a queue-gateway poison-request stability hotfix (cm-prod F8).
- **Recommended upgrade target.** `v0.16.19` (current stable / head of the v0.16 LTS line) supersedes `v0.16.18` / `v0.16.17` / `v0.16.16` / `v0.16.15` / `v0.16.14` / `v0.16.13` / `v0.16.12` / `v0.16.11` / `v0.16.10` / `v0.16.9` / `v0.16.8` / `v0.16.7` / `v0.16.6` / `v0.16.5` / `v0.16.4` / `v0.16.3` / `v0.16.2` / `v0.16.1` / `v0.16.0` / `v0.15.4` / `v0.15.3` / `v0.15.2` / `v0.15.1` / `v0.15.0`, all the `v0.16.17-rc1` / `v0.16.16-rc1..rc4` / `v0.16.13-rc1..rc2` / `v0.16.11-rc1` / `v0.16.10-rc1..rc3` / `v0.16.0-rc1..rc3` / `v0.15.0-rc1` / `v0.15.0-betaN` / `v0.14.5-betaN` prereleases — treat the latest stable tag as current for production. The `v0.16.17-rc1` candidate is folded into the `v0.16.17` stable LTS tag (as are the `v0.16.16-rc1..rc4` candidates into `v0.16.16` and the `v0.16.13-rc1` / `-rc2` candidates into `v0.16.13`). Conservative installs that want to stay on this LTS line across future minors can pin with `agb upgrade --channel lts` (see [OPERATIONS.md §"Upgrade"](./OPERATIONS.md#upgrade)).
- v0.7.x → v0.16.19 leap path: **direct `agent-bridge upgrade --apply` to `v0.16.19`** is the current recommended (stable) path — the v0.13.7-v0.13.10 cycle fixed the Bash 5.3.9 heredoc-stdin deadlock chain by extracting leap-path bodies to `lib/upgrade-helpers/`, so the leap is single-atomic from any v0.7.x/v0.8.x/v0.9.x/v0.10.x/v0.11.x/v0.12.x/v0.13.x/v0.14.x/v0.15.x/v0.16.x source. On an iso-v2 host, the automatic fresh-group preflight (KNOWN_ISSUES §28 / #1836, shipped in v0.16.11) now warns when the controller's supplementary group set is stale so the cosmetic `Errno13` reconcile warnings no longer surprise you — running the upgrade from a fresh login shell (or `newgrp` into the agent groups) still avoids them outright. v0.13.10 is the minimum-safe fallback if you're pinned below v0.14.x or troubleshooting an in-flight leap. See [OPERATIONS.md §"Upgrade"](./OPERATIONS.md#upgrade) for recipe + follow-up notes.
...
이 host 를 현재 권장 target (`v0.16.19` 이상) 까지 올린다. 끝나면 done note 한 줄 요약으로 보고.
