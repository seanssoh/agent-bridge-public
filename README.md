...
**Current stable / LTS**: v0.16.12 (2026-06-15) — the current head of the **v0.16 Long-Term Support (LTS) line** and its recommended install. The LTS designation is on the `v0.16` *line*; every v0.16.x release is an LTS release — `v0.16.12` supersedes `v0.16.11` as the line head (same line, not a different LTS). Headline: **the upgrade respawn-race fix** — the #1820 layout-v2 reconcile quiesce is now **init-system-aware** on both **systemd (#1905)** and **launchd (#655)**, closing an `rc=3` half-applied-upgrade footgun that hit every systemd-managed install: the quiesce stopped the daemon only at the script level, so the init system respawned it inside the quiesce window → the fail-closed reconcile fence saw a live pid → abort. The upgrader now stops the liveness timer + service via `systemctl --user` (establishing `XDG_RUNTIME_DIR` first, a real-host catch) on systemd, and `disable`→`bootout`→`bootstrap`→`kickstart`s the LaunchAgent on launchd — validated by cm-prod's #1905 host-acceptance passing **5/5** (the exact footgun that required a manual unit-stop on v0.16.11 now upgrades untouched). Also: **iso transcript harvest crosses the iso boundary via run-as-iso (#1894)** — the controller-UID memory harvester now reads iso agents' `<iso-home>/.claude/projects` as the iso UID via the sudoers `bash` allowlist instead of falling to `--skipped-permission` forever; **resume config-dir resolver realpath-hardened (#1893, #1889 fast-follow)** against a `.claude` symlinked to the operator's global HOME (and the quarantine-archive refuses to move a transcript that realpaths into the operator's projects root); and **usage probe CDN-edge classification (#1824)** — a Cloudflare 429/403 with no anthropic origin headers is classified `edge-blocked` and never fabricates a synthetic near-limit cache that would poison rotation, with per-token backoff and both delta-seconds + RFC 7231 HTTP-date `Retry-After` forms honored. The `v0.16.11` headline (dynamic agents become vanilla-equivalent — a dynamic Claude or Codex agent runs like vanilla Claude Code / Codex CLI against the operator's global config with native resume, and the bridge layers only its comms/queue hooks via project-local settings, superseding the private-config-dir isolation that was destroying operator sessions — **#1900**/**#1901**), the `v0.16.10` headline (dynamic-agent restart + layout-v2 migration train), the `v0.16.9` headline (idle-reaper policy fix), and earlier headlines carry forward. See [CHANGELOG.md](./CHANGELOG.md#01612--2026-06-15-lts) for the full list.

**Previous stable**: v0.16.11 (2026-06-15) — superseded by `v0.16.12` on the same v0.16 LTS line. Headline: **dynamic agents become vanilla-equivalent — a dynamic Claude or Codex agent runs like vanilla Claude Code / Codex CLI against the operator's global config (settings, model, session history, installed plugins, MCP, credentials) with native resume (`claude -c` / `codex resume --last`), and the bridge layers only its comms/queue hooks via project-local settings — superseding the private-config-dir isolation that was destroying operator sessions** (**#1900**/**#1901**), plus iso-v2 create-path completeness (**#1895**), roster-first fail-closed doc-backfill engine detection (**#1896**), and a 3-way-sharded unit/static CI smoke battery (**#1898**). See [CHANGELOG.md](./CHANGELOG.md#01611--2026-06-15-lts). Before it, **v0.16.10 (2026-06-13)** was a large reliability + migration train, hardened through a fleet soak across sean-mac (macOS shared-mode) and cm-prod (first real Linux iso-v2 production host) — `agent restart <dynamic>` relaunch + self-restart survival (**#1852**/**#1853**), provisioning preserve across recreate (**#1857**), a gated layout-v2 four-writer migration with fail-closed v1→v2 reconciliation (**#1820**), iso-v2 reconcile graceful-skip + structured audit + cron-worker prod-mutation fence, and a #1838 tool-policy security fix ([CHANGELOG.md](./CHANGELOG.md#01610--2026-06-13)).
- **Recommended upgrade target.** `v0.16.12` (current stable / head of the v0.16 LTS line) supersedes `v0.16.11` / `v0.16.10` / `v0.16.9` / `v0.16.8` / `v0.16.7` / `v0.16.6` / `v0.16.5` / `v0.16.4` / `v0.16.3` / `v0.16.2` / `v0.16.1` / `v0.16.0` / `v0.15.4` / `v0.15.3` / `v0.15.2` / `v0.15.1` / `v0.15.0`, all the `v0.16.11-rc1` / `v0.16.10-rc1..rc3` / `v0.16.0-rc1..rc3` / `v0.15.0-rc1` / `v0.15.0-betaN` / `v0.14.5-betaN` prereleases — treat the latest stable tag as current for production. The `v0.16.11-rc1` candidate is folded into the `v0.16.11` stable LTS tag. Conservative installs that want to stay on this LTS line across future minors can pin with `agb upgrade --channel lts` (see [OPERATIONS.md §"Upgrade"](./OPERATIONS.md#upgrade)).
- v0.7.x → v0.16.12 leap path: **direct `agent-bridge upgrade --apply` to `v0.16.12`** is the current recommended (stable) path — the v0.13.7-v0.13.10 cycle fixed the Bash 5.3.9 heredoc-stdin deadlock chain by extracting leap-path bodies to `lib/upgrade-helpers/`, so the leap is single-atomic from any v0.7.x/v0.8.x/v0.9.x/v0.10.x/v0.11.x/v0.12.x/v0.13.x/v0.14.x/v0.15.x/v0.16.x source. On an iso-v2 host, the automatic fresh-group preflight (KNOWN_ISSUES §28 / #1836, shipped in v0.16.11) now warns when the controller's supplementary group set is stale so the cosmetic `Errno13` reconcile warnings no longer surprise you — running the upgrade from a fresh login shell (or `newgrp` into the agent groups) still avoids them outright. v0.13.10 is the minimum-safe fallback if you're pinned below v0.14.x or troubleshooting an in-flight leap. See [OPERATIONS.md §"Upgrade"](./OPERATIONS.md#upgrade) for recipe + follow-up notes.
...
이 host 를 현재 권장 target (`v0.16.12` 이상) 까지 올린다. 끝나면 done note 한 줄 요약으로 보고.
