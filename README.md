...
**Current version**: v0.15.3 (2026-06-03) — a process-stability + Codex-provisioning batch on top of `v0.15.2`. Headline: the **Codex provisioning upgrade wave (#8945)** brings the bridge's Codex support up to codex-cli 0.135/0.136 — a dedicated Codex `AGENTS.md` Task-Processing-Protocol template + engine-aware urgent nudge so Codex agents reliably run the full claim→process→done queue flow (not just the literal command); five new audit-only Codex hooks (PreCompact/PostCompact/SubagentStart/SubagentStop/PermissionRequest, the last bounded/redacted/throttled); agent-scoped Codex slash commands + `bridge-<role>` permission profiles (installed under the agent's own `.codex/`, never the controller's); and an availability-gated `codex doctor` + watchdog AGENTS.md-contract fix. Alongside it, the **incident #8807 process-explosion fail-safes** — a daemon resource guard that defers spawns under memory/process pressure (#1479), tightened MCP-orphan reaper patterns with PID-reuse revalidation (#1482), and cron-backfill coalescing so a restart no longer floods the inbox (#1481) — plus the engine→binary autostart-gate fix so antigravity (`agy`) is no longer permanently skipped (#1483). The `v0.15.2` headline (headless Claude OAT self-rotation + Darwin-gated macOS `apiKeyHelper`) and `v0.15.1` (`agb setup template-sync`) carry forward. See [CHANGELOG.md](./CHANGELOG.md#0153--2026-06-03) for the full list.
- **Recommended upgrade target.** `v0.15.3` supersedes `v0.15.2` / `v0.15.1` / `v0.15.0` and all the `v0.15.0-rc1` / `v0.15.0-betaN` / `v0.14.5-betaN` prereleases — treat the latest stable tag as current.
- v0.7.x → v0.15.3 leap path: **direct `agent-bridge upgrade --apply` to `v0.15.3`** is the current recommended path — the v0.13.7-v0.13.10 cycle fixed the Bash 5.3.9 heredoc-stdin deadlock chain by extracting leap-path bodies to `lib/upgrade-helpers/`, so the leap is single-atomic from any v0.7.x/v0.8.x/v0.9.x/v0.10.x/v0.11.x/v0.12.x/v0.13.x/v0.14.x source. v0.13.10 is the minimum-safe fallback if you're pinned below v0.14.x or troubleshooting an in-flight leap. See [OPERATIONS.md §"Upgrade"](./OPERATIONS.md#upgrade) for recipe + follow-up notes.
...
`v0.15.2` (stable) 이 현재 권장 target 이다. v0.7.x / v0.8.x / v0.9.x / v0.10.x / v0.11.x / v0.12.x / v0.13.x / v0.14.x 어디서든 단일 `upgrade --apply` 한 번으로 `v0.15.2` 까지 올라간다 (v0.13.7-v0.13.9 가 Bash 5.3.9 heredoc-chain 을 풀어놨기 때문에 중간 hop 불필요).
...
이 host 를 현재 권장 target (`v0.15.2` 이상) 까지 올린다. 끝나면 done note 한 줄 요약으로 보고.
