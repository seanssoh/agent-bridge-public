...
**Current stable / LTS**: v0.16.10 (2026-06-13) — the current head of the **v0.16 Long-Term Support (LTS) line** and its recommended install. The LTS designation is on the `v0.16` *line*; every v0.16.x release is an LTS release — `v0.16.10` supersedes `v0.16.9` as the line head (same line, not a different LTS). Headline: **a large reliability + migration train, hardened through a fleet soak across sean-mac (macOS shared-mode) and cm-prod (the first real Linux iso-v2 production host) — zero data loss, zero drift on both.** Consolidates the rc1→rc3 candidates: `agent restart <dynamic>` now relaunches instead of deregistering + quarantining the transcript and self-restart survives the kill→relaunch (**#1852**/**#1853**); per-agent plugins/settings/skills are preserved across dynamic recreate with a provenance ledger and fail-closed token validation (**#1857**); a gated **layout-v2 four-writer migration** with a fail-closed, daemon-down v1→v2 reconciliation (both-side backups, content-hash conflict policy, foreign-symlink-ancestor fencing, post-apply invariant) stops post-v0.8 installs from forking agent memory (**#1820**); the iso-v2 reconcile graceful-skips agent-private 0600 memory it cannot read and records a structured, auditable `isolation_v2_migration` result, and cron workers refuse to auto-execute interactive-gated / irreversible production-mutation tasks (rc3); reconcile observability hardening (rc2); and a tool-policy security fix that closed a spoofed-admin peer-write bypass caught by CI before merge (**#1838**). Known follow-up (next v0.16.x patch, not a blocker): a controller-shell stale-supplementary-group condition (KNOWN_ISSUES §28 / #1836) can produce cosmetic `Errno13` reconcile warnings when `agb upgrade` is run from a long-lived login shell — a controller group refresh (fresh login / `newgrp`) clears them with zero data impact, and an automatic fresh-group preflight ships next. The `v0.16.9` headline (idle-reaper policy fix + fleet-reliability sweep), the `v0.16.8` headline (session-resume + settings/CI sweep), the `v0.16.7` headline (`trusted-routed` A2A transport), and earlier headlines carry forward. See [CHANGELOG.md](./CHANGELOG.md#01610--2026-06-13) for the full list.

**Previous stable**: v0.16.9 (2026-06-12) — superseded by `v0.16.10` on the same v0.16 LTS line. Headline: **fleet-reliability sweep from a day of live v0.16.8 field reports — idle-reaper policy fix that was silently killing operator-created agents** (**#1795** reap requires `ephemeral` AND `loop!=1`, **#1781** doc-migration memory rollback, **#1783** picker idle false-escalation, **#1786** tasks-db verb + WAL root cause, **#1787**/**#1788** doctor case-insensitive orphan/drift, **#1792** cron scope fence). See [CHANGELOG.md](./CHANGELOG.md#0169--2026-06-12).
- **Recommended upgrade target.** `v0.16.10` (current stable / head of the v0.16 LTS line) supersedes `v0.16.9` / `v0.16.8` / `v0.16.7` / `v0.16.6` / `v0.16.5` / `v0.16.4` / `v0.16.3` / `v0.16.2` / `v0.16.1` / `v0.16.0` / `v0.15.4` / `v0.15.3` / `v0.15.2` / `v0.15.1` / `v0.15.0`, all the `v0.16.10-rc1..rc3` / `v0.16.0-rc1..rc3` / `v0.15.0-rc1` / `v0.15.0-betaN` / `v0.14.5-betaN` prereleases — treat the latest stable tag as current for production. The `v0.16.10-rc1..rc3` candidates are folded into the `v0.16.10` stable LTS tag. Conservative installs that want to stay on this LTS line across future minors can pin with `agb upgrade --channel lts` (see [OPERATIONS.md §"Upgrade"](./OPERATIONS.md#upgrade)).
- v0.7.x → v0.16.10 leap path: **direct `agent-bridge upgrade --apply` to `v0.16.10`** is the current recommended (stable) path — the v0.13.7-v0.13.10 cycle fixed the Bash 5.3.9 heredoc-stdin deadlock chain by extracting leap-path bodies to `lib/upgrade-helpers/`, so the leap is single-atomic from any v0.7.x/v0.8.x/v0.9.x/v0.10.x/v0.11.x/v0.12.x/v0.13.x/v0.14.x/v0.15.x/v0.16.x source. On an iso-v2 host, run the upgrade from a fresh login shell (or `newgrp` into the agent groups) so the controller's supplementary group set is current — this avoids cosmetic `Errno13` reconcile warnings (KNOWN_ISSUES §28 / #1836; an automatic fresh-group preflight ships in the next v0.16.x patch). v0.13.10 is the minimum-safe fallback if you're pinned below v0.14.x or troubleshooting an in-flight leap. See [OPERATIONS.md §"Upgrade"](./OPERATIONS.md#upgrade) for recipe + follow-up notes.
...
`v0.16.10` (stable, LTS) 이 현재 권장 target 이다. v0.7.x / v0.8.x / v0.9.x / v0.10.x / v0.11.x / v0.12.x / v0.13.x / v0.14.x / v0.15.x / v0.16.x 어디서든 단일 `upgrade --apply` 한 번으로 `v0.16.10` 까지 올라간다 (v0.13.7-v0.13.9 가 Bash 5.3.9 heredoc-chain 을 풀어놨기 때문에 중간 hop 불필요). iso-v2 호스트는 fresh 로그인 셸(또는 `newgrp`)에서 업그레이드를 실행해 controller 그룹셋을 최신으로 유지한다 — stale-셸 `Errno13` reconcile 경고(cosmetic, KNOWN_ISSUES §28 / #1836; 자동 fresh-group preflight 는 다음 v0.16.x patch) 회피.
...
이 host 를 현재 권장 target (`v0.16.10` 이상) 까지 올린다. 끝나면 done note 한 줄 요약으로 보고.
