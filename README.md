...
**Current stable / LTS**: v0.16.17 (2026-06-27) — the current head of the **v0.16 Long-Term Support (LTS) line** and its recommended install. The LTS designation is on the `v0.16` *line*; every v0.16.x release is an LTS release — `v0.16.17` supersedes `v0.16.16` as the line head (same line, not a different LTS). Headline: **a security + auth/A2A reliability LTS bundle.** It promotes the **#2137 keychain-free auth-hijack security root-fix** to official (soaked as `v0.16.17-rc1` since 2026-06-26): every broad-scope keychain-free write (`sync` / `backfill-settings` / daemon backfill) now consults `bridge_auth_apikeyhelper_allowed_for` and **excludes the admin / attached-interactive agent** from the managed `apiKeyHelper` (so a live admin can't be silently moved onto API-key billing → an "Invalid API key" break) while still syncing its OAuth credential, `--help` no longer mutates, unknown flags fail closed, and a sanctioned `claude-token keychain-free enable|disable|status` verb does an atomic, fail-closed gate write with a settings-write audit row. It folds in the **#2146 macOS-NAT64 A2A receiver-crash fleet fix** — `serve --detach` now binds + proves the listener synchronously, then **re-execs a fresh interpreter** that adopts the proven listener instead of re-binding in a fork-polluted grandchild (fixing a `getaddrinfo` `SIGSEGV` receiver crash-loop on NAT64 hosts that took the A2A leg down); the untrusted-remote HMAC / source-addr / allowlist / dedupe surface is unchanged. And it lands the **#18849 Part 1 seamless token-rotation machinery** (#2143 / #2144) **gated default-OFF / INERT** — safe to land on the LTS line with **zero live behavior change** (real auth is unaffected while the gate is off); Part 2 (the daemon threshold-gate + cascade) is **Stage 2 and is not in this cut**. Lineage: **v0.16.16 (2026-06-26)** was the prior LTS line head — a **Teams reply silent-non-delivery fix (#2112)** (a production customer agent's replies were accepted by the SDK but never delivered while the reply tool reported success; the path now surfaces the Bot Framework send outcome + an audit row) bundled with the rc1–rc4 reliability/hardening set, and the **LTS endpoint at which mainline development moved to `v0.17`**; **v0.16.15 (2026-06-17)** was a queue-gateway poison-request stability hotfix (a single poison request could silently kill the **whole fleet's** queue gateway — cm-prod F8), and earlier v0.16.x headlines (v0.16.14 systemd `--user` enable-linger daemon-down hotfix, v0.16.13 pane-binding config-trust **#1738** + tmux fleet-down guard **#1932**/**#1936** + canonical hook-path self-heal **#1934**) carry forward. See [CHANGELOG.md](./CHANGELOG.md#01617--2026-06-27-lts) for the full list.

**Previous stable**: v0.16.16 (2026-06-26) — superseded by `v0.16.17` on the same v0.16 LTS line. Headline: **a Teams reply silent-non-delivery fix (#2112)** — a production customer agent reported replies that the SDK accepted but never delivered while the reply tool reported success; the reply path now returns a three-way send outcome (`confirmed` / `unconfirmed` / `failed`) + a sanitized `teams_outbound_reply` audit row (teams plugin 0.1.1 → 0.1.2), bundled with the rc1–rc4 reliability/hardening set. This was the **LTS endpoint of the v0.16.x line** at which mainline development moved to `v0.17` (the `lts` channel stays pinned to the v0.16.x line). See [CHANGELOG.md](./CHANGELOG.md#01616--2026-06-26-lts). Before it, **v0.16.15 (2026-06-17)** was a queue-gateway poison-request stability hotfix (a single poison request could silently kill the whole fleet's queue gateway — cm-prod F8), **v0.16.14 (2026-06-17)** was a systemd `--user` daemon enable-linger hotfix (cm-prod F1, a ~27h silent daemon-down), and **v0.16.13 (2026-06-17)** was a security + reliability hardening bundle (config-mutation pane-binding trust **#1738**, tmux-socket fleet-down guard **#1932**/**#1936**, canonical hook-path + self-heal **#1934**).
- **Recommended upgrade target.** `v0.16.17` (current stable / head of the v0.16 LTS line) supersedes `v0.16.16` / `v0.16.15` / `v0.16.14` / `v0.16.13` / `v0.16.12` / `v0.16.11` / `v0.16.10` / `v0.16.9` / `v0.16.8` / `v0.16.7` / `v0.16.6` / `v0.16.5` / `v0.16.4` / `v0.16.3` / `v0.16.2` / `v0.16.1` / `v0.16.0` / `v0.15.4` / `v0.15.3` / `v0.15.2` / `v0.15.1` / `v0.15.0`, all the `v0.16.17-rc1` / `v0.16.16-rc1..rc4` / `v0.16.13-rc1..rc2` / `v0.16.11-rc1` / `v0.16.10-rc1..rc3` / `v0.16.0-rc1..rc3` / `v0.15.0-rc1` / `v0.15.0-betaN` / `v0.14.5-betaN` prereleases — treat the latest stable tag as current for production. The `v0.16.17-rc1` candidate is folded into the `v0.16.17` stable LTS tag (as are the `v0.16.16-rc1..rc4` candidates into `v0.16.16` and the `v0.16.13-rc1` / `-rc2` candidates into `v0.16.13`). Conservative installs that want to stay on this LTS line across future minors can pin with `agb upgrade --channel lts` (see [OPERATIONS.md §"Upgrade"](./OPERATIONS.md#upgrade)).
- v0.7.x → v0.16.17 leap path: **direct `agent-bridge upgrade --apply` to `v0.16.17`** is the current recommended (stable) path — the v0.13.7-v0.13.10 cycle fixed the Bash 5.3.9 heredoc-stdin deadlock chain by extracting leap-path bodies to `lib/upgrade-helpers/`, so the leap is single-atomic from any v0.7.x/v0.8.x/v0.9.x/v0.10.x/v0.11.x/v0.12.x/v0.13.x/v0.14.x/v0.15.x/v0.16.x source. On an iso-v2 host, the automatic fresh-group preflight (KNOWN_ISSUES §28 / #1836, shipped in v0.16.11) now warns when the controller's supplementary group set is stale so the cosmetic `Errno13` reconcile warnings no longer surprise you — running the upgrade from a fresh login shell (or `newgrp` into the agent groups) still avoids them outright. v0.13.10 is the minimum-safe fallback if you're pinned below v0.14.x or troubleshooting an in-flight leap. See [OPERATIONS.md §"Upgrade"](./OPERATIONS.md#upgrade) for recipe + follow-up notes.
...
이 host 를 현재 권장 target (`v0.16.17` 이상) 까지 올린다. 끝나면 done note 한 줄 요약으로 보고.
