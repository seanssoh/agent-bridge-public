...
**Current stable / LTS**: v0.16.22 (2026-07-03) — the current head of the **v0.16 Long-Term Support (LTS) line** and its recommended install. The LTS designation is on the `v0.16` *line*; every v0.16.x release is an LTS release — `v0.16.22` supersedes `v0.16.21` as the line head (same line, not a different LTS). Headline: **a reliability fast-follow — macOS post-upgrade daemon recovery (#2085 / #2203) + durable idle-nudge re-spool (#2179).** On macOS, `upgrade --restart-daemon` now confirms the daemon actually came back (a running pid **and** a fresh heartbeat, not just a loaded launchd job) and adds a recovery net, while **refusing** to fork a competing plain-bash daemon on an out-of-band launchd split (avoiding KeepAlive thrash) — re-verified end-to-end with **real `launchctl`** in a disposable tart macOS VM. A daemon idle-nudge that drops at the post-grace boundary is now **durably re-spooled** via the existing pending-attention path instead of being silently lost (idempotent — at most one spooled copy, dropped once the task is claimed), closing the ~30-min-latency fleet-wide "idle-agent nudges lost → time-sensitive tasks unclaimed" regression. Both are straight backports of the reviewed `v0.17.0-beta9` mainline fixes (net effect proven line-for-line identical — the lts-line-ancestry gate); the token-updater lease auto-rotation stays mainline and is **deferred to v0.16.23** (tracked in #2255). Lineage: **v0.16.21 (2026-07-01)** backported the daemon **"never-die" hardening wave** (#2209 06-30 fleet-down RCA — six defense-in-depth fixes #2205 / #2207 / #2208 / #2210 / #2211 / #2222 from `v0.17.0-beta8`); **v0.16.20 (2026-06-29)** closed the **#2182 fleet-critical teams plugin-cache fresh-build race**; **v0.16.19 (2026-06-28)** closed the **#2171 / #19460 M4 fleet-down token-rotation regression**; **v0.16.18 (2026-06-27)** made fleet token auto-rotation *enable-able* on the LTS line (the **`global-auth-sync enable|disable|status` verb** #19140 / #2155 + the **#19146 PreToolUse Bash guard** #2157); **v0.16.17 (2026-06-27)** was a security + auth/A2A reliability LTS bundle (the **#2137 keychain-free auth-hijack root-fix**, the **#2146 macOS-NAT64 A2A receiver-crash fleet fix**), and earlier v0.16.x headlines carry forward. See [CHANGELOG.md](./CHANGELOG.md#01622--2026-07-03-lts) for the full list.

**Previous stable**: v0.16.21 (2026-07-01) — superseded by `v0.16.22` on the same v0.16 LTS line. Headline: **the daemon "never-die" hardening wave is backported to the LTS line.** After the 06-30 M4 fleet-down incident (#2209 RCA), the daemon could go silently DOWN and stay down across five failure classes (a hung restart failing open, launchd unloaded+disabled drift with no self-heal, the token lifeline dying with the daemon, an interrupted upgrade leaving version skew + partial state, and a `--no-restart-daemon` reconcile booting the daemon out); this backported the same six-fix defense-in-depth wave shipped on `v0.17.0-beta8` (#2205 / #2207 / #2208 / #2210 / #2211 / #2222) — a **fleet-host-down regression** class that qualifies for LTS backport. **Scope: never-die only** (the v0.17 auto-rotate roadmap, dynamic→static converter, and §10 lighting stay mainline). All six fixes re-verified green on the v0.16 line; codex `implement-ok`. See [CHANGELOG.md](./CHANGELOG.md#01621--2026-07-01-lts). Before it, **v0.16.20 (2026-06-29)** closed the **#2182 fleet-critical teams plugin-cache fresh-build race**, and **v0.16.19 (2026-06-28)** closed the **#2171 / #19460 M4 fleet-down token-rotation regression**.
- **Recommended upgrade target.** `v0.16.22` (current stable / head of the v0.16 LTS line) supersedes `v0.16.21` / `v0.16.20` / `v0.16.19` / `v0.16.18` / `v0.16.17` / `v0.16.16` / `v0.16.15` / `v0.16.14` / `v0.16.13` / `v0.16.12` / `v0.16.11` / `v0.16.10` / `v0.16.9` / `v0.16.8` / `v0.16.7` / `v0.16.6` / `v0.16.5` / `v0.16.4` / `v0.16.3` / `v0.16.2` / `v0.16.1` / `v0.16.0` / `v0.15.4` / `v0.15.3` / `v0.15.2` / `v0.15.1` / `v0.15.0`, all the `v0.16.17-rc1` / `v0.16.16-rc1..rc4` / `v0.16.13-rc1..rc2` / `v0.16.11-rc1` / `v0.16.10-rc1..rc3` / `v0.16.0-rc1..rc3` / `v0.15.0-rc1` / `v0.15.0-betaN` / `v0.14.5-betaN` prereleases — treat the latest stable tag as current for production. The `v0.16.17-rc1` candidate is folded into the `v0.16.17` stable LTS tag (as are the `v0.16.16-rc1..rc4` candidates into `v0.16.16` and the `v0.16.13-rc1` / `-rc2` candidates into `v0.16.13`). Conservative installs that want to stay on this LTS line across future minors can pin with `agb upgrade --channel lts` (see [OPERATIONS.md §"Upgrade"](./OPERATIONS.md#upgrade)).
  - **Mainline (beta, not for production):** active feature development is on the **`v0.17.0-beta` line — currently `v0.17.0-beta9`** (GitHub **Pre-release**; the `Latest` marker stays on the LTS tag). beta9's marquee item is **token-updater lease auto-rotation, shipped default-OFF**; the v0.17 roadmap (auto-rotate, dynamic→static converter, §10 lighting) rides mainline and is **not** LTS-backported. Production installs should stay on the `lts` channel above — only opt into a beta to trial an unreleased feature.
- v0.7.x → v0.16.22 leap path: **direct `agent-bridge upgrade --apply` to `v0.16.22`** is the current recommended (stable) path — the v0.13.7-v0.13.10 cycle fixed the Bash 5.3.9 heredoc-stdin deadlock chain by extracting leap-path bodies to `lib/upgrade-helpers/`, so the leap is single-atomic from any v0.7.x/v0.8.x/v0.9.x/v0.10.x/v0.11.x/v0.12.x/v0.13.x/v0.14.x/v0.15.x/v0.16.x source. On an iso-v2 host, the automatic fresh-group preflight (KNOWN_ISSUES §28 / #1836, shipped in v0.16.11) now warns when the controller's supplementary group set is stale so the cosmetic `Errno13` reconcile warnings no longer surprise you — running the upgrade from a fresh login shell (or `newgrp` into the agent groups) still avoids them outright. v0.13.10 is the minimum-safe fallback if you're pinned below v0.14.x or troubleshooting an in-flight leap. See [OPERATIONS.md §"Upgrade"](./OPERATIONS.md#upgrade) for recipe + follow-up notes.
...
이 host 를 현재 권장 target (`v0.16.22` 이상) 까지 올린다. 끝나면 done note 한 줄 요약으로 보고.
