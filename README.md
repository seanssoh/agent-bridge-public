...
**Current stable / LTS**: v0.16.11 (2026-06-15) — the current head of the **v0.16 Long-Term Support (LTS) line** and its recommended install. The LTS designation is on the `v0.16` *line*; every v0.16.x release is an LTS release — `v0.16.11` supersedes `v0.16.10` as the line head (same line, not a different LTS). Headline: **dynamic agents become vanilla-equivalent — a dynamic Claude or Codex agent now runs like vanilla Claude Code / Codex CLI against the operator's global config (settings, model, session history, installed plugins, MCP, credentials) with native resume (`claude -c` / `codex resume --last`), and the bridge layers only its comms/queue hooks via project-local settings — superseding the private-config-dir isolation that was destroying operator sessions** (**#1900**/**#1901**). The session-detect / quarantine machinery is no-op for dynamic agents (safety via "no destructive machinery", not isolation); static/admin agents are unchanged. Also: the operator-session-hijack fix that scoped a fresh agent's resume/quarantine to its own config dir (**#1889**); **iso-v2 create-path completeness** — later-created iso agents' `memory/` is normalized iso-owned `2770` and `agent-meta.env` is always created/verified (**#1895**); doc-backfill / watchdog engine detection is roster-first fail-closed so an absent `agent-meta.env` never mis-materializes a codex entrypoint on a claude agent (**#1896**); a common iso-boundary graceful-skip across the controller scanners (**#1878**); plus watchdog/cron/channel fixes (**#1872**/**#1880**/**#1881**/**#1888**) and a 3-way-sharded unit/static CI smoke battery (**#1898**). Soak: a local OrbStack real-Linux (Ubuntu noble, iso-v2) soak (clean `0.16.4`→rc1 upgrade, the six release-critical smokes passing on real Linux, dynamic-vanilla launch builder confirmed live) + the full sharded CI battery green on Linux; downstream cm-prod runs the contracted iso-v2 production acceptance post-upgrade via the `lts` channel. The `v0.16.10` headline (dynamic-agent restart + layout-v2 migration train), the `v0.16.9` headline (idle-reaper policy fix), and earlier headlines carry forward. See [CHANGELOG.md](./CHANGELOG.md#01611--2026-06-15-lts) for the full list.

**Previous stable**: v0.16.10 (2026-06-13) — superseded by `v0.16.11` on the same v0.16 LTS line. Headline: **a large reliability + migration train, hardened through a fleet soak across sean-mac (macOS shared-mode) and cm-prod (first real Linux iso-v2 production host)** — `agent restart <dynamic>` relaunch + self-restart survival (**#1852**/**#1853**), provisioning preserve across recreate (**#1857**), a gated layout-v2 four-writer migration with fail-closed v1→v2 reconciliation (**#1820**), iso-v2 reconcile graceful-skip + structured audit + cron-worker prod-mutation fence, and a #1838 tool-policy security fix. See [CHANGELOG.md](./CHANGELOG.md#01610--2026-06-13).
- **Recommended upgrade target.** `v0.16.11` (current stable / head of the v0.16 LTS line) supersedes `v0.16.10` / `v0.16.9` / `v0.16.8` / `v0.16.7` / `v0.16.6` / `v0.16.5` / `v0.16.4` / `v0.16.3` / `v0.16.2` / `v0.16.1` / `v0.16.0` / `v0.15.4` / `v0.15.3` / `v0.15.2` / `v0.15.1` / `v0.15.0`, all the `v0.16.11-rc1` / `v0.16.10-rc1..rc3` / `v0.16.0-rc1..rc3` / `v0.15.0-rc1` / `v0.15.0-betaN` / `v0.14.5-betaN` prereleases — treat the latest stable tag as current for production. The `v0.16.11-rc1` candidate is folded into the `v0.16.11` stable LTS tag. Conservative installs that want to stay on this LTS line across future minors can pin with `agb upgrade --channel lts` (see [OPERATIONS.md §"Upgrade"](./OPERATIONS.md#upgrade)).
