...
**Latest pre-release**: v0.16.0-beta1 (2026-06-04) — the first cut of **A2A Rooms (beta)**, a room/leader(방장)/join-on-approval membership model that unifies internal-team and cross-bridge agent messaging under one boundary. A leader runs `agb room create`, hands out a one-time invite link (only the token's sha256 hash is stored), and approves each `agb room join` — and the same rooms back an opt-in **internal-queue ACL** (`agb room acl enforce`, default **off** = zero behavior change) so inter-agent queue creates can be restricted to shared-room members, OS-enforced under linux-user isolation (iso v2). beta1 is **single-node** (one install's control plane + its own roster); multi-node transport (Tailscale / Cloudflare Zero Trust), node bootstrap, and cross-node roster relay are **beta2**. Bundled with it: the **#1497 HOME/path SSOT** (`operator_home()` consolidating four home resolvers + a `BRIDGE_AGENT_HOME_RESOLVED` channel for bash↔Python parity) and the **#1506 isolate normalize** (a `create`→`isolate` now normalizes a pre-existing profile tree to the iso v2 contract). See [docs/a2a-rooms.md](./docs/a2a-rooms.md) for operator usage and [docs/design/a2a-rooms-design.md](./docs/design/a2a-rooms-design.md) for the design. This is a **beta** for live testing — `v0.15.4` remains the recommended stable.
...
- **Recommended upgrade target.** `v0.15.4` (current stable) supersedes `v0.15.3` / `v0.15.2` / `v0.15.1` / `v0.15.0` and all the `v0.15.0-rc1` / `v0.15.0-betaN` / `v0.14.5-betaN` prereleases — treat the latest stable tag as current for production. `v0.16.0-beta1` is the latest **pre-release** (A2A Rooms beta, single-node) — pull it only to live-test the rooms feature; it does not demote the `v0.15.4` stable.
- v0.7.x → v0.15.4 leap path: **direct `agent-bridge upgrade --apply` to `v0.15.4`** is the current recommended (stable) path; pull `v0.16.0-beta1` on top only to live-test A2A Rooms — the v0.13.7-v0.13.10 cycle fixed the Bash 5.3.9 heredoc-stdin deadlock chain by extracting leap-path bodies to `lib/upgrade-helpers/`, so the leap is single-atomic from any v0.7.x/v0.8.x/v0.9.x/v0.10.x/v0.11.x/v0.12.x/v0.13.x/v0.14.x source. v0.13.10 is the minimum-safe fallback if you're pinned below v0.14.x or troubleshooting an in-flight leap. See [OPERATIONS.md §"Upgrade"](./OPERATIONS.md#upgrade) for recipe + follow-up notes.
