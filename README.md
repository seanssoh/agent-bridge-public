...
**Current stable**: v0.14.1 (2026-05-16) — completeness pass after v0.14.0 E2E (8 fixes total).
- Recommended upgrade target. v0.14.1 closes the v0.14.0 fresh-install regressions discovered during a Ubuntu 24.04 E2E test (`_template/` evidence false-positive, codex-absent admin-pair crash-loop, fresh-install onboarding nudge, Linux `ensure_matrix_path failed` spam on hosts without `ab-shared` group, engine-CLI-missing retry burst, BSD `mktemp` literal-path bug) plus audit-A backlog (heartbeat tempfile leak, BSD-portable mktemp ratchet).
- v0.7.x → v0.14.x leap path: **direct `agent-bridge upgrade --apply` to v0.14.1** is the current recommended path — the v0.13.7-v0.13.10 cycle fixed the Bash 5.3.9 heredoc-stdin deadlock chain by extracting leap-path bodies to `lib/upgrade-helpers/`, so the leap is single-atomic from any v0.7.x/v0.8.x/v0.9.x/v0.10.x/v0.11.x/v0.12.x/v0.13.x source. v0.13.10 is the minimum-safe fallback if you're pinned below v0.14.x or troubleshooting an in-flight leap. See [OPERATIONS.md §"Upgrade"](./OPERATIONS.md#upgrade) for recipe + follow-up notes.
...
- Stabilization roadmap: [`docs/stabilization-plan-2026-05-15.md`](./docs/stabilization-plan-2026-05-15.md). v0.14.0/v0.14.1 cleared S0-S3 + S5 Track A1/A2 + 8 E2E/backlog fixes; S4 (Bash 3.2 cleanup), S5 B/C/D, S6+ are deferred to next cycle.
