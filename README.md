...
**Current stable / LTS**: v0.16.20 (2026-06-29) — the current head of the **v0.16 Long-Term Support (LTS) line** and its recommended install. The LTS designation is on the `v0.16` *line*; every v0.16.x release is an LTS release — `v0.16.20` supersedes `v0.16.19` as the line head (same line, not a different LTS). Headline: **the #2182 fleet-critical teams plugin-cache fresh-build race is closed on the LTS line.** On a `v0.16.19` upgrade that bumped a channel plugin's version (teams `0.1.1→0.1.2`), an agent that did **not** already have that cached version fresh-built its per-agent plugin cache on launch — and a race partially copied the `@azure` dependency, so the required-contract check found `node_modules/@azure/core-client/package.json` missing, aborted launch with `criticality=channel-required`, and the verify-retry loop **reused the partial cache** → the agent was left `stopped` (a **permanent wedge**). This straight-cherry-picks the `main` fix (PR #2184): `bridge-dev-plugin-cache.py` now assembles a fresh cache **atomically** (pid-private temp → contract verify → `os.rename` publish, so the live path only ever becomes a *complete* cache) and **retires + rebuilds an incomplete existing cache** (winner-guarded — never deleting a complete cache another agent published) instead of reusing it. The required-contract is deliberately **not** narrowed. Re-verified on the v0.16 base: smoke `2182-plugin-cache-atomic-build` T1-T6 green, byte-identical to the `main` fix (shared patch-id). Lineage: **v0.16.19 (2026-06-28)** closed the **#2171 / #19460 M4 fleet-down token-rotation regression** (the `cmd_rotate --preflight` live candidate probe + daemon enablement + measured-usage prefilter + macOS keychain-shadow self-heal; PR-B2 Part 2 `active_dead_recover` tracked fast-follow); **v0.16.18 (2026-06-27)** made fleet token auto-rotation *enable-able* on the LTS line (the **`global-auth-sync enable|disable|status` verb** #19140 / #2155 + **#18849 Part 2** + the **#19146 PreToolUse Bash guard** #2157); **v0.16.17 (2026-06-27)** was a security + auth/A2A reliability LTS bundle (the **#2137 keychain-free auth-hijack root-fix**, the **#2146 macOS-NAT64 A2A receiver-crash fleet fix**, **#18849 Part 1** INERT); **v0.16.16 (2026-06-26)** was a Teams reply silent-non-delivery fix (**#2112**) + the **LTS endpoint at which mainline development moved to `v0.17`**, and earlier v0.16.x headlines carry forward. See [CHANGELOG.md](./CHANGELOG.md#01620--2026-06-29-lts) for the full list.

**Previous stable**: v0.16.19 (2026-06-28) — superseded by `v0.16.20` on the same v0.16 LTS line. Headline: **the #2171 / #19460 M4 fleet-down token-rotation regression is closed on the LTS line.** Reactive Claude-token rotation re-picked a stale / limited candidate when the usage probe degraded (403 / edge-blocked) and the whole fleet went dark; this backported the **core** of the hybrid fix (the four PRs shipped on `v0.17.0-beta7`): `cmd_rotate --preflight` live candidate probe (under a daemon **global preflight budget**), a measured-usage prefilter, and a macOS keychain-shadow `reconcile-keychain` self-heal, plus the `classify_token_kind` classifier backport the cherry-picks needed. The remaining **PR-B2 Part 2** (daemon `active_dead_recover`) is a tracked fast-follow; `--preflight` OFF = byte-identical legacy. See [CHANGELOG.md](./CHANGELOG.md#01619--2026-06-28-lts). Before it, **v0.16.18 (2026-06-27)** made fleet token auto-rotation *enable-able* on the LTS line (the **`global-auth-sync` verb** #19140 / #2155 + **#18849 Part 2** + the **#19146 PreToolUse Bash guard** #2157), **v0.16.17 (2026-06-27)** was a security + auth/A2A reliability LTS bundle (the **#2137 keychain-free auth-hijack root-fix**, the **#2146 macOS-NAT64 A2A receiver-crash fleet fix**, **#18849 Part 1** INERT), and **v0.16.16 (2026-06-26)** was a Teams reply silent-non-delivery fix (**#2112**) + the **LTS endpoint at which mainline development moved to `v0.17`**.
- **Recommended upgrade target.** `v0.16.20` (current stable / head of the v0.16 LTS line) supersedes `v0.16.19` / `v0.16.18` / `v0.16.17` / `v0.16.16` / `v0.16.15` / `v0.16.14` / `v0.16.13` / `v0.16.12` / `v0.16.11` / `v0.16.10` / `v0.16.9` / `v0.16.8` / `v0.16.7` / `v0.16.6` / `v0.16.5` / `v0.16.4` / `v0.16.3` / `v0.16.2` / `v0.16.1` / `v0.16.0` / `v0.15.4` / `v0.15.3` / `v0.15.2` / `v0.15.1` / `v0.15.0`, all the `v0.16.17-rc1` / `v0.16.16-rc1..rc4` / `v0.16.13-rc1..rc2` / `v0.16.11-rc1` / `v0.16.10-rc1..rc3` / `v0.16.0-rc1..rc3` / `v0.15.0-rc1` / `v0.15.0-betaN` / `v0.14.5-betaN` prereleases — treat the latest stable tag as current for production. The `v0.16.17-rc1` candidate is folded into the `v0.16.17` stable LTS tag (as are the `v0.16.16-rc1..rc4` candidates into `v0.16.16` and the `v0.16.13-rc1` / `-rc2` candidates into `v0.16.13`). Conservative installs that want to stay on this LTS line across future minors can pin with `agb upgrade --channel lts` (see [OPERATIONS.md §"Upgrade"](./OPERATIONS.md#upgrade)).
- v0.7.x → v0.16.20 leap path: **direct `agent-bridge upgrade --apply` to `v0.16.20`** is the current recommended (stable) path — the v0.13.7-v0.13.10 cycle fixed the Bash 5.3.9 heredoc-stdin deadlock chain by extracting leap-path bodies to `lib/upgrade-helpers/`, so the leap is single-atomic from any v0.7.x/v0.8.x/v0.9.x/v0.10.x/v0.11.x/v0.12.x/v0.13.x/v0.14.x/v0.15.x/v0.16.x source. On an iso-v2 host, the automatic fresh-group preflight (KNOWN_ISSUES §28 / #1836, shipped in v0.16.11) now warns when the controller's supplementary group set is stale so the cosmetic `Errno13` reconcile warnings no longer surprise you — running the upgrade from a fresh login shell (or `newgrp` into the agent groups) still avoids them outright. v0.13.10 is the minimum-safe fallback if you're pinned below v0.14.x or troubleshooting an in-flight leap. See [OPERATIONS.md §"Upgrade"](./OPERATIONS.md#upgrade) for recipe + follow-up notes.
...
이 host 를 현재 권장 target (`v0.16.20` 이상) 까지 올린다. 끝나면 done note 한 줄 요약으로 보고.
