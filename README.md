...
**Current version**: v0.15.2 (2026-06-01) — a stabilization + headless-hardening batch on top of `v0.15.1`. Headline: **headless Claude OAT self-rotation** — on a cron host with no `claude-hud`, the daemon now sources Anthropic utilization directly (proactive native `api/oauth/usage` probe, #1443) and rotates the token reactively on a 429 (#1441), so long-running headless agents no longer stall on an expired token; plus a Darwin-gated macOS Claude **`apiKeyHelper`** auth path (Keychain-free) with a deep inherited-env credential-leak hardening pass (#1444/#1452) — both documented in [OPERATIONS.md](./OPERATIONS.md). Also: a whole-fleet crash-loop fix on per-agent-HOME installs (#1439), the long-standing daemon "ACTION REQUIRED" nudge double-fire fix (#1199/#1451), CSI-u Enter submit for Claude Code 2.1.158+ (#1450, external contributor Jong Ko), v2 per-agent-workdir protected globs (#1448), the codex `features.hooks` launch-flag migration (#1446), and a fail-safe daemon spool rederive (#1425). The `v0.15.1` headline (`agb setup template-sync` — seed new agents from a reference agent) and `v0.15.0` (A2A cross-bridge self-heal, handoffd receiver supervision, the `agb a2a setup` wizard, the iso-v2 hardening campaign) carry forward. See [CHANGELOG.md](./CHANGELOG.md#0152--2026-06-01) for the full list.
- **Recommended upgrade target.** `v0.15.2` supersedes `v0.15.1` / `v0.15.0` and all the `v0.15.0-rc1` / `v0.15.0-betaN` / `v0.14.5-betaN` prereleases — treat the latest stable tag as current.
- v0.7.x → v0.15.2 leap path: **direct `agent-bridge upgrade --apply` to `v0.15.2`** is the current recommended path — the v0.13.7-v0.13.10 cycle fixed the Bash 5.3.9 heredoc-stdin deadlock chain by extracting leap-path bodies to `lib/upgrade-helpers/`, so the leap is single-atomic from any v0.7.x/v0.8.x/v0.9.x/v0.10.x/v0.11.x/v0.12.x/v0.13.x/v0.14.x source. v0.13.10 is the minimum-safe fallback if you're pinned below v0.14.x or troubleshooting an in-flight leap. See [OPERATIONS.md §"Upgrade"](./OPERATIONS.md#upgrade) for recipe + follow-up notes.
