...
**Latest pre-release**: v0.16.0-rc3 (2026-06-06) — the third `0.16.0` release candidate. Headline: **A2A inbound backpressure + security-receiver stabilization** — a live incident where "A2A that used to work stopped working" root-caused to a backpressure **all-time-count bug**: the open-task count counted the entire dedupe history instead of currently-OPEN tasks, so the receiver `503`-rejected every peer past the cumulative cap **forever** (and the original fix-attempt ran `ATTACH`/`DETACH` inside an open `BEGIN IMMEDIATE` that fails deterministically on SQLite 3.51 — a remote-triggered DoS a CI smoke missed because it only exercised the autocommit path). It was fixed alongside an **8-finding adversarial audit (B1-B8)** of the A2A receiver: large-body reaper corruption on a symlinked home, crash-consistency (no task duplication on a mid-deliver crash), a stale-pending backpressure+room-gate bypass, a slow-loris pre-auth deadline DoS, and a **two-cap Retry-After** model that honors a peer's `Retry-After` as a hard delivery floor (bounded by a separate `delivery_max_retry_after_seconds`, default 600s) while keeping the #1575-B anti-DoS exponential-backoff ceiling (`delivery_backoff_ceiling_seconds`, default 120s) — with a non-finite guard after a `Retry-After: inf` remote-DoS was caught in adversarial review. The cut also lands six accumulated fixes / hotpatch-upstreams: **#1575-B** sender backoff ceiling cap, **#1588** smoke-test LIVE-state isolation, **#1466** `bridge-core.sh` de-heredoc (footgun #11), **#848** dispatcher roster-skip for queue verbs, **#1568** routine idle-nudge inject-busy gate, **#1574** PreToolUse hook hardening. Builds on the rc1/rc2 **daemon supervision redesign (#1563)**; the fail-closed receiver bind/HMAC/dedupe boundary is unchanged and there is no `agb upgrade` schema/contract change (wire protocol `a2a-enqueue-v1` unchanged). This is a **release candidate** for live testing — `v0.15.4` remains the recommended stable.
...
- **Recommended upgrade target.** `v0.15.4` (current stable) supersedes `v0.15.3` / `v0.15.2` / `v0.15.1` / `v0.15.0` and all the `v0.15.0-rc1` / `v0.15.0-betaN` / `v0.14.5-betaN` prereleases — treat the latest stable tag as current for production. `v0.16.0-rc3` is the latest **pre-release** (the `0.16.0` release candidate — A2A inbound backpressure + security-receiver stabilization on top of the #1563 daemon supervision redesign) — pull it only to live-test the rc features; it does not demote the `v0.15.4` stable.
- v0.7.x → v0.15.4 leap path: **direct `agent-bridge upgrade --apply` to `v0.15.4`** is the current recommended (stable) path; pull `v0.16.0-rc3` on top only to live-test the `0.16.0` features — the v0.13.7-v0.13.10 cycle fixed the Bash 5.3.9 heredoc-stdin deadlock chain by extracting leap-path bodies to `lib/upgrade-helpers/`, so the leap is single-atomic from any v0.7.x/v0.8.x/v0.9.x/v0.10.x/v0.11.x/v0.12.x/v0.13.x/v0.14.x source. v0.13.10 is the minimum-safe fallback if you're pinned below v0.14.x or troubleshooting an in-flight leap. See [OPERATIONS.md §"Upgrade"](./OPERATIONS.md#upgrade) for recipe + follow-up notes.
