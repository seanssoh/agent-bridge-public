...
**Current stable / LTS**: v0.16.15 (2026-06-17) — the current head of the **v0.16 Long-Term Support (LTS) line** and its recommended install. The LTS designation is on the `v0.16` *line*; every v0.16.x release is an LTS release — `v0.16.15` supersedes `v0.16.14` as the line head (same line, not a different LTS). Headline: **a queue-gateway poison-request stability hotfix** — a single poison request could silently kill the **whole fleet's** queue gateway: `bridge-queue-gateway.py`'s `serve-once` ran each request's handler with no per-request guard, and `run_queue` did `subprocess.run(cwd=<client-recorded cwd>)`; an iso agent that ran `agb` from inside a `0700` iso-owned dir (e.g. `.teams/attachments/<id>`) recorded a cwd the controller-run gateway could not `chdir` into → `PermissionError` aborted the entire batch, and the request — promoted to `<id>.working.json` — re-crashed every tick, so every other agent's queued request timed out forever while the daemon reported healthy (cm-prod v0.16.13 RCA finding F8; on a production host a single attachment-cwd request killed the fleet queue ~26h). The fix falls back to a controller-safe cwd and dead-letters a poison request out of the drain instead of crashing the batch. With F1 (the systemd-`--user` enable-linger 27h daemon-down, shipped in v0.16.14), this **completes the F1/F8 daemon-stability P1 pair** from the RCA — cut directly to official given the live impact on installs already on v0.16.13/14. The `v0.16.14` headline (a systemd `--user` daemon enable-linger hotfix — the cm-prod F1 27h daemon-down) and the `v0.16.13` headline (a security + reliability hardening bundle) — (1) **config-mutation trust derives from an unspoofable pane binding, not env (#1738)** — `agb config set` / `set-env` no longer trust forgeable `BRIDGE_AGENT_ID` / `--from` / `BRIDGE_CALLER_SOURCE` env; trust now derives from a controller-published per-agent pane-pid binding matched against process ancestry (absolute `/bin/ps` + `/proc`), hardened with an ownership store-writability fail-close, a clean-allowlist tmux-liveness probe (no `DYLD_*` / `LD_PRELOAD` / `TMUX_TMPDIR` injection) + a kernel-boundary pane-owner-UID check, a fail-safe daemon prune (a transient tmux outage prunes nothing), and a hook gate that denies a config mutation hidden behind `eval` / `bash -c` / `env` / `command` / `exec`; (2) **a smoke run can no longer down the live agent fleet (#1932/#1936)** — every bridge agent shares the host default tmux socket (a single point of failure) and the destructive smoke harness now severs an inherited `$TMUX` + forces a private socket (fail-closed if it would resolve to the shared one), after a smoke run took every agent down twice on a live host; (3) **hook command paths never resolve under a transient `/tmp` (#1934)** — a settings render with a temp `BRIDGE_HOME` wrote `/tmp`-rooted hook command paths into LIVE agent settings + the `_template` scaffold + Codex `.codex/hooks.json`, so a `/tmp` reap bricked a production farm (`UserPromptSubmit` fail-closed-deaf + `PreToolUse *` tool-deadlock); paths now resolve to a stable canonical dir for both engines, a missing hook **file** self-heals (session-start + daemon reconcile re-render with a per-agent storm-guard) instead of bricking the agent, and the `isolated-plugin-grants/` dir is re-asserted to a durable `controller:ab-shared 0710`. Promoted from `v0.16.13-rc2` after a three-way re-test (macbook isolated + a real OrbStack Linux iso-v2 `agb upgrade` + cm-prod production-host isolated verification). The `v0.16.12` headline (**the upgrade respawn-race fix** — the #1820 layout-v2 reconcile quiesce is now init-system-aware on both **systemd (#1905)** and **launchd (#655)**), the `v0.16.11` headline (dynamic agents become vanilla-equivalent — **#1900**/**#1901**), the `v0.16.10` headline (dynamic-agent restart + layout-v2 migration train), and earlier headlines carry forward. The remaining cm-prod RCA findings (F3 #1943 cron model-gate, F5 #1947 iso index-rebuild graceful-skip, F7 #1945 iso restart settings perm, F4 #1946 headless config path, F6 #1944 nudge backoff) are **stability-unrelated** and deferred to the next version, which will go through a new **RC-soak release rule** (RC tag → a few days' soak on the macbook + cm-prod → promote to official). See [CHANGELOG.md](./CHANGELOG.md#01615--2026-06-17-lts) for the full list.

**Previous stable**: v0.16.14 (2026-06-17) — superseded by `v0.16.15` on the same v0.16 LTS line. Headline: **a systemd `--user` daemon enable-linger hotfix** — a sudo-self systemd-user install bound the always-on reconcile daemon + its liveness-backstop timer to the invoking login session without lingering, so a single session churn stopped both → a ~27h silent production outage + frozen Claude token rotation (cm-prod F1); the installer now runs `loginctl enable-linger` on the `--apply` path so `agb upgrade` reaches existing installs. See [CHANGELOG.md](./CHANGELOG.md#01614--2026-06-17-lts). Before it, **v0.16.13 (2026-06-17)** was a security + reliability hardening bundle (config-mutation pane-binding trust **#1738**, tmux-socket fleet-down guard **#1932**/**#1936**, canonical hook-path + self-heal **#1934**), and **v0.16.12 (2026-06-15)** was the upgrade respawn-race fix (init-system-aware quiesce on systemd **#1905** + launchd **#655**).
- **Recommended upgrade target.** `v0.16.15` (current stable / head of the v0.16 LTS line) supersedes `v0.16.14` / `v0.16.13` / `v0.16.12` / `v0.16.11` / `v0.16.10` / `v0.16.9` / `v0.16.8` / `v0.16.7` / `v0.16.6` / `v0.16.5` / `v0.16.4` / `v0.16.3` / `v0.16.2` / `v0.16.1` / `v0.16.0` / `v0.15.4` / `v0.15.3` / `v0.15.2` / `v0.15.1` / `v0.15.0`, all the `v0.16.13-rc1..rc2` / `v0.16.11-rc1` / `v0.16.10-rc1..rc3` / `v0.16.0-rc1..rc3` / `v0.15.0-rc1` / `v0.15.0-betaN` / `v0.14.5-betaN` prereleases — treat the latest stable tag as current for production. The `v0.16.13-rc1` / `-rc2` candidates are folded into the `v0.16.13` stable LTS tag. Conservative installs that want to stay on this LTS line across future minors can pin with `agb upgrade --channel lts` (see [OPERATIONS.md §"Upgrade"](./OPERATIONS.md#upgrade)).
- v0.7.x → v0.16.15 leap path: **direct `agent-bridge upgrade --apply` to `v0.16.15`** is the current recommended (stable) path — the v0.13.7-v0.13.10 cycle fixed the Bash 5.3.9 heredoc-stdin deadlock chain by extracting leap-path bodies to `lib/upgrade-helpers/`, so the leap is single-atomic from any v0.7.x/v0.8.x/v0.9.x/v0.10.x/v0.11.x/v0.12.x/v0.13.x/v0.14.x/v0.15.x/v0.16.x source. On an iso-v2 host, the automatic fresh-group preflight (KNOWN_ISSUES §28 / #1836, shipped in v0.16.11) now warns when the controller's supplementary group set is stale so the cosmetic `Errno13` reconcile warnings no longer surprise you — running the upgrade from a fresh login shell (or `newgrp` into the agent groups) still avoids them outright. v0.13.10 is the minimum-safe fallback if you're pinned below v0.14.x or troubleshooting an in-flight leap. See [OPERATIONS.md §"Upgrade"](./OPERATIONS.md#upgrade) for recipe + follow-up notes.
...
`v0.16.15` (stable, LTS) 이 현재 권장 target 이다. v0.7.x / v0.8.x / v0.9.x / v0.10.x / v0.11.x / v0.12.x / v0.13.x / v0.14.x / v0.15.x / v0.16.x 어디서든 단일 `upgrade --apply` 한 번으로 `v0.16.15` 까지 올라간다 (v0.13.7-v0.13.9 가 Bash 5.3.9 heredoc-chain 을 풀어놨기 때문에 중간 hop 불필요). iso-v2 호스트는 자동 fresh-group preflight (KNOWN_ISSUES §28 / #1836, v0.16.11 에서 출하됨) 가 controller 그룹셋이 stale 하면 경고해 cosmetic `Errno13` reconcile 경고가 더 이상 놀라게 하지 않는다 — fresh 로그인 셸(또는 `newgrp`)에서 업그레이드를 실행하면 경고 자체를 회피한다.
...
이 host 를 현재 권장 target (`v0.16.15` 이상) 까지 올린다. 끝나면 done note 한 줄 요약으로 보고.
