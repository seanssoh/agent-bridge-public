...
**Current stable / LTS**: v0.16.3 (2026-06-08) — the current head of the **v0.16 Long-Term Support (LTS) line** and its recommended install. The LTS designation is on the `v0.16` *line*; `v0.16.3` is its latest patch, superseding `v0.16.2` as the line head (same line, not a different LTS). Headline: **`lts` upgrade channel + a 9-lane stabilization wave over `0.16.2`, gated by per-PR + full-branch integration codex review and orbstack Linux iso v2 VM-verify**. Marquee feature: **`agb upgrade --channel lts` pins an install to the v0.16 LTS line** — it resolves the highest non-prerelease tag *within* the LTS series (root `LTS_SERIES` pointer, currently `0.16`) so a higher minor shipping elsewhere doesn't auto-jump the install off the LTS line; the choice is **sticky** (recorded in `state/upgrade/channel`, so a later bare `upgrade --apply` stays on the LTS line), `--version`/`--ref` are one-shot and `AGENT_BRIDGE_UPGRADE_CHANNEL` is transient (neither rewrites the sticky), and the resolver **fails closed** (clear remediation, non-zero rc) on a missing/malformed `LTS_SERIES` or an unverifiable sticky write rather than silently slipping back to `stable` (**#1687**). Three marquee fixes: **`upgrade --apply` self-restart no longer reads as a failure** — an upgrade that restarts its own session was observed exiting 137 (parent SIGKILL during restart), which a wrapper read as failure; an upgrade-complete marker is now written before the restart, promoted post-restart, and surfaced in `--json`, so the marker (not the exit code) is the source of truth (**#1662**); **settings rerender preserves an operator-added `statusLine` (HUD)** — the every-upgrade isolated-home rerender carried forward only an allowlist that omitted `statusLine`, silently dropping a claude-hud HUD; `statusLine` is now in `PRESERVED_USER_KEYS` and survives every rerender (**#1689**); and **the A2A receiver self-heals after an upgrade left it on stale code** — a pre-`v0.16.1` → `v0.16.x` first upgrade never restarted the tailnet receiver (so it silently 429'd inbound A2A); a daemon-tick detector (`receiver-boot.json` vs `last-upgrade.json`) now performs a guarded, preflight-gated one-shot auto-restart, fully fail-safe on a malformed marker (**#1685**). The rest of the wave: daemon-control lock held through the full critical section via a shell-global token + `flock -n` (**#1667**); cron-runner derives `summary_short` from the first non-empty `summary` line instead of discarding a whole child payload (data-loss fix, **#1677**); `rematerialize-agent-identity --dry-run` no longer blanks the agent in its JSON (**#1670**); `link-shared-settings` is idempotent on an iso-owned existing symlink (**#1672**); Teams `server.ts` emits a clear `EADDRINUSE` diagnostic (**#1671**); and a deterministic, weekday-independent `1659` cron-smoke fixture (**#1683**). Same `agb upgrade` path; wire protocol `a2a-enqueue-v1` unchanged. The `v0.16.2` headline (four robustness/perf fixes surfaced by the fleet rollout — `agb status` cron cadence-health #1659, broken-pipe exit-code #1660, upgrade/rollback singleton lock #1661, iso plugin-cache sidecar-skip #1663, flock-backend VM-verified), the `v0.16.1` headline (an A2A audit follow-up, iso-upgrade robustness #1635/#1636, MS365 `get_valid_token` #1650, `set -u` roster-map hardening #1627), and earlier `v0.16.0` / `v0.15.x` headlines carry forward. See [CHANGELOG.md](./CHANGELOG.md#0163--2026-06-08) for the full list.

**Previous stable**: v0.16.2 (2026-06-08) — superseded by `v0.16.3` on the same v0.16 LTS line; the official `0.16.2` release. Headline: **stabilization patch over `0.16.1` — four robustness/perf fixes surfaced by the v0.16.1 fleet rollout (macbook / SYRS mac-mini / cm-prod), VM-verified on Linux iso v2 with the flock backend** (the path that exposed two real production bugs the macOS mkdir-backend tests could not). The four fixes: `agb status` cron cadence-health is no longer O(cron-run-records × window) — memoized matcher + latest-run-per-`(agent, schedule)` reduction, so render scales with distinct schedules not historical run rows (was ≈84s CPU on a 5,632-record host, **#1659**); `upgrade --apply` preserves its intended exit code when stdout is closed via an `emit_json(payload, rc)` helper (observed exit 144 on a *successful* migration before, **#1660**); `agb upgrade`/`rollback` hold a singleton lock (shared `lib/bridge-lock.sh`, flock-first/`mkdir` fallback, `state/locks/upgrade.lock`), refusing a second concurrent run fast, also fixing two Linux-only flock bugs (`flock -w 0` → `flock -n`; a muted-stderr `exec`, **#1661**); and an upgrade-conflict sidecar can no longer cascade-fail iso agent launches — the iso plugin-cache build pattern-skips upgrade/VCS sidecars while *required* plugin-contract material fails loud (was a real cm-prod iso v2 outage, **#1663**). The earlier `v0.16.1` headline (an A2A audit follow-up, iso-upgrade robustness #1635/#1636, MS365 `get_valid_token` #1650, `set -u` roster-map hardening #1627), the `v0.16.0` headline (A2A rooms whole-room fan-out #1594 + Cloudflare One/WARP-Mesh transport #1595 + bounded AskUserQuestion #1569, on the rc1-rc3 foundation), and the `v0.15.x` headlines carry forward. See [CHANGELOG.md](./CHANGELOG.md#0162--2026-06-08) for the full list.
- **Recommended upgrade target.** `v0.16.3` (current stable / head of the v0.16 LTS line) supersedes `v0.16.2` / `v0.16.1` / `v0.16.0` / `v0.15.4` / `v0.15.3` / `v0.15.2` / `v0.15.1` / `v0.15.0`, all the `v0.16.0-rc1..rc3` / `v0.15.0-rc1` / `v0.15.0-betaN` / `v0.14.5-betaN` prereleases — treat the latest stable tag as current for production. The `v0.16.0-rc1..rc3` candidates are folded into the `v0.16.0`→`v0.16.1`→`v0.16.2`→`v0.16.3` stable line. Conservative installs that want to stay on this LTS line across future minors can pin with `agb upgrade --channel lts` (see [OPERATIONS.md §"Upgrade"](./OPERATIONS.md#upgrade)).
- v0.7.x → v0.16.3 leap path: **direct `agent-bridge upgrade --apply` to `v0.16.3`** is the current recommended (stable) path — the v0.13.7-v0.13.10 cycle fixed the Bash 5.3.9 heredoc-stdin deadlock chain by extracting leap-path bodies to `lib/upgrade-helpers/`, so the leap is single-atomic from any v0.7.x/v0.8.x/v0.9.x/v0.10.x/v0.11.x/v0.12.x/v0.13.x/v0.14.x/v0.15.x/v0.16.x source. v0.13.10 is the minimum-safe fallback if you're pinned below v0.14.x or troubleshooting an in-flight leap. See [OPERATIONS.md §"Upgrade"](./OPERATIONS.md#upgrade) for recipe + follow-up notes.
