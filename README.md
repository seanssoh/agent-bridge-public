...
**Current stable / LTS**: v0.16.7 (2026-06-10) — the current head of the **v0.16 Long-Term Support (LTS) line** and its recommended install. The LTS designation is on the `v0.16` *line*; every v0.16.x release is an LTS release — `v0.16.7` supersedes `v0.16.6` as the line head (same line, not a different LTS). Headline: **`trusted-routed` A2A transport** — a new `transport.kind` for hosts on a corporate-routed trusted private network (**#1758**): the receiver binds its configured private `listen.address` with an interface-assignment proof and **no WARP/Tailscale enrollment required** (explicit address required — empty hard-fails; loopback/wildcard refused; unknown kinds hard-fail; HMAC / allowlist / source-check / dedupe byte-untouched), and a **per-peer `transport.kind` override** lets one node serve mesh peers and routed peers simultaneously — the sender picks the egress source per destination (mesh peers keep the pinned mesh source; routed-marked peers go OS-routed), with a typo'd peer entry degrading that one row to retry instead of halting the runner. This is an operator-directed capability-add on the LTS line (see `docs/release-lines.md`) so `lts`-channel installs can receive it. Also: dynamic-agent every-prompt "UserPromptSubmit hook error" eliminated (**#1755** — cross-scope pinned-interpreter hook dedupe + unique-temp atomic writes with no error swallow) and operator-pinned session preferences (`model`, thinking/notif toggles) survive settings re-renders incl. symlink-adoption fold with the launched-channel sticky-false repair (**#1756**). The `v0.16.6` headline (mesh-robustness + read-guard hardening + reliability sweep, 10 lanes), the `v0.16.5` headline (zero-touch A2A Rooms mesh #1705-#1708/#1695), and earlier `v0.16.x` / `v0.15.x` headlines carry forward. See [CHANGELOG.md](./CHANGELOG.md#0167--2026-06-10) for the full list, [docs/a2a-cross-bridge.md](./docs/a2a-cross-bridge.md) for the trusted-routed trust model + rollout recipe, and [docs/a2a-rooms.md](./docs/a2a-rooms.md) for mesh operator usage.

**Previous stable**: v0.16.6 (2026-06-10) — superseded by `v0.16.7` on the same v0.16 LTS line; the official `0.16.6` release. Headline: **mesh-robustness + read-guard hardening + reliability sweep** — a 10-lane wave: transient-peer resilience + WARP bounce-gate (**#1732**/**#1733**/**#1742**), the read-guard static-decodable bypass class closed (**#1709**), `config set-env` (**#1734**), test-bind rooms.db guard (**#1728**), queue-gateway crash-loop fix (**#1652**), upgrade-time orphan reaper (**#1567**), settings cosmetic-conflict collapse (**#1675**/**#1694**), operator-global settings inheritance (**#11901**), A2A receiver-supervisor robustness on macOS tailnet conditions (**#1679**/**#1680**), and the fresh-install admin-identity scaffold fix (**#1750**). See [CHANGELOG.md](./CHANGELOG.md#0166--2026-06-10) for the full list.
- **Recommended upgrade target.** `v0.16.7` (current stable / head of the v0.16 LTS line) supersedes `v0.16.6` / `v0.16.5` / `v0.16.4` / `v0.16.3` / `v0.16.2` / `v0.16.1` / `v0.16.0` / `v0.15.4` / `v0.15.3` / `v0.15.2` / `v0.15.1` / `v0.15.0`, all the `v0.16.0-rc1..rc3` / `v0.15.0-rc1` / `v0.15.0-betaN` / `v0.14.5-betaN` prereleases — treat the latest stable tag as current for production. The `v0.16.0-rc1..rc3` candidates are folded into the `v0.16.0`→`v0.16.1`→`v0.16.2`→`v0.16.3`→`v0.16.4`→`v0.16.5`→`v0.16.6`→`v0.16.7` stable line. Conservative installs that want to stay on this LTS line across future minors can pin with `agb upgrade --channel lts` (see [OPERATIONS.md §"Upgrade"](./OPERATIONS.md#upgrade)).
- v0.7.x → v0.16.7 leap path: **direct `agent-bridge upgrade --apply` to `v0.16.7`** is the current recommended (stable) path — the v0.13.7-v0.13.10 cycle fixed the Bash 5.3.9 heredoc-stdin deadlock chain by extracting leap-path bodies to `lib/upgrade-helpers/`, so the leap is single-atomic from any v0.7.x/v0.8.x/v0.9.x/v0.10.x/v0.11.x/v0.12.x/v0.13.x/v0.14.x/v0.15.x/v0.16.x source. v0.13.10 is the minimum-safe fallback if you're pinned below v0.14.x or troubleshooting an in-flight leap. See [OPERATIONS.md §"Upgrade"](./OPERATIONS.md#upgrade) for recipe + follow-up notes.
