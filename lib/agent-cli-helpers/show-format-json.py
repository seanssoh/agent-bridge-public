...
Invocation contract (7 positional arguments — file paths):
    ...
    sys.argv[7] = iso-boundary-quickref.txt [Issue #1357]
...
Issue #1357 (iso_boundary_quickref): null for shared-mode agents, list
of rows for iso v2 effective agents. 7th argv optional for backward
compatibility with older dispatchers — treats missing as null.

Output: a single JSON document on stdout matching the historical
`agent show --json` schema, plus the #1360 `next_actions` array and
the #1357 `iso_boundary_quickref` field.
...
`python3 show-format-json.py <records> <diag> <health> <session-source> <alive> [<next-actions>] [<iso-quickref>]`
...
def main() -> int:
    ...
    # Issue #1357: surface the iso boundary quickref. Empty file marker
    # (shared mode) maps to null; non-empty file becomes a list so
    # downstream consumers can render each row independently. Defensive:
    # if argv[7] is missing (older dispatchers) treat as null rather than
    # raising IndexError so the JSON contract stays backward-compatible.
    quickref_rows = None
    if len(sys.argv) > 7:
        raw_quickref = _read_text(sys.argv[7])
