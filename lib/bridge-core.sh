...
bridge_reset_roster_maps() {
  ...
  # #2051: per-agent restart-peer map (self-restart guard redirect/delegate target).
  unset BRIDGE_AGENT_RESTART_PEER
  ...
  # Issue #1234 (Lane δ, v0.15.0-beta2): per-agent daemon auto-start policy
  # ("auto" | "hold"). Operator-controlled affordance to suppress the warm
  # always-on autostart loop while channel setup is intentionally incomplete.
  # Default unset → reads as "auto" via bridge_agent_start_policy, preserving
  # pre-existing behavior for every agent that has never been touched by
  # `agent update --start-policy ...`. Associative array (per #1213 lessons —
  # never expose a scalar `BRIDGE_AGENT_START_POLICY` env to subprocesses).
  ...
  # #2051: restart-peer map — keep associative (same #2020 index-0 hazard).
  declare -g -A BRIDGE_AGENT_RESTART_PEER=()
