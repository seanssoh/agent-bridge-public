...
| Edit `<bridge_home>/plugins-cache/<marketplace>/<plugin>/.mcp.json` | `EACCES` (mode 0640 `awfmanager:ab-shared`, you are group member but not owner) | Ask the operator to run controller-side `agb plugins seed` (or the marketplace-specific seed) and then restart you. Most plugins ship as stdio proxies that look up tokens at call time, so direct `.mcp.json` edits are rarely the right answer anyway. |
