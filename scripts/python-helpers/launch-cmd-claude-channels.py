#!/usr/bin/env python3
"""Merge required channel ids into a Claude launch command.

Extracted from `lib/bridge-state.sh::bridge_claude_launch_with_channels`
as part of issue #835 (Wave A). The previous in-line Python body was
read through bash stdin redirection; on Homebrew Bash 5.3.9 that read
can wedge in `heredoc_write` when the wrapper is invoked inside a
command substitution from an absolute-path-sourced shell — the same
class that closed #800 / #815 / #827 / #840 for the daemon, CLI,
status, and session-id hot paths. Living in a real script bypasses the
...
Args (positional, order-sensitive):
    ...
    sys.argv[2] — required_csv (comma-separated channel ids to ensure)

Stdout: rewritten launch_cmd with each required channel passed as a
distinct `--channels <id>` pair. Existing `--channels` and
`--channels=<id>` tokens are absorbed and re-emitted in a canonical
de-duplicated order (existing first, then required). Always exits 0.
"""
