...
select_for_path() {
  ...
  case "$path" in
    ...
    bridge-queue.py|bridge-task.sh|bridge-audit.py)
      ...
      # → wrongly requeued).
      add_required queue 1106-nudge-shell-recheck nudge-task-age-gate nudge-redundant-active-agent 1323-nudge-eligibility-recheck-twostage a2a-cross-bridge 1100-audit-since-tz 1115-cli-usage-drift J-beta4-workflow-docs K-beta4-nits beta5-2-delta-nudge-session-empty beta5-2-kappa-state-audit-reconcile 1408-daemon-alert-nudge-hygiene
      add_integration integration-minimal
      ;;
