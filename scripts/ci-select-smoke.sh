...
select_for_path() {
  ...
  case "$path" in
    ...
    bridge-cron.py|bridge-cron-runner.py|bridge-cron-scheduler.py)
      ...
      add_required cron-run-artifacts-retention cron-migrate-payloads cron-shell-runner queue
      add_integration integration-minimal
      ;;
