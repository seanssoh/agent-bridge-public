...
# from a cached os.scandir DirEntry. 1966-orphan-walk-prune is the non-vacuous
# correctness teeth (keep-set RESULT byte-identical pre/post the prune). In the
# full static suite so any scripts/smoke/* or bridge_orphan_classifier.py move
# re-runs the result-preserving guard.
add_required 1966-orphan-walk-prune
...
# Issue #2079: A2A cross-server routing authz (receiver-enforced symmetric
# admin↔admin gate). For room-scoped CROSS-NODE delivery, if EITHER endpoint is
# the configured bridge-admin of its node, the delivery is allowed ONLY when
# BOTH are configured admins. The admin signal is a TRI-STATE `bridge_admin`
# bit derived from each node's CONFIGURED admin id (NOT name=='patch'),
# materialized into the leader-signed roster + member cache (bridge_rooms_common.py:
# _sorted_members/_canonical_member_list/_cached_members + admin_cross_node_authz/
# room_admin_authz_check/relay_admin_authz_check), persisted via the join request
# (bridge_a2a_common.py build/parse_room_join_request joiner_is_admin) onto
# room_members.admin (approve_join/approve_cross_node/create_room). Receiver
# enforcement is post-HMAC in bridge-handoffd.py (room_scoped_check + _relay_resolve),
# collapsing every admin-authz reject to a SINGLE generic 403 (reason audit-only,
# no oracle); the sender-side UX guard (bridge-rooms.py cmd_talk) refuses a bare
# ambiguous --to before any POST. In the static suite so any scripts/smoke/* or
# rooms-layer change re-runs the 10 mutation-proven teeth via the catch-all.
add_required 2079-routing-authz
# Issue #2081 (SECURITY, fail-open fix): the #2079 admin↔admin authz predicate
# recomputes a LOCAL endpoint's admin status from $BRIDGE_ADMIN_AGENT_ID, but the
# receiver LAUNCH paths did not provide it (the systemd handoffd unit runs with
# only BRIDGE_HOME; `agb a2a daemon start` / `bridge-handoff-daemon.sh start`
# reach the spawn without a roster load), so a LOCAL admin classified UNKNOWN and
# `non-admin@remote -> admin@local` failed OPEN. The fix RESOLVES the admin id
# from the on-disk roster on every launch path before serving:
# bridge-handoffd.py serve -> rooms.ensure_admin_agent_id_in_env (covers the
# systemd unit via startup-resolution), and lib/bridge-a2a.sh
# bridge_a2a_export_admin_agent_id (covers the shell daemon-start paths). This
# smoke mutation-proves the env-unset/systemd-style launch DENIES the delivery,
# vs the pre-fix ALLOW. In the static suite so any scripts/smoke/* or rooms-layer
# change re-runs it via the catch-all.
add_required 2081-launch-admin-id
# Issue #2109 (durable half of #2079): robust room_members.admin migration +
# local-admin backfill + leader-only roster re-broadcast. _migrate_schema now
# PRECHECKS each (table,column) via PRAGMA table_info and swallows ONLY a
# duplicate-column race on the ALTER — a real `database is locked` RE-RAISES so a
# half-migrated schema can't silently crash the next `room show`. The reconcile
# roster-epoch step (bridge_reconcile_common.py) backfills THIS node's admin bits
# from the local configured admin id (bridge_rooms_common.py backfill_local_admin
# — local-node rows only, empty config leaves -1, never infers a remote admin)
# and, as the LEADER, bumps the epoch + enqueues a durable roster rebroadcast
# (reclassify_and_rebroadcast_local_admin via bump_epoch + enqueue_roster_broadcast).
# In the static suite so any scripts/smoke/* or rooms/reconcile-layer change
# re-runs the mutation-proven teeth via the catch-all.
add_required 2109-rooms-admin-migration
# Issue #16247 (A2A DNS-hostname peer keying): a 4th peer-identity leg
# `hostname` (DNS A/AAAA) live-resolves via getaddrinfo (precedence node_id >
# tailscale_name > hostname > literal address). The RECEIVER source-address
# check (bridge-handoffd.py 4 sites) now accepts MEMBERSHIP in the peer's full
# current A/AAAA set (peer_source_addresses in bridge_a2a_common.py) — a
# multi-homed peer is accepted on any current IP, not a single string —
# IPv4-mapped-normalized, fail-closed + bounded (timeout) with a positive
# cache that serves across a transient DNS blip and a short negative cache.
# Mutation-proves the single-string collapse fails the multi-A non-first case.
add_required 16247-hostname-keying
...
select_for_path() {
  ...
  case "$path" in
    ...
    docs/agent-runtime/memory-schema.md)
      # Issue #1814: memory-schema.md is the SSOT body that
      # render_shared_memory_schema_md propagates into shared/MEMORY-SCHEMA.md
      # and symlinks from each home. A change to the canon must rerun the
      # propagation + clobber-guard smoke. Precedes the is_docs_only_path
      # early-return below (which would otherwise select only the global set).
      add_required 1814-memory-schema-symlink-clobber-guard
      ;;
    docs/agent-runtime/common-instructions.md)
      # Issue #1814: common-instructions.md is now propagated verbatim into
    ...
    agents/_template/MEMORY-SCHEMA.md)
      ...
      # editable fork. Rerun the SSOT/clobber smoke so a re-expansion back into
      # a full schema body (the divergence #1814 retired) is caught.
      add_required 1814-memory-schema-symlink-clobber-guard
      ;;
    agents/_template/CLAUDE.md|agents/_template/.claude/commands/wrap-up.md)
      ...
      # Issues #1815/#1816: the template CLAUDE.md block is single-sourced from
      # the engine render and must stay pointer-only + ghost-ref-free; rerun the
      # ghost-ref gate and the marker/stamp/backup smoke on any template drift.
      add_required 1060-layout-fresh-v2-static-claude 1060-layout-fresh-v2-static-codex 1060-layout-shared-workdir-pair 1067-codex-provisioning 1815-managed-block-ghost-ref-gate 1816-managed-block-marker-stamp-backup
      ;;
  ...
  case "$path" in
    agent-bridge|agb|bridge-upgrade.sh|bridge-memory.sh|bridge-intake.sh|bridge-bundle.sh|bridge-cron.sh|bridge-daemon.sh|bridge-discord-relay.sh|bridge-send.sh|bridge-agent.sh|bridge-profile.sh|bridge-task.sh)
      ...
      case "$path" in
        agent-bridge|agb)
          ...
          ;;
        bridge-agent.sh)
          # FR #2061 Track B: run_convert (the `agent convert <a> --to static`
          # verb) orchestrates the §0.1 last-flip transaction — quiesce →
          # materialize + #1455 invariant → Track A apply → crash-clear →
          # audited roster flip (LAST) → optional start, with internal
          # rollback on any pre-flip failure. Pull 2061-convert-verb on every
          # bridge-agent.sh move so the ordering contract (a mid-convert
          # failure NEVER leaves a static-but-empty role), the baked
          ...
          # Issue #2216: run_convert carries the source dynamic agent's RESOLVED
          # model+effort (the operator-global model a dynamic-vanilla agent
          # inherits, NOT the empty roster field) into the baked launch_cmd, the
          # materialized roster fields, and the rendered settings.effective.json
          # so a converted role boots on the working model instead of the
          # unavailable user-class template default. Pull the carry-over smoke on
          # every bridge-agent.sh move so the operator-global resolution + the
          # override precedence cannot silently regress.
          add_required 2216-convert-preserve-model-effort
          ;;
      esac
      ...
      if [[ "$path" == "bridge-agent.sh" ]]; then
        ...
        # Issue #2063: bridge-agent.sh's run_ack_crash now retires a VALID crash
        # report when the agent is currently healthy (was: ACK-suppress only,
        # leaving the report to re-fire forever), and the new run_clear_crash
        # verb retires it unconditionally. Pull the smoke on every bridge-agent.sh
        # move so a refactor cannot drop the healthy-retire decision or the
        # clear-crash verb wiring.
        add_required 2063-stale-crash-report-launch-cmd-guard
        ...
        # Issue #2014: bridge-agent.sh's bridge_ensure_auto_memory_isolation
        # now seeds an ABSOLUTE autoMemoryDirectory (no leading "~" for the
        # Claude CLI to mis-expand per session) resolved via
        # bridge_agent_claude_home_dir (operator home on shared mode, iso UID
        # home on Linux iso-v2), and auto-upgrades the bridge's own legacy
        # tilde seed in place while still fail-closing on a user-customized
        # value. Pull the smoke on every bridge-agent.sh move so a refactor
        # cannot reintroduce the tilde seed or clobber an operator-set value.
        ...
        add_required 1968-forget-session-fresh 2014-auto-memory-absolute-seed
      fi
      ;;
    lib/bridge-cron.sh|lib/bridge-state.sh)
      ...
      if [[ "$path" == "lib/bridge-state.sh" ]]; then
        ...
        add_required 1795-reaper-ephemeral-policy
        # Issue #2063: lib/bridge-state.sh hosts bridge_agent_crash_report_launch_
        # stale (the daemon staleness-guard predicate) + bridge_agent_clear_crash_
        # report (the retire primitive). Pull the stale-crash-report smoke on every
        # bridge-state.sh move so a refactor cannot drop the launch-base signature
        # compare (which would re-open the false crash-loop churn on a healthy
        # agent after a launch-cmd change) or weaken the fail-safe bias that keeps
        # a real same-launch crash alarming.
        add_required 2063-stale-crash-report-launch-cmd-guard
  ...
  case "$path" in
    ...
    plugins/teams/cardintent.ts|plugins/teams/cardintent.test.ts)
      # v0.17.0-beta2 (Model B Adaptive Card): cardintent.ts is the `cardintent`
      # fence parser + quoteResult Adaptive Card (v1.2) renderer. The smoke runs
      # the bun unit suite (fence extraction, CardIntent validation, valueState
      # mapping, list/detail render shape) + the §10 forbidden-cost-key golden +
      # the graceful never-throw fallback, with a teeth mutation that neuters the
      # §10 guard on a copy. Pull on every cardintent.ts / .test.ts touch so a
      # refactor cannot silently drop the §10 guard or the fallback contract.
      add_required cardintent-ac-render
      add_integration integration-minimal
      ;;
    ...
    plugins/ms365/server.ts|plugins/teams/server.ts)
      ...
      # `cardintent` fence parser + quoteResult Adaptive Card renderer that the
      # `reply` tool's text-only path calls via the renderOutbound() seam. The
      # cardintent-ac-render smoke pins the §10 forbidden-cost-key golden, the
      # graceful never-throw fallback (no fence / invalid JSON / schema fail →
      # text-only, fence stripped, existing reply path UNCHANGED), and that
      # server.ts actually wires renderOutbound() in (renderer not orphaned),
      # via the bun unit suite + a teeth mutation on a copy. Pull on every
      # teams/server.ts touch so a refactor cannot silently orphan the renderer
      # or drop the §10 guard.
      add_required cardintent-ac-render
      ...
      # Issue #2035 (ms365 token-freshness hardening): getAccessToken /
      # get_valid_token now (item 1) normalize a legacy 13-digit millisecond
      # `expires_at` to seconds on read via normalizeTokenExpiry (>1e12 → /1000)
      # so a legacy ms-format token file no longer reads as valid-forever and
      # never refreshes; and (item 2) get_valid_token gains an optional
      # min_remaining_seconds / force proactive-refresh override (CLI:
      # --min-remaining / --force) WITHOUT changing the global 300s constant.
      # The 2035 smoke drives the real freshness path via the get-valid-token
      # CLI with a fetch-mock so every assertion is mutation-proven (refresh
    ...
    bridge-queue.py|bridge-task.sh|bridge-audit.py)
      ...
      # Issue #1973 Track A: bridge-queue.py's proxy_via_queue_gateway now
      # special-cases the gateway EX_TEMPFAIL (75) for claim/done — preserving
      # the nonzero rc and printing safe-next-command guidance, NEVER fabricating
      # success. Pull 1973a-gateway-drain-containment on every bridge-queue.py
      # move so the degraded-claim/done contract cannot regress to a false 0.
      add_required queue 1106-nudge-shell-recheck nudge-task-age-gate nudge-redundant-active-agent 1323-nudge-eligibility-recheck-twostage 1199-action-required-claimed-skip a2a-cross-bridge 1100-audit-since-tz 1115-cli-usage-drift J-beta4-workflow-docs K-beta4-nits beta5-2-delta-nudge-session-empty beta5-2-kappa-state-audit-reconcile 1408-daemon-alert-nudge-hygiene 1973a-gateway-drain-containment
      ...
      add_required 1425-cron-dispatch-nudge-scope
      # Issue #1944 (cm-prod F6): the unclaimed-escalation once-latch smoke
      # drives process_unclaimed_queue_escalation end-to-end against a real
      # queue DB, so it depends on the find-open --all / upsert-open / show
      # shapes bridge-queue.py owns (created_ts / status / assigned_to /
      # body rendering). Pull it on every bridge-queue.py move so a refactor
      # of those shapes cannot silently break the dedupe contract.
      add_required 1944-unclaimed-escalation-once-latch
      # Issue #1986 (cm-prod RCA): process_blocked_task_aging now re-surfaces a
      # re-bound (existing-open) blocked-aging reminder/escalation VISIBLY on its
      # cadence (resurface_open_alert: re-queue + drop last_nudge_key) instead of
      # a silent in-place refresh, and relaxes the escalation one-shot to a
      # bounded periodic re-fire. The smoke drives daemon-step end-to-end against
      # a real queue DB, depending on the upsert-open re-bind, the daemon-step
      # nudge-candidate tsv, and the §30 done-re-mint shape. Pull it on every
      # bridge-queue.py move so a refactor cannot silently re-introduce the
      # silent-refresh (no visible re-fire) or break the §30 dedupe contract.
      add_required 1986-blocked-aging-visible-refire
      # Issue #2067: the daemon exempts the blocked-aging REMINDER class
      # (created_by=='daemon' AND title startswith "[blocked-aging] task #")
      # from the [unclaimed-task] escalation, in lib/daemon-helpers/
      # unclaimed-task-filter.py. The 2067 smoke cross-checks against
      # bridge-queue.py's BLOCKED_REMINDER_TITLE_PREFIX constant and exercises
      # create / find-open / cancel against a real queue DB. Pull it on every
      # bridge-queue.py move so a rename of the title-prefix constant or a change
      # to the create/find-open shape cannot silently desync the exemption from
      # the emit path (the reminder→admin noise would return).
      add_required 2067-reminder-unclaimed-exempt
      ...
      add_required 1792-cron-scope-fence
      # Issue #1970: bridge-queue.py's daemon idle stale-claim reclaim is now
      # lease-aware (skips a LIVE-idle claimant while lease_until_ts is valid;
      # inactive branch UNCHANGED; NULL lease still reclaimable) and cmd_update
      # renews the lease for the claimant/assignee only. Pull
      # 1970-idle-reclaim-lease-aware on every bridge-queue.py move so a
      # refactor cannot regress the idle-branch lease gate, re-introduce the
      # in-progress-claim-yank false positive, or let a creator/admin update
      # spoof-extend the claimant's lease.
      add_required 1970-idle-reclaim-lease-aware
      add_integration integration-minimal
      ;;
    ...
    bridge-stall.py|bridge-audit.sh)
      ...
      add_required beta5-2-kappa-state-audit-reconcile
      # Issue #1991 safety floor (+ #2007 codex extension): bridge-stall.py hosts
      # the detect-only typed blocked-prompt classifier (`detect-prompt
      # [--engine claude|codex]`) the daemon safety-floor sweep consumes. Pull
      # the floor smoke on every bridge-stall.py move so a regression to the
      # affordance gates / coarse_state mapping / shell-format field names — or
      # to the #2007 Codex hook-trust / unknown-modal detectors and the engine
      # dispatch — is caught at PR time.
      add_required 1991-blocked-prompt-safety-floor
      add_required 1991-agentic-resolver
      # Issue #2217 (roadmap step 5): bridge-stall.py's flush-left raw `Error: HTTP
      # 429` classification is the L3 (inference/picker) entry to the auto-rotate
      # decision chain. The cross-layer integration smoke pins the whole route
      # (HUD stdin-tap → native usage-probe → reactive-429). Pull it on every
      # bridge-stall.py move so a change to the flush-left/transport-qualified
      # 429 detection can't silently sever the reactive backstop.
    ...
    bridge-a2a.py|bridge-handoffd.py|bridge_a2a_common.py|bridge_reconcile_common.py|bridge-rooms.py|bridge_rooms_common.py|bridge-handoff-daemon.sh|lib/bridge-a2a.sh|lib/daemon-helpers/a2a-receiver-exit-cause.py|lib/daemon-helpers/a2a-receiver-staleness.py|handoff.local.example.json|scripts/smoke/a2a-cross-bridge-helper.py|scripts/smoke/a2a-tailscale-identity-resolve-helper.py|scripts/smoke/a2a-daemon-selfheal-reconcile-helper.py|scripts/smoke/a2a-migrate-identity-helper.py|scripts/smoke/a2a-ip-change-announce-helper.py|scripts/smoke/a2a-setup-wizard-helper.py|scripts/smoke/a2a-rooms-p1a-helper.py|scripts/smoke/a2a-rooms-p1b-acl-helper.py|scripts/smoke/a2a-rooms-1517-bootstrap-helper.py|scripts/smoke/rooms-p4-1-cross-node-join-helper.py|scripts/smoke/rooms-p4-1-post-hook.sh|scripts/smoke/rooms-p4-2-roster-broadcast-helper.py|scripts/smoke/rooms-p4-2-post-hook.sh|scripts/smoke/rooms-p4-3-room-talk-helper.py|scripts/smoke/rooms-p4-3-post-hook.sh|scripts/smoke/1594-rooms-fanout-helper.py|scripts/smoke/1594-rooms-fanout-post-hook.sh|scripts/smoke/1594-rooms-fanout-local-hook.sh|scripts/smoke/rooms-p4-5-polish.sh|scripts/smoke/rooms-p4-5-helper.py|scripts/smoke/1563-pr8-a2a-diag-recovery-helper.py|scripts/smoke/1575b-a2a-backoff-ceiling-helper.py|scripts/smoke/1618-outbox-retry-resets-attempts-helper.py|scripts/smoke/1628-a2a-deliver-per-row-guard-helper.py|scripts/smoke/1629-healthz-not-semaphore-gated-helper.py|scripts/smoke/1595-cloudflare-warp-mesh-helper.py|scripts/smoke/1758-trusted-routed-transport.sh|scripts/smoke/1758-trusted-routed-transport-helper.py|scripts/smoke/1701-warp-healthz-socket-held-helper.py|scripts/smoke/1697-a2a-net-status-helper.py|scripts/smoke/1685-boot-marker-helper.py|scripts/smoke/v0165-l0-reconcile-skeleton-helper.py|scripts/smoke/v0165-l2-tunnel-health-helper.py|scripts/smoke/v0165-l1-stable-addr-helper.py|scripts/smoke/v0165-l3-peer-reachability-helper.py|scripts/smoke/v0165-l4-token-join-helper.py|scripts/smoke/v0165-l4-token-join-post-hook.sh|scripts/smoke/v0165-l5-relay-roster-helper.py|scripts/smoke/v0165-l5-relay-post-hook.sh|scripts/smoke/v0165-l5-roster-post-hook.sh|scripts/smoke/v0165-l6-net-status-v2.sh|scripts/smoke/v0165-l6-net-status-v2-helper.py|scripts/smoke/v0166-la-tunnel-bounce-gate.sh|scripts/smoke/v0166-la-tunnel-bounce-gate-helper.py|scripts/smoke/v0166-lb-transient-peers.sh|scripts/smoke/v0166-lb-transient-peers-helper.py|scripts/smoke/1728-test-bind-state-path-guard.sh|scripts/smoke/1728-test-bind-state-path-guard-helper.py|scripts/smoke/1679-1680-a2a-receiver-supervisor-robustness.sh|scripts/smoke/15783-a2a-restart-sources-env.sh|scripts/smoke/2073-autojoin-stale-peer-selfheal.sh|scripts/smoke/2073-autojoin-stale-peer-selfheal-helper.py|scripts/smoke/2073-autojoin-stale-peer-selfheal-post-hook.sh|scripts/install-handoffd-systemd.sh|scripts/smoke/2025-a2a-whois-discovery.sh|scripts/smoke/2025-a2a-whois-discovery-helper.py|scripts/smoke/2079-routing-authz.sh|scripts/smoke/2079-routing-authz-helper.py|scripts/smoke/2081-launch-admin-id.sh|scripts/smoke/2081-launch-admin-id-helper.py|scripts/smoke/2081-roster-parse-cases.tsv|scripts/smoke/2109-rooms-admin-migration.sh|scripts/smoke/2109-rooms-admin-migration-helper.py|scripts/smoke/2140-handoffd-detach-reexec.sh|scripts/smoke/16247-hostname-keying.sh|scripts/smoke/16247-hostname-keying-helper.py)
      ...
      # Issue #15783: bridge_a2a_receiver_start (lib/bridge-a2a.sh) sources the
      # managed install-wide env-override file (agent-env.local.sh) before the
      # `nohup python3 bridge-handoffd.py serve` spawn so the receiver's
      # os.environ inherits every `agb config set-env` override
      # (BRIDGE_A2A_ROOM_AUTOJOIN + all BRIDGE_A2A_*) regardless of the caller's
      # ambient env — fixing the no-op documented enable procedure when the
      # restart comes from a clean terminal via bridge-handoff-daemon.sh (which
      # never loads bridge_load_roster). 15783-a2a-restart-sources-env drives the
      # REAL receiver-start with a stub handoffd that dumps os.environ and proves
      # BOTH overrides propagate; its mutation case removes the source line and
      # asserts the override then disappears. Pull on every move to the receiver
      # lifecycle helper so the propagation cannot regress to the no-op state.
      add_required a2a-cross-bridge queue I-beta4-a2a-3-gaps J-beta4-workflow-docs beta5-2-lambda-a2a-robustness a2a-tailscale-identity-resolve a2a-daemon-selfheal-reconcile a2a-migrate-identity a2a-ip-change-announce 1405-handoffd-supervision 1679-1680-a2a-receiver-supervisor-robustness a2a-setup-wizard a2a-rooms-p1a a2a-rooms-p1b-acl a2a-rooms-1517-bootstrap 1398-a2a-inbound-stopped-target-force a2a-backpressure-openonly 1623-a2a-backpressure-failopen rooms-p4-1-cross-node-join rooms-p4-2-roster-broadcast rooms-p4-3-room-talk 1594-rooms-fanout rooms-p4-5-polish 1563-pr4-a2a-receiver-healthz 1563-pr5-fp-control-matrix 1563-pr8-a2a-diag-recovery 1575b-a2a-backoff-ceiling 1618-outbox-retry-resets-attempts 1628-a2a-deliver-per-row-guard 1595-cloudflare-warp-mesh 1758-trusted-routed-transport 1612-upgrade-restart-receiver 1630-a2a-fresh-arrival-nudge 1629-healthz-not-semaphore-gated 1701-warp-healthz-socket-held 1697-a2a-net-status 1685-receiver-staleness-selfheal v0165-l0-reconcile-skeleton v0165-l2-tunnel-health v0165-l1-stable-addr v0165-l3-peer-reachability v0165-l4-token-join v0165-l5-relay-roster v0165-l6-net-status-v2 v0166-la-tunnel-bounce-gate v0166-lb-transient-peers 1728-test-bind-state-path-guard 1933-a2a-from-id 15783-a2a-restart-sources-env 2073-autojoin-stale-peer-selfheal 2025-a2a-whois-discovery 2079-routing-authz 2081-launch-admin-id 2109-rooms-admin-migration 2140-handoffd-detach-reexec 16247-hostname-keying
      ;;
    ...
    bridge-daemon.sh|bridge-sync.sh|bridge-watchdog.sh|bridge-cron.sh|lib/bridge-cron.sh|lib/bridge-state.sh|lib/bridge-notify.sh)
      # Issue #68 (live-leak guard): bridge-daemon.sh's verb dispatch calls
      # bridge_guard_foreign_checkout "$CMD" before the `case`. Re-run the
      # guard smoke on any daemon move so a refactor of the dispatch cannot
      # drop or misplace the fail-closed call (the smoke asserts wiring +
      # position statically).
      add_required 68-live-leak-foreign-checkout-guard
      # Issue #2196: bridge-daemon.sh hosts the codex app-server orphan reaper —
      # the cadence-gated periodic pass (process_codex_app_server_reaper) and the
      # standalone `reap-codex-orphans` subcommand, both sharing
      # bridge_codex_app_server_reaper_run's live-session protection. Re-run the
      # live-protection smoke on any daemon move so a refactor cannot regress the
      # gating (ppid==1 + not-live + age floor) or unwire the pass/verb.
      add_required 2196-codex-orphan-reaper
      # #21895 phase-1 (sub-PR 4/4): bridge-daemon.sh hosts the token-updater
      # lease lifecycle tick (bridge_daemon_token_lease_tick registered in the
      # main loop on BRIDGE_USAGE_MONITOR_INTERVAL_SECONDS) + its bounded
      # exit-trap check-in folded into _bridge_daemon_on_exit. Re-run the tick
      # smoke on any daemon move so a refactor cannot regress the default-OFF
      # no-op, the re-checkout triggers (expiry / mapping drift / heartbeat
      # 404|409), the cadence gate, or the "single EXIT trap" invariant.
      add_required token-updater-daemon-tick
      ...
      # Issue #2036 (#1563/#1579 follow-up): cmd_sync_cycle now ALSO cadence-gates
      # the heavy formerly-un-gated steps (bridge_sync — single ≤30s site, the
      # post_sync second full sync removed — l1_roster_reload,
      # mcp_orphan_cleanup_early, cron_staging_apply, claude_token_recovery,
      # daily_backup, context_pressure_scan 30→60) so the serial cycle reaches the
      # idle-nudge dispatch EVERY ~5s tick instead of once per ~2-minute cycle.
      # 1563-pr7-tick-cadence gained the H1/H2/H3/G7/H5 teeth; 2036-daemon-nudge-
      # cadence is the behavioral mutation-proof (nudge every tick + per-step
      # intervals + fail-open). Pull BOTH on every bridge-daemon.sh move so a
      # revert that re-bloats the cycle or re-starves nudge is caught on the PR gate.
      ...
      # Issue #1996: bridge-daemon.sh's PreCompact notice + followup send
      # paths build the send-managed-message argv inline (bypassing
      # bridge_channel_send_managed_message), so they each resolve and thread
      # --teams-state-dir for teams. lib/bridge-notify.sh's bridge_notify_send
      ...
      # Issue #1973 Track A: process_queue_gateway_requests now names the
      # serve-once ceiling (BRIDGE_QUEUE_GATEWAY_SERVE_ONCE_TIMEOUT_SECONDS),
      # stops discarding all gateway stderr, and audits queue_gateway_drain_-
      # timeout / queue_gateway_drain_stalled with a `status --json` snapshot
      # instead of collapsing every failure to processed=0. Pull
      # 1973a-gateway-drain-containment on every bridge-daemon.sh move so the
      # drain-failure instrumentation cannot silently regress.
      # Issue #1955: bridge-daemon.sh's cmd_run now calls
      # bridge_daemon_self_diagnose at startup — a warn-only self-diagnosis
      # when the daemon is unsupervised (orphaned, no launchd/systemd job) or
      # running from a non-canonical source root (a dev/operator checkout).
      ...
      # (observe-only), storm, or — #2007 — drop the claude|codex engine gate /
      # the per-engine detector dispatch / the engine in the report+audit. Mutation-tested teeth.
      add_required 1991-blocked-prompt-safety-floor
      add_required 1991-agentic-resolver
      ...
      add_required 1568-routine-nudge-inject-busy-gate
      # Issue #2179: bridge-daemon.sh's nudge_agent_session now, on the
      # post-grace `submit_lost_post_grace` drop, distinguishes a claude
      # mid-turn/false-idle drop (banner is the live tail, blocker=none) from a
      # clean prompt miss and durably re-spools the mid-turn case via the
      # EXISTING pending-attention path (idempotency: at most one spooled copy;
      # the flush loop re-derives it against the live queue and drops it once the
      # task is claimed). Pull 2179-idle-nudge-durable-respool on every
      # bridge-daemon.sh move so the mid-turn classification, the blocker=none /
      # claude scoping, the single-copy idempotency lease, and the extended
      # respooled/respool_reason audit detail cannot silently regress back to the
      # ~30-min-latency drop-and-wait-for-escalation shape.
      add_required 2179-idle-nudge-durable-respool
      ...
      # Issue #1944 (cm-prod F6): the unclaimed-task escalation marker is
      # now a once-per-(agent, task) latch (cooldown defaults to 0) so a
      # long-stuck queued task is escalated ONCE, not re-minted every
      # cooldown window. Pull 1944-unclaimed-escalation-once-latch on every
      # bridge-daemon.sh move so a future PR cannot silently revert the
      # default back to the 1800s re-arm churn (the latch short-circuit,
      ...
      add_required 1944-unclaimed-escalation-once-latch
      # Issue #2067: process_unclaimed_queue_escalation now exempts the daemon
      # blocked-aging REMINDER class (created_by=='daemon' AND title startswith
      # "[blocked-aging] task #") from the [unclaimed-task] admin escalation, via
      # the lib/daemon-helpers/unclaimed-task-filter.py skip the daemon function
      # feeds. The smoke drives the escalation end-to-end against a real queue DB
      # and proves: a reminder is exempt, a genuine work task still escalates, a
      # non-daemon task with the same prefix STILL escalates (precision), the
      # admin-actionable [blocked-escalation] class is NOT exempt, and a mutation
      # removing the exemption re-escalates (non-vacuous). Pull it on every
      # bridge-daemon.sh move so a refactor of the scan loop or the helper
      # dispatch cannot silently re-introduce the N×N reminder→admin noise storm.
      add_required 2067-reminder-unclaimed-exempt
      ...
      # Issue #1789 (D2 pool-level cooldown): bridge-daemon.sh's
      # process_usage_monitor now SUPPRESSES the `claude-token rotate` attempt
      # on subsequent monitor passes while the whole token pool is saturated
      # (bridge_claude_pool_rotate_suppressed / _note_ / _clear_), instead of
      # re-forking the rotate+sync subprocess every 300s pass. Pull the smoke on
      # every bridge-daemon.sh move so a refactor cannot drop the suppression
      # gate (re-opening the across-pass round-robin thrash), move the rotate
      # fork above the gate, or break the record/clear of the cooldown window.
      add_required 1789-token-rotate-pool-cooldown
      # Issue #2171 PR-B2 Part1: bridge-daemon.sh's usage-monitor rotate call now
      # enables `--preflight --preflight-budget --preflight-timeout` for EVERY
      # rotation (proactive + reactive) so a selected candidate is LIVE-probed
      # before the active pointer syncs fleet-wide, and the `bridge_with_timeout`
      # ceiling is raised to budget + overhead so a full budgeted multi-candidate
      # ring fits inside it instead of being SIGKILLed into invalid_rotation_output.
      # 2171-rotate-preflight-budget's D1 block pins the daemon enablement + the
      # ceiling>=budget relationship. Pull it on every bridge-daemon.sh move so a
      # refactor cannot drop --preflight (re-dormanting B1) or shrink the ceiling
      # back below the budget.
      add_required 2171-rotate-preflight-budget
      # Issue #2171 PR-B2 Part2 (incident #19460 M4 fleet-down): bridge-daemon.sh's
      # nudge fanout now runs a deterministic active-dead recover BEFORE a blind
      # nudge — consuming the usage-probe Option-B auth-dead marker (engine==claude
      # + rotation-scope gated), then reusing the #19460 mark-adverse + the reactive
      # rotate (--preflight/--sync) + the #1789 pool-exhausted suppression. Pull
      # 2171-daemon-active-dead-recover on every bridge-daemon.sh move so a refactor
      # cannot drop the gate, move it outside the dead-session→nudge window, spawn a
      # second rotator/notifier, or break the suppress-on-pool-exhausted continue.
      add_required 2171-daemon-active-dead-recover
      # Issue #18849 Part 2 PR-2: the cascade rotator routes a fully-unavailable
      # pool to `all_tokens_limited`, which the daemon's process_usage_monitor
      # turns into the #1789 D2 suppression window + the latched operator
      # pool-exhausted notify. 18849-p2-cascade-warn includes source-level wiring
      # guards on that daemon handler (skipped:all_tokens_limited record + the
      # notice latch), so pull it on every bridge-daemon.sh move too.
      add_required 18849-p2-cascade-warn
      ...
      if [[ "$path" == "bridge-daemon.sh" ]]; then
        ...
        add_required 1738-config-caller-binding
        # Issue #2063: bridge-daemon.sh's process_crash_reports gained the
        # launch-cmd staleness guard — it retires a report.env whose recorded
        # CRASH_LAUNCH_CMD no longer matches the agent's current launch base
        # (auto-retire + audit-once) instead of re-driving the false alarm/
        # `crash_notified_origin_suppressed` churn on a healthy agent. Pull the
        # smoke on every bridge-daemon.sh move so a refactor cannot drop the
        # guard (re-opening the churn) or let it over-suppress a real same-launch
        # crash loop.
        add_required 2063-stale-crash-report-launch-cmd-guard
        # Issue #2217 (roadmap step 4): bridge-daemon.sh's rate_limit stall branch
        # gained bridge_daemon_reactive_429_rotate — the reactive 429 → preflighted
        # rotation trigger (feature flag + scope + CF/transport gate + per-token
        # latch + shared lock/cooldown). Pull the smoke on every bridge-daemon.sh
        # move so a refactor cannot drop a gate (which would let the managed pool
        # rotate on an edge/prose 429 or re-rotate the same capped token) or the
        # all-tokens-limited HOLD (which would loop).
        add_required 2217-reactive-429-rotate
        # Issue #2217 (roadmap step 5): the cross-layer decision-chain integration
        # smoke proves the WHOLE auto-rotate route (HUD stdin-tap → native
        # usage-probe → inference/picker reactive-429) — its scenario 2 chains the
        # sean-mac blind-probe backstop through bridge_daemon_reactive_429_rotate.
    ...
    lib/daemon-helpers/unclaimed-task-filter.py)
      # Issue #1970: this helper now ages a queued task from
      # max(created_ts, updated_ts) instead of created_ts only, so a
      # just-requeued / `agb update`-touched task gets a fresh grace window
      # before the [unclaimed-task] escalation fires. Pull
      # 1970-idle-reclaim-lease-aware (the filter's freshness boundary) AND
      # 1944-unclaimed-escalation-once-latch (the end-to-end escalation path
      # this helper feeds) on every move so a change to the freshness
      # computation (or the updated_ts default for legacy rows) cannot
      # silently re-open the post-requeue false-escalation or the #1944 churn.
      add_required 1970-idle-reclaim-lease-aware 1944-unclaimed-escalation-once-latch
      # Issue #2067: this helper now also EXEMPTS the daemon blocked-aging
      # REMINDER class (created_by=='daemon' AND title startswith
      # "[blocked-aging] task #") from the [unclaimed-task] escalation, keyed on
      # BOTH the title prefix and the daemon origin so a genuine work task with
      # the same prefix still escalates. Pull 2067-reminder-unclaimed-exempt on
      # every move so a change to the skip predicate (loosening it to title-only,
      # or dropping it) cannot silently re-open the N×N reminder→admin storm or
      # over-exempt real work.
      add_required 2067-reminder-unclaimed-exempt
      ;;
    ...
    lib/daemon-helpers/codex-app-server-reaper.py|scripts/smoke/2196-codex-orphan-reaper.sh)
      # Issue #2196: the codex app-server / app-server-broker.mjs orphan reaper
      # (broker reparented to launchd ppid==1, not backed by a live codex
      # session, age>=floor). The smoke drives the REAL reaper via the
      # --ps-snapshot test seam against real sleep stand-ins and asserts the KEY
      # safety property — a roster-backed live app-server with the same ppid==1
      # signature is NEVER reaped while genuine orphans ARE. Pull it on any move
      # of the detector/reaper or its smoke.
      add_required 2196-codex-orphan-reaper
      ;;
    ...
    lib/bridge-agent-convert.sh)
      # FR #2061 Track A: the pure config-dir migration engine
      # (bridge_convert_resolve_config_dirs / _enumerate_cwds / _build_manifest /
      # _apply_manifest / _rollback). Pull 2061-convert-migration on every move
      # of the engine so the zero-omission manifest, copy-not-move byte-equality,
      # backup + internal rollback, idempotent skip-if-identical, and the
      # iso-target fail-closed guard cannot silently regress (silent data loss is
      # the exact failure this engine exists to prevent).
      add_required 2061-convert-migration
      ;;
...
add_required launch launch-dev-channels-injection tmux-injection upgrade-source-preservation upgrade-shared-settings-propagate agent-create-name-validation agent-create-caller-trust-gate agent-create-idle-timeout 1105-agent-add-audit 1100-audit-since-tz agent-update agent-update-launch-cmd-redaction 1122-admin-auto-caller-source 1136-always-on-no agent-doctor upgrade-conflicts-lifecycle 1601-conflicts-adopt-guard managed-autocompact-window per-agent-settings-rendering 1756-settings-preserve-model-user-keys status-engine-detect 835-static-admin-launch isolated-agent-delete-reap 1121-agent-delete-os-purge 1140-purge-home-os-cleanup 1400-purge-home-degrade-no-sudo 1028-isolated-workdir-check 1118-v2-engine-binary-path v2-scaffold-home-and-workdir 1060-layout-fresh-v2-static-claude 1060-layout-fresh-v2-static-codex 1060-layout-shared-workdir-pair 1067-codex-provisioning 1115-cli-usage-drift 1151-step-a-helper 1155-bootstrap-skill-guard 1158-marker-load-order 1165-track-a-scaffold-modes 1213-iso-uid-predicate beta27-D-inject-timestamp-resolved I-agent-description-roster γ-cli-consistency C1-beta3-1251-restart-preflight-rollback A3-beta3-1248-restart-session-id-resume A12-beta3-1246-1252-daemon-supp-group-and-state-dir A-beta4-iso-path-resolution E-beta4-fresh-install-gate-state-dir K-beta4-nits α-beta5-upgrade-backfill-normalize gamma-beta5-reconcile-helper-status beta5-2-epsilon-tmux-inject-busy beta5-2-theta-upgrade-backfill-perms 1360-onboarding-next-actions-persona 1409-claude-midturn-busy-gate 1639-post-restart-auto-wake 1425-spool-rederive 1617-pending-attention-arrival-stale 1513-iso-teams-prune-eacces plugin-requires-resolver 1520-shared-claude-config-dir 1520b-create-time-creds-sync 1417-identity-sync-on-start 1638-settings-cosmetic-conflict 1675-1694-settings-homebrew-abspath-conflict 1769-freshness-gate-resume 1753-hud-config-seed
      ...
      # Issue #2002: bridge-start.sh's bridge_start_should_send_onboarding_nudge
      # now gates the fresh-install onboarding nudge on a GENUINE first launch
      # (admin + pending + REPLACE==0 + initial-inbox marker absent), not just
      # `onboarding_state == pending`. Pull 2002-onboarding-nudge-guard whenever
      # bridge-start.sh moves so a refactor cannot drop the --replace or
      # launch-history gates and re-leak the restart-re-types-onboarding bug.
      if [[ "$path" == "bridge-start.sh" ]]; then
        add_required 2002-onboarding-nudge-guard
      fi
      # Issue #2007: bridge-start.sh's static/managed Codex launch path now runs
      # the first-party hook PRETRUST (inside bridge_ensure_codex_hooks) after
      # the render and before the Codex process starts, so a hook-changing
      # upgrade never wedges a managed Codex agent at the trust gate. Pull
      # 2007-codex-hook-trust whenever bridge-start.sh moves so a refactor can't
      # drop the launch-time pretrust seam or weaken its first-party boundary.
      if [[ "$path" == "bridge-start.sh" ]]; then
        add_required 2007-codex-hook-trust
      fi
...
add_required launch launch-dev-channels-injection tmux-injection upgrade-source-preservation upgrade-shared-settings-propagate agent-create-name-validation agent-create-caller-trust-gate agent-create-idle-timeout 1105-agent-add-audit 1100-audit-since-tz agent-update agent-update-launch-cmd-redaction 1122-admin-auto-caller-source 1136-always-on-no agent-doctor upgrade-conflicts-lifecycle 1601-conflicts-adopt-guard managed-autocompact-window per-agent-settings-rendering 1756-settings-preserve-model-user-keys status-engine-detect 835-static-admin-launch isolated-agent-delete-reap 1121-agent-delete-os-purge 1140-purge-home-os-cleanup 1400-purge-home-degrade-no-sudo 1028-isolated-workdir-check 1118-v2-engine-binary-path v2-scaffold-home-and-workdir 1060-layout-fresh-v2-static-claude 1060-layout-fresh-v2-static-codex 1060-layout-shared-workdir-pair 1067-codex-provisioning 1115-cli-usage-drift 1151-step-a-helper 1155-bootstrap-skill-guard 1158-marker-load-order 1165-track-a-scaffold-modes 1213-iso-uid-predicate beta27-D-inject-timestamp-resolved I-agent-description-roster γ-cli-consistency C1-beta3-1251-restart-preflight-rollback A3-beta3-1248-restart-session-id-resume A12-beta3-1246-1252-daemon-supp-group-and-state-dir A-beta4-iso-path-resolution E-beta4-fresh-install-gate-state-dir K-beta4-nits α-beta5-upgrade-backfill-normalize gamma-beta5-reconcile-helper-status beta5-2-epsilon-tmux-inject-busy beta5-2-theta-upgrade-backfill-perms beta5-2-xi-misc-fixes 1360-onboarding-next-actions-persona 1639-post-restart-auto-wake 1425-spool-rederive 1617-pending-attention-arrival-stale 1513-iso-teams-prune-eacces plugin-requires-resolver 1520-shared-claude-config-dir 1520b-create-time-creds-sync 1638-settings-cosmetic-conflict 1675-1694-settings-homebrew-abspath-conflict 1769-freshness-gate-resume
      ...
      # Roadmap step 3 (#2217 / codex #20878): bridge-start.sh / bridge-run.sh are
      # managed-Claude launch entrypoints that must call bridge_ensure_hud_usage_tap
      # (R1). Pull the universality ratchet on every start/run move so a dropped tap
      # call site (managed agent untapped -> usage monitor blind -> proactive token
      # rotation never fires -> agent hard-caps) is caught here, not in the field.
      add_required 20878-hud-tap-universality-ratchet
      add_integration integration-minimal
      add_live live-tmux-daemon
      ;;
    ...
    hooks/tool-policy.py|hooks/askuserquestion_escalate.py|bridge-config.py|lib/system_config_paths.py)
      ...
      # Issue #19146 (SECURITY): hooks/tool-policy.py hosts the PreToolUse Bash
      # guard (_bash_git_primary_checkout_guard_reason) that blocks destructive
      # git mutations in the operator's primary checkout from a dispatched-fixer
      # (worktree-confined) session — the Bash/git counterpart of the Edit/Write
      # #341 gate. Pull the accumulated-bypass-matrix smoke whenever
      # tool-policy.py moves so a future PR cannot regress the allowlist (env
    ...
    lib/bridge-isolation*.sh|lib/bridge-migration.sh|bridge-migrate.sh|lib/bridge-marker-bootstrap.sh|lib/bridge-layout-resolver.sh|lib/upgrade-helpers/isolation-v2-migrate.sh|tests/isolation*)
      ...
      # Issue #1971: lib/bridge-isolation-v2.sh now gates the ab-* OS-group
      # CREATE path (`bridge_isolation_v2_ensure_group`) behind the same
      # platform predicate ENFORCE uses (`_bridge_isolation_v2_cred_platform_ok`),
      # and adds `bridge_isolation_v2_darwin_cleanup_inert_groups` (called from
      # lib/upgrade-helpers/isolation-v2-migrate.sh) to remove the inert macOS
      # ab-* groups a pre-#1971 install left behind. Pull
      # 1971-macos-group-create-gate on every isolation-lib + migrate-helper
      # move so a future PR cannot regress (a) the Linux-only CREATE gate
      # (re-eagering inert macOS groups, or worse gating OFF Linux provisioning),
      # or (b) the cleanup safety contract (provenance marker + operator-only
      # membership + macOS-only) that keeps it from ever removing a real/operator
      # group.
      add_required isolation isolated-bin-agb isolated-skills-sync isolated-settings-rendering 1495-settings-invalid-hook-key isolated-cli-policy v2-cross-class-read isolation-v2-migrate-lock-portability isolation-v2-migrate-macos-skip isolation-v2-marker-only-migrate isolation-v2-macos-noise-suppression isolation-v2-platform-discriminator isolation-v2-bucket2-gates 1971-macos-group-create-gate layout-resolver-marker-over-env 857-pr1-isolation-write-helper 857-pr6-isolation-v3-channel-dotenv-migrate 864-upgrade-perm-regressions 1021-isolation-v2-shared-plugin-perms 1025-isolated-create-agent-env-install 1077-migrate-iso-v2-data-dir 1113-watchdog-legacy-backfill 1158-marker-controller-uid-exemption 1158-marker-load-order 1161-marker-readable-by-isolated 1165-track-b-sudo-escalate-and-state 1342-write-state-marker-matrix launch isolated-agent-delete-reap 1121-agent-delete-os-purge 1140-purge-home-os-cleanup phase2-install-tree-reconciler phase3-agent-home-contract 1207-stale-supp-groups-allowlist A-beta4-iso-path-resolution H-beta4-iso-ownership G-beta4-watchdog-noise J-beta4-workflow-docs α-beta5-upgrade-backfill-normalize gamma-beta5-reconcile-helper-status beta5-2-theta-upgrade-backfill-perms 1506-isolate-normalize 1520c-create-isolate-profile-publish 1533-create-isolate-content-publish 1766-iso-settings-readable 1513-iso-teams-prune-eacces 1836-iso-first-start-queue-dir-ownership 1891-iso-create-path-completeness 1899-dynamic-vanilla-codex-v2-secret-env-repin
    ...
    lib/bridge-skills.sh|.claude/skills/*)
      ...
      # Issue #2020: lib/bridge-skills.sh's bridge_sync_claude_runtime_skills
      # is the runtime-skill materialization arm that reads
      # bridge_agent_skills_csv (guarded on bridge_var_is_assoc). The
      # 2020-skills-assoc-declare smoke drives that real sync end-to-end from a
      # bare roster source; pull it on every bridge-skills.sh move so a refactor
    ...
    hooks/pre-compact.py|bridge-memory.py|scripts/librarian-process-ingest.py)
      ...
      add_required 17957-disposable-no-poller
      # Issue #744: librarian-process-ingest.py now hard-rejects an agent's
      # memory/shared/ captures (agent domain content) instead of auto-
      # promoting them into the single wiki/operating-rules.md SSOT, and the
      # daily-ingest canary treats that reject (and duplicate) as
      # non-halting so a memory/shared capture in the batch[0] slot cannot
      # wedge the whole batch. Re-run the guard + canary-position regression
      # whenever this classifier/canary moves.
      add_required 744-librarian-shared-guard
      add_integration integration-minimal
      ;;
    ...
    hooks/*|bridge-hooks.py|lib/bridge-hooks.sh)
      ...
      # Issue #1990: bridge-hooks.py's `save_json` + the isolated-home renderer
      # now write through a PER-WRITER unique temp file so concurrent renders
      # (the daemon plugin-MCP-liveness restart racing a manual reseed) never
      # collide on a shared `<name>.tmp`. 1990-settings-render-concurrent-tmp
      # is the mutation-proven concurrent-render regression — pull it on every
      # bridge-hooks.py move so the shared-tmp footgun cannot silently return.
      add_required hooks upgrade-shared-settings-propagate managed-autocompact-window isolated-settings-rendering 1495-settings-invalid-hook-key 1453-channel-sticky-false-inbound per-agent-settings-rendering shared-settings-preserve-user-keys 1990-settings-render-concurrent-tmp 1689-statusline-preserve-rerender 1756-settings-preserve-model-user-keys 2216-convert-preserve-model-effort 11901-shared-global-settings-inherit 1759-selfref-global-loop-guard 1961-statusline-compose admin-hook-exemption 1067-codex-provisioning 1120-controller-ops-isolated 1139-link-shared-settings-perm 1672-link-shared-settings-idempotent 1145-ensure-dir-actually-sudo 1145-option1-deferral-guard 1151-step-a-helper 1165-track-c-hooks-and-dispatcher 1175-exhaustive-pathlib-audit 1178-helper-contract-daemon-supp 1205-hook-iso-fail-open 1212-bridge-hooks-marketplace 1213-iso-uid-predicate 1934-hook-path-canonical-fence 1934-hook-file-self-heal beta27-D-inject-timestamp-resolved beta27-E-hook-permission-fail-open-markers 1358-admin-credential-routine-exempt 1199-action-required-claimed-skip codex-precompact-hook codex-postcompact-hook codex-subagent-hooks codex-permission-request-hook 1890-dynamic-vanilla-claude 1899-dynamic-vanilla-codex
      ...
      # invariant.
      add_required 1923-askuserquestion-hard-ban
      # Issue #2007: bridge-hooks.py gained the first-party Codex hook PRETRUST
      # surface — codex_hook_identity / codex_hook_trusted_hash / the
      # _CODEX_FIRST_PARTY_HOOKS inventory / codex_first_party_trust_targets /
      # the narrow config.toml [hooks.state] patcher (codex_patch_config_trust +
      # verify-after-write) + the `ensure-codex-hook-trust` entrypoint — and
      # lib/bridge-hooks.sh gained bridge_codex_pretrust_first_party_hooks, wired
      # AFTER the render in bridge_ensure_codex_hooks / bridge_ensure_codex_agent_
      # hooks. Pull 2007-codex-hook-trust on every hooks/* / bridge-hooks.py /
      # lib/bridge-hooks.sh move so a future patch cannot regress the version-
      # invariant hash algorithm, the STRICT first-party boundary (foreign hooks
      # stay untrusted; never --dangerously-bypass-hook-trust), the enabled=false
      # + unknown-field preservation, or the fail-closed skip path.
      # Issue #1981 (SAFETY): bridge_link_claude_settings_to_shared
      # (lib/bridge-hooks.sh) gained an operator-global hijack guard that runs
      # FIRST — before #1945 F7's deferral/iso-UID render, the #1766 group
      # publish, and the link-shared-settings call — refusing to symlink the
      # operator's real `~/.claude/settings.json` when the managed workdir
      # resolves onto it (workdir == operator HOME, or the reverse direction).
      # Pull 1981-operator-global-hijack-guard on every hooks/* / bridge-hooks.*
    ...
    bridge-upgrade.sh|bridge-upgrade.py|lib/bridge-lock.sh|scripts/export-public-snapshot.sh|VERSION|LTS_SERIES)
      ...
      # Issue #1985: bridge-upgrade.py's cmd_cleanup_residue now runs the
      # operator-global settings hijack detect/repair sweep (report-only by
      # default, gated neutral repair behind --repair-operator-global-settings-
      # hijack, backup-before-mutate, os.replace replaces the symlink). Pull
      # 1985-operator-global-settings-hijack-cleanup on every bridge-upgrade.py
      # move so the narrow detection, the backup-fail-closed refusal, and the
      # report-only-never-fails-upgrade contract cannot silently regress.
      ...
      # Issue #1931 (DATA-LOSS): the migrate-agents rematerialize pass must never
      # clobber an operator-customized workdir CLAUDE.md / AGENTS.md with the
      ...
      # the universality ratchet on every upgrade move so a dropped propagation call
      # site is caught.
      add_required 20878-hud-tap-universality-ratchet
      add_integration integration-minimal
      ;;

    lib/upgrade-helpers/rematerialize-agent-identity.sh|lib/upgrade-helpers/preserve-customized-entrypoint.py|scripts/smoke/upgrade-migrate-rematerialize-workdir.sh|scripts/smoke/1636-rematerialize-scaffolding.sh|scripts/smoke/1670-rematerialize-dryrun-agent-preserved.sh|scripts/smoke/1781-doc-migration-memory-preserve.sh|scripts/smoke/1809-agents-md-backfill.sh|scripts/smoke/1931-upgrade-preserves-workdir-claudemd.sh)
      ...
      # Issue #1931 (DATA-LOSS): the helper now refuses to clobber an operator-
      # customized workdir entrypoint with the placeholder template (preserve +
      # managed-block-only refresh, the #1817 contract applied to the workdir).
      # The preserve decision lives in preserve-customized-entrypoint.py; pull the
      # #1931 smoke whenever the helper, that decision script, or any sibling
      # rematerialize smoke moves so the preserve guard stays exercised.
      add_required upgrade-migrate-rematerialize-workdir 1636-rematerialize-scaffolding 1670-rematerialize-dryrun-agent-preserved 1781-doc-migration-memory-preserve 1809-agents-md-backfill 1931-upgrade-preserves-workdir-claudemd
      add_integration integration-minimal
      ;;

    lib/upgrade-helpers/reconcile-iso-onboarding-layer.sh|scripts/smoke/2193-iso-onboarding-reconcile.sh)
      # Issue #2193: the iso-onboarding-layer reconcile helper (invoked by
      # bridge-upgrade.py:reconcile_iso_onboarding_layers) writes the iso-owned
      # workdir/home SESSION-TYPE.md copies as the agent user after migrate.
      # Pull the smoke whenever the helper or its own smoke changes so the
      # sudo-to-agent write path + idempotence + best-effort fault tolerance
      # stay exercised. The bridge-upgrade.py arm above also selects it on a
      # migrate change (the Python caller lives there).
      add_required 2193-iso-onboarding-reconcile
      ;;
    ...
    scripts/smoke/1966-orphan-walk-prune.sh|scripts/smoke/1966-orphan-walk-prune-helper.py)
      # Issue #1966: editing the orphan keep-set walk-prune smoke or its Python
      # correctness-teeth helper re-runs the smoke directly (the scripts/smoke/*
      # catch-all pulls it via the full static suite; this arm keeps the
      # selection focused when only these two files change).
      add_required 1966-orphan-walk-prune
      ;;
    ...
    lib/upgrade-helpers/codex-hooks-propagate.sh)
      # Issue #1067 S08: the upgrade-time codex hook propagate helper re-renders
      # per-agent Codex hooks. Issue #2007 extended it to also PRETRUST the
      # bridge's own first-party hooks in the sibling config.toml after each
      # render (so the hook-changing upgrade never wedges a codex agent at the
      # trust gate on the next restart). Pull both the provisioning smoke (T3
      # exercises the propagate render) and the pretrust smoke when only this
      # helper changes; the bridge-upgrade.sh arm + scripts/smoke/* catch-all
      # also select them on the broader upgrade-entry / full-static paths.
      add_required 1067-codex-provisioning 2007-codex-hook-trust
      ;;
    ...
    lib/bridge-cleanup.sh)
      ...
      # Issue #1985: bridge_cleanup_daily_backup_residue now passes the #1984
      # operator-global resolver + the repair/restore/backup-dir flags through to
      # cleanup-residue. Pull the 1985 smoke on every cleanup-lib move so the
      # report-only default, the backup-fail-closed refusal, and the neutral
      # os.replace repair cannot silently regress.
      add_required cleanup-payload-empty-stdin-872 upgrade telegram-relay-residue-cleanup 1786-tasksdb-doctor-verb 1985-operator-global-settings-hijack-cleanup
      add_integration integration-minimal
      ;;
    ...
    bridge-auth.py|bridge-auth.sh)
      ...
      # Issue #2137: bridge-auth.py/.sh now gate the keychain-free apiKeyHelper
      # path — wrapper flag-guard (--help/unknown-flag), broad-scope WRITE admin
      # exclusion, the sanctioned `keychain-free enable/disable/status` verb +
      # fail-closed preflight, and the settings-write audit. 2137-keychain-free-
      ...
      add_required 18696-keychain-free-token-kind-guard
      # Issue #18849 Part 1: bridge-auth.py/.sh now host the operator-global
      # seamless rotation step (`sync-global` PATCH writer + `global-auth-status`
      # detection surface) for dynamic-vanilla Claude. 18849-dynamic-global-auth-
      # sync pins the default-OFF double gate, PATCH-not-overwrite (refreshToken/
      # unknown preserved), root fail-closed, rollback-on-failure, idempotency,
      # and identity DETECTION-not-sync. Pull it on every auth move so a refactor
      # cannot regress any of the 7 operator-credential safety gates.
      add_required 18849-dynamic-global-auth-sync
      # Issue #18849 Part 1b-v2 (closes #2145): the displayed-identity source is
      # now the OPERATOR-PROVIDED account_email (source==operator), not a
      # user:profile probe. 18849-operator-account-email pins the add/receive/set
      # capture + format validation, the operator-source gate on the
      # ~/.claude.json write (empty/non-operator => no write), the replace
      # stale-clear, the optional verify-only probe (verify_skipped + warn-only,
      # never a source/blocker), the configured-identity status surface, and the
      # dropped user:profile scope. Pull it on every auth move.
      add_required 18849-operator-account-email
      # Issue #19460 Fix 1 PR-A: bridge-auth.py now hosts the `mark-adverse`
      # deterministic active-dead primitive (stamp a real 429/quota or 401/403
      # onto the row without a probe). 19460-mark-adverse pins the quota disable +
      # reset windows (mark-quota parity), the auth_failed adverse-stamp-only (no
      # permanent disable), the token-id/fingerprint stale-capture guard, and that
      # the cascade skips a freshly-marked candidate. Pull it on every auth move.
      add_required 19460-mark-adverse
      # Issue #18849 Part 2 PR-2: cmd_rotate now CASCADES past every unavailable
      # candidate (a recent hard limit/auth `last_check_status`, or a future
      # `limited_until`/`disabled_until` window) to the next AVAILABLE token, and
      # routes a fully-unavailable pool to the existing `all_tokens_limited`
      # refusal (which feeds the daemon's #1789 D2 latched pool-exhausted warn).
      # 18849-p2-cascade-warn pins the cascade, the exhaustion routing +
      # soonest_reset, the fail-safe (a stale adverse check never strands a
      # token), and the no-per-candidate-probe contract. Pull it on every auth
      # move so a refactor cannot regress the registry-signal availability model.
      add_required 18849-p2-cascade-warn
      # Issue #2171 (incident #19460 M4 fleet-down) PR-B1: cmd_rotate now supports
      # an opt-in `--preflight` that LIVE-probes the selected candidate (the
      # cmd_check probe) before committing the active pointer — authorize ONLY on
      # `available`, exclude quota/auth/timeout/failed/403/unknown, persist bounded
      # failure evidence (never a permanent disable), and revalidate (active id +
      # candidate fingerprint, fresh view) at commit so a raced probe is discarded.
      # 2171-rotate-live-preflight pins the available-commit, the adverse cascade +
      # one-pass bound, the all-limited refusal, the wrong-row-commit-0 revalidate,
      # the OAT-only fail-closed kind gate, the unlocked-probe lock dance, and the
      # --preflight-OFF backward-compat regression. Pull it on every auth move.
      add_required 2171-rotate-live-preflight
      # Issue #2171 PR-B2 Part1: rotate now accepts a GLOBAL `--preflight-budget`
      # that caps the summed live-probe time across the ring (per-candidate probe =
      # min(--preflight-timeout, remaining budget)); a spent budget FAILS CLOSED —
      # an unprobed candidate is excluded with the stable trace reason
      # `preflight_budget_exhausted` and never committed from stale registry
      # availability, and a fully exhausted/failed pass emits the existing
      # `skipped:all_tokens_limited` envelope (never a truncated/invalid rotate
      # JSON). 2171-rotate-preflight-budget pins the budget-expiry fail-closed, the
      # min(timeout,budget) per-candidate bound, the all_tokens_limited preservation,
      # and the daemon-parse roundtrip (no error:invalid_rotation_output). Pull it
      # on every bridge-auth move so the budget bound cannot silently regress.
      add_required 2171-rotate-preflight-budget
      # Issue #2171 (incident #19460 M4 fleet-down) PR-B3: rotation_candidate_
      # availability now also runs a token-free measured-usage prefilter — an index
      # of {token_digest -> per-window near-limit booleans} built OUTSIDE
      # registry_lock and injected into the SINGLE eligibility gate (legacy cascade
      # + preflight Step A + revalidate) so a near-limit-but-unstamped candidate is
      # skipped (reason `measured_near_limit`). Absent/stale/malformed/no-digest-
      # match fails OPEN to the registry-stamp verdict (additive; no-cache/LTS
      # no-op). 2171-prb3-measured-usage-prefilter pins the 7 plan-ok cases incl.
      # the display-fingerprint-vs-active-digest regression and the no-cache-IO-
      ...
      # Issue #2164: cmd_rotate / cmd_activate now SYNCHRONOUSLY converge the
      # operator-global ~/.claude/.credentials.json on `--sync` (via
      # _converge_operator_global_inline) so a reactive rotation (cron-runner
      # `rotate --if-auto-enabled --sync`, daemon usage monitor, or manual
      # rotate/activate --sync) does not leave a dynamic-vanilla Claude agent
      # wedged on the stale exhausted token until the ~3600s periodic tick.
      # 2164-rotate-global-sync pins the inline converge on both mutation
      # sources, the default-OFF double gate (auto_rotate AND global-auth-sync
      # opt-in), the no-`--sync` legacy no-op, and the fail-safe (a converge
      # error never rolls back the committed rotation). Pull it on every auth
      # move so a refactor cannot regress the inline-converge contract.
      add_required 2164-rotate-global-sync
      ...
      add_required 1899-dynamic-vanilla-codex
      # Issue #2217 (roadmap step 4): bridge-auth.py/.sh gained the read-only,
      # NON-SECRET `claude-token active-digest` verb (active_token_id:fingerprint)
      # that keys the daemon's reactive-429-rotation latch. 2217-reactive-429-rotate
      # exercises the rotate trigger that consumes that digest; pull it on every
    ...
    bridge-daemon-helpers.py)
      ...
      # #21895 phase-1 (sub-PR 4/4): bridge-daemon-helpers.py now hosts the
      # ``lease-status-parse`` + ``lease-heartbeat-parse`` subcommands that turn
      # the Contract-A lease `status`/`heartbeat` --json envelopes into the TSV
      # rows the daemon lease tick reads for its re-checkout decision. The
      # token-updater-daemon-tick smoke exercises both parsers end-to-end (via
      # the real helper), so pull it on every helpers move so a future PR cannot
      # regress the sentinel-`-`-for-empty column contract the daemon depends on.
      add_required token-updater-daemon-tick
    ...
    bridge-provider-health.py|lib/bridge-provider-health.sh|lib/provider-health-static-check.sh|scripts/smoke/fallback-p1a-provider-health-oracle-helper.py)
      # Issue #2066 (v0.17 Anthropic-outage fallback feature, P1a): the
      # provider-health outage oracle core (bridge-provider-health.py — state
      # machine, synthetic-probe + DNS-sanity seams, report/probe/recover), its
      # shell read accessors (lib/bridge-provider-health.sh), and the smoke's
      # DNS-guard mutation helper. Pull the non-vacuous oracle smoke on any move
      # so a refactor cannot regress scoped-on-first, the DNS guard (our-network
      # vs Anthropic), N-of-M fleet promotion, recovery hysteresis, or the
      # zero-steady-state contract.
      add_required fallback-p1a-provider-health-oracle
      add_required daemon
      ;;

    bridge-usage-probe.py|bridge-usage.sh|bridge-usage.py|scripts/hud-usage-tap.py)
      ...
      add_required 17927-p2-statusline-usage-feed
      # #18849 Part 2: bridge-usage.py extends the parse-time stale-attribution
      # guard (synthetic 429-signal caches) to REAL preemptive candidates — a
      # native-probe reading whose `_token_digest` mismatches the live
      # --active-token-digest is a post-rotation leftover and must NOT re-rotate
      # the freshly-active token (the operator-observed "rotation at ~41% usage").
      # Pin the rotation-attribution smoke on any usage-file move so a refactor
      # cannot drop the digest carry-through or the rotation-lane gate.
      add_required 18849-p2-rotation-attribution
      # Issue #2171 PR-B3 Part 2: bridge-usage.py's cmd_monitor now surfaces a
      # WRONG-HOME cache-split — a resolved claude cache whose `_token_digest`
      # mismatches the live --active-token-digest — as a `cache_split_diagnostics`
      # envelope entry (added ONLY when a split is observed, so the steady-state
      # output is byte-identical). It stays diagnostic + no-signal (the reading was
      # already inert for the rotation lane); no alternate-home search. Pull the
      # PR-B3 smoke on any usage-file move so the conditional diagnostic emit and
      # the no-misread-as-0% contract can't regress.
      add_required 2171-prb3-measured-usage-prefilter
      # Issue #2171 PR-B2 Part2 (incident #19460 M4 fleet-down): bridge-usage-probe.py
      # is the SOLE writer of the Option-B auth-dead marker (a 401 / origin-403 on the
      # active token → token-free marker; a clean read clears it) and hosts the
      # read-only `auth-dead-marker-check` fanout gate; bridge-usage.sh plumbs the
      # marker path to the probe. Pull 2171-daemon-active-dead-recover on any usage-file
      # move so a refactor cannot drop the marker write/clear, weaken the edge-403
      # exclusion, or break the fresh+id+fingerprint+digest consume contract.
      add_required 2171-daemon-active-dead-recover
      ...
      # Issue #2066 (P1a): the outage-class classifier extension lives in
      # bridge-usage-probe.py (classify_probe_http_error gained an OUTAGE bucket
      # + classify_outage_class_text). Pull the oracle smoke on any usage-probe
      # move so a refactor cannot regress the outage-vs-quota/auth discrimination
      # (a 429/401 misread as outage would falsely strand the fleet on fallback).
      add_required fallback-p1a-provider-health-oracle
      ...
      add_required weekly-warn-threshold-sanitize weekly-usage-quota-escalation
      # Roadmap step 3 (#2217 / codex #20878): the read resolver
      # (bridge_usage_resolve_claude_cache_path, R2 read) must keep the
      # plugins/claude-hud/.usage-cache.json suffix the hud-usage-tap.py write side
      # uses. Pull the universality ratchet on any bridge-usage / hud-usage-tap move
      # so a one-sided rename (tap writes where the monitor never reads) is caught.
      add_required 20878-hud-tap-universality-ratchet
      # Issue #2217 (roadmap step 5): the cross-layer decision-chain integration
      # smoke drives the REAL bridge-usage-probe.py (cache_is_fresh content-age
      # gate + the edge-blocked classifier) and bridge-usage.py monitor as the L1
      # (tap) + L2 (probe) half of the auto-rotate route, proving the sean-mac
      # blind-probe condition (stale tap + edge-blocked probe → no proactive
      # candidate) that the reactive backstop must cover. Pull it on every
      # usage-probe / usage / hud-usage-tap move so a refactor of the freshness
      # gate or the edge-vs-quota classifier that re-opened (or wrongly closed)
      # the blind condition is caught at the chain boundary.
      add_required 2217-rotate-decision-chain
      add_integration integration-minimal
      ;;
    ...
    scripts/python-helpers/launch-cmd-*.py)
      ...
      # Issue #2063: launch-cmd-base-signature.py derives a launch command's
      # stable base signature (stripping volatile resume/continue tokens) and is
      # the daemon staleness guard's comparison primitive. Pull the stale-crash-
      # report smoke on every launch-cmd helper move so a change to the
      # tokenizer / volatile-token strip set / containment predicate cannot
      # silently re-open the false crash-loop churn or start over-suppressing a
      # real same-launch crash loop.
      add_required 2063-stale-crash-report-launch-cmd-guard
      add_integration integration-minimal
      ;;
    ...
    scripts/python-helpers/cleanup-payload-renderer.py)
      ...
      # Issue #1985: the renderer also emits the operator-global settings hijack
      # block (detected/repaired/repair_failed). Pull the 1985 smoke on every
      # renderer move so the loud-warning surface stays wired.
      add_required cleanup-payload-empty-stdin-872 upgrade telegram-relay-residue-cleanup 1985-operator-global-settings-hijack-cleanup
      add_integration integration-minimal
      ;;
    ...
    bridge-lib.sh|lib/bridge-core.sh|lib/bridge-agents.sh|agent-roster.sh|agent-roster.local.example.sh|bridge-config.sh)
      ...
      add_required 1881-channel-enable-live-mcp-readiness
      # Issue #2020: agent-roster.sh declares BRIDGE_AGENT_SKILLS associative
      # (the only #1427 per-agent map previously missing the up-front declare).
      # `add_all_required_static` already pulls the smoke via the master list;
      # pin it explicitly so a master-list/catch-all coupling change cannot
      # silently drop it on an agent-roster.sh move — same class as the #1407/
      # #1627 assoc-declaration guards.
      add_required 2020-skills-assoc-declare
      ;;
    ...
    bridge-watchdog.py)
      ...
      # never catch a renderer that stamps the marker. 2062-watchdog-rendered-block
      # renders via bridge-docs.py then scans (status=ok), with a mutation control
      # that stamps the marker and proves it WOULD trip the probe. Pull it on
      # every watchdog move so a future marker-stamp regression can't slip back in.
      add_required 2062-watchdog-rendered-block-not-drift
      add_integration integration-minimal
      ;;
    ...
    bridge-docs.py)
      ...
      add_required 1813-canon-links-resolve-v2
      # Issue #1814 (DATA-LOSS): bridge-docs.py owns the MEMORY-SCHEMA SSOT
      # unify (render_shared_memory_schema_md + AGENT_SHARED_LINKS membership)
      # and the symlink-clobber guard in sync_memory_schema_from_template. Pull
      # the mutation-proven guard smoke on every bridge-docs.py move so a
      # refactor cannot reintroduce write-through-symlink canon destruction or
      # drop the canon propagation back to a per-home template fork.
      add_required 1814-memory-schema-symlink-clobber-guard
      # Issue #1815: bridge-docs.py owns AGENT_RUNTIME_REWRITE_FILES and the
      # managed-block render. Pull the ghost-ref gate so a move cannot re-add
      # the HEARTBEAT.md/CHECKLIST.md ghost references or regress the
      # ACTIVE-PREFERENCES existence-gate precedent.
      add_required 1815-managed-block-ghost-ref-gate
      # Issue #1816: bridge-docs.py owns render_agent_bridge_block (version
      # stamp, pointer-only body) and normalize_claude (unbalanced-marker guard,
      # changed-only backups). Pull the mutation-proven marker/stamp/backup
      # smoke so a move cannot drop the stamp, double-prepend on a lost END
      # marker, or reintroduce unbounded no-op backups.
      add_required 1816-managed-block-marker-stamp-backup
      # Issue #2062: render_agent_bridge_block emits the BEGIN marker as a STABLE
      # LITERAL with the version stamp on a separate in-block line (the #1816
      # stamp-on-marker shape false-flagged healthy homes in the watchdog). Pull
      # the watchdog-rendered-block smoke on every bridge-docs.py move so the
      # renderer output can never drift back to a stamped marker the consumers
      # don't match.
      add_required 2062-watchdog-rendered-block-not-drift
      add_integration integration-minimal
      ;;
    ...
    scripts/bridge-daemon-liveness.sh|bridge-watchdog-silence.py)
      ...
      # Issue #2207 (never-die Track C): the watcher now folds a bounded
      # out-of-process token-lifeline tick (recover-due + UNCONDITIONAL sync +
      # gated sync-global) into its stale-heartbeat paths so the token lifeline
      # survives a daemon-down window. Pull the Track C token-lifeline smoke on
      # every liveness-watcher move so the staleness gate, the recover→sync
      # propagation pair, the unconditional-sync periodic survival, the global
      # opt-in gate, the interval throttle, and the v1 no-rotation scope guard
      # cannot silently regress.
      add_required 1463-launchd-keepalive-singleton-thrash watchdog-silence-stderr-capture 2208-watchdog-hung-restart-escalate launch queue 1973c-liveness-recovery 2040-daemon-enabled-but-unloaded 2055-interrupted-upgrade-reenable 2207-token-lifeline-out-of-process
      add_integration integration-minimal
      ;;
    ...
    bridge-resolver.sh|bridge-resolver-policy.py|lib/bridge-prompt-resolver.sh|runtime-templates/shared/prompt-resolver-actions.json|scripts/picker-sweep.sh)
      ...
      if [[ "$path" == "scripts/picker-sweep.sh" ]]; then
        add_required 1916-picker-sweep-migrate-atomic
        # Issue #2217 (roadmap step 4): scripts/picker-sweep.sh now routes its
        # rate-limit rotation through the SHARED cross-path cooldown
        # (lib/bridge-reactive-rotate.sh) so a picker rotate and the daemon
        # reactive-429 rotate dedupe to one event. Pull the smoke on every
        # picker-sweep move so a refactor cannot drop the shared-cooldown
        # consult/stamp (re-opening the same-tick double-rotate window).
        add_required 2217-reactive-429-rotate
        # Issue #2217 (roadmap step 5): the decision-chain integration smoke's
        # scenario 6 (concurrent picker + daemon detections → rotate ONCE) rides
        # the SAME shared cooldown picker-sweep stamps. Pull it on every
    ...
    lib/bridge-reactive-rotate.sh)
      # Issue #2217 (roadmap step 4): the shared reactive-429-rotation gate —
      # cross-process lock + cooldown, the CF/edge-exclusion + transport-qualified
      # 429 gate, the rotation-eligible scope predicate, and the feature flag.
      # Both the daemon stall branch and picker-sweep route through it. Pull the
      # smoke on every move so a refactor cannot loosen the gate or break the
      # shared lock/cooldown dedup.
      add_required 2217-reactive-429-rotate
      # Issue #2217 (roadmap step 5): the cross-layer decision-chain integration
      # smoke routes scenarios 2/4/6 through this shared gate (CF/edge exclusion,
      # transport-qualified 429, shared cooldown) as the L3 leg of the auto-rotate
      # chain. Pull it on every move so a gate loosening surfaces at the chain
      # boundary too (not only in the step-4 trigger units).
      add_required 2217-rotate-decision-chain
      add_integration integration-minimal
      ;;
