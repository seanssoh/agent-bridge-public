...
select_for_path() {
  ...
  case "$path" in
    ...
    bridge-queue.py|bridge-task.sh|bridge-audit.py)
      ...
      # → wrongly requeued).
      add_required queue 1106-nudge-shell-recheck nudge-task-age-gate nudge-redundant-active-agent 1323-nudge-eligibility-recheck-twostage a2a-cross-bridge 1100-audit-since-tz 1115-cli-usage-drift J-beta4-workflow-docs K-beta4-nits beta5-2-delta-nudge-session-empty beta5-2-kappa-state-audit-reconcile 1408-daemon-alert-nudge-hygiene 1425-spool-rederive
      add_integration integration-minimal
      ;;
    ...
    bridge-usage.py|bridge-usage.sh|bridge-usage-probe.py|scripts/smoke/1437-native-usage-probe-helper.py)
      # Issue #1437 PRIMARY: the native Anthropic OAuth usage probe. The
      # probe maps the (undocumented) api/oauth/usage `utilization` fields
      # into the EXACT .usage-cache.json shape bridge-usage.py already
      # consumes (data.fiveHour/sevenDay/...). Pull 1437-native-usage-probe
      # on every move of the probe module, the usage wrapper, or the monitor
      # so a future refactor cannot silently drift the mapping contract, the
      # credential-safety invariant (token only in the Authorization header,
      # never persisted/env-exported), or the risk-mitigation gates (>=5min
      # cache, cooldown, Retry-After, scope guard, graceful degrade).
      add_required 1437-native-usage-probe
      ;;
    ...
    bridge-daemon.sh|bridge-sync.sh|bridge-watchdog.sh|bridge-cron.sh|lib/bridge-cron.sh|lib/bridge-state.sh|lib/bridge-notify.sh)
      # Issue #1437 PRIMARY: bridge-daemon.sh's process_usage_monitor now
      # documents + relies on the native-probe refresh ahead of the usage
      # monitor read (the embedded probe inside `bridge-usage.sh monitor`).
      # Pull 1437-native-usage-probe on every daemon move so the headless
      # proactive-rotation wiring cannot regress.
      add_required 1437-native-usage-probe
    ...
    bridge-cron.py|bridge-cron-runner.py|bridge-cron-scheduler.py)
      ...
      # Issue #1437 (reactive Claude OAT rotation): bridge-cron-runner.py hosts
      # maybe_reactive_rotate + the detect/rotate-first/disable/redispatch
      # chain. Pin 1437-reactive-cron-rotation whenever the runner moves so a
      # future PR cannot regress the ordering or the no-alternate no-loop case.
      add_required cron-run-artifacts-retention cron-migrate-payloads cron-mutation-audit cron-shell-runner cron-runner-schema-openai-strict cron-path-augmentation-874 queue beta5-2-eta-cron-iso-uid-preflight 1359-cron-create-iso-staging 1379-iso-cron-staging-group 1383-iso-cron-result-json-group 1437-reactive-cron-rotation
      add_integration integration-minimal
      ;;
...
add_required launch launch-dev-channels-injection tmux-injection upgrade-source-preservation upgrade-shared-settings-propagate agent-create-name-validation agent-create-caller-trust-gate agent-create-idle-timeout 1105-agent-add-audit 1100-audit-since-tz agent-update agent-update-launch-cmd-redaction 1122-admin-auto-caller-source 1136-always-on-no agent-doctor upgrade-conflicts-lifecycle managed-autocompact-window per-agent-settings-rendering status-engine-detect 835-static-admin-launch isolated-agent-delete-reap 1121-agent-delete-os-purge 1140-purge-home-os-cleanup 1028-isolated-workdir-check 1118-v2-engine-binary-path v2-scaffold-home-and-workdir 1060-layout-fresh-v2-static-claude 1060-layout-fresh-v2-static-codex 1060-layout-shared-workdir-pair 1067-codex-provisioning 1115-cli-usage-drift 1151-step-a-helper 1155-bootstrap-skill-guard 1158-marker-load-order 1165-track-a-scaffold-modes 1213-iso-uid-predicate beta27-D-inject-timestamp-resolved I-agent-description-roster γ-cli-consistency C1-beta3-1251-restart-preflight-rollback A3-beta3-1248-restart-session-id-resume A12-beta3-1246-1252-daemon-supp-group-and-state-dir A-beta4-iso-path-resolution E-beta4-fresh-install-gate-state-dir K-beta4-nits α-beta5-upgrade-backfill-normalize gamma-beta5-reconcile-helper-status beta5-2-epsilon-tmux-inject-busy beta5-2-theta-upgrade-backfill-perms beta5-2-xi-misc-fixes 1360-onboarding-next-actions-persona 1425-spool-rederive
    ...
    bridge-auth.py|bridge-auth.sh)
      ...
      # daemon_warn line.
      # Issue #1437: bridge-auth.py gained `classify-output` (reuses
      # classify_probe / QUOTA_LIMIT_MARKERS) + `mark-quota` (deterministic
      # disable), the two CLI verbs the reactive cron rotation chain calls.
      # Pin 1437-reactive-cron-rotation whenever the auth surface moves so a
      # marker/CLI-shape change is co-verified against the reactive path.
      add_required F-beta4-oauth-bootstrap daemon-periodic-token-sync 1358-admin-credential-routine-exempt 1437-reactive-cron-rotation
      add_integration integration-minimal
      ;;
