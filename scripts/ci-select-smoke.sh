...
select_for_path() {
  ...
  case "$path" in
    ...
    bridge-cron.py|bridge-cron-runner.py|bridge-cron-scheduler.py)
      ...
      # Issue #1842 (CRITICAL, 7-day production intake outage):
      # bridge-cron-runner.py's `shell_artifact_route` run-dir tamper-check now
      # permits the legitimate iso v2 setgid group-write bit (group=ab-agent-
      # <agent>) for the owning agent while still rejecting other-write, a
      # wrong/unexpected group, and a non-iso agent's group-writable dir. Pull
      # 1842-cron-tamper-iso-groupwrite on every runner move so a future PR
      # cannot re-broaden the exemption past that one expected iso group, drop
      # the other-write/wrong-group/non-iso denials, or re-introduce the
      # blanket group-write rejection that blocked every iso v2 cron.
      add_required cron-run-artifacts-retention cron-migrate-payloads cron-mutation-audit cron-shell-runner cron-runner-schema-openai-strict cron-path-augmentation-874 queue beta5-2-eta-cron-iso-uid-preflight 1359-cron-create-iso-staging 1379-iso-cron-staging-group 1383-iso-cron-result-json-group 8807-cron-backfill-coalesce 1459-cron-dispatch-recovery 1659-cron-status-walk-perf 1677-cron-summary-short-derive 1792-cron-scope-fence 1826-cron-at-naive-tz 1842-cron-tamper-iso-groupwrite
      add_integration integration-minimal
      ;;
