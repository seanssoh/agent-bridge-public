...
# Issue #1849 (sibling of #1827 / #1222): `bridge-memory.py summarize monthly`
...
if command -v timeout >/dev/null 2>&1; then
  timeout 900 "$bridge_python" "$bridge_home/bridge-memory.py" summarize monthly \
    --agent "$agent" --home "$home" --json >/dev/null 2>&1 || summ_rc=$?
elif command -v gtimeout >/dev/null 2>&1; then
  gtimeout 900 "$bridge_python" "$bridge_home/bridge-memory.py" summarize monthly \
    --agent "$agent" --home "$home" --json >/dev/null 2>&1 || summ_rc=$?
else
  "$bridge_python" "$bridge_home/bridge-memory.py" summarize monthly \
    --agent "$agent" --home "$home" --json >/dev/null 2>&1 || summ_rc=$?
fi
