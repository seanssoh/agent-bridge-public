...
test_t8_ci_select_registration() {
  ...
  # closes the pipe on first match, printf then takes SIGPIPE, and the pipeline
  # returns non-zero — a false "missing" that only triggers once the required
  # list is long enough to fill the pipe buffer before grep exits (Linux/CI;
  # macOS's shorter list hides it). The here-string has no writer in the pipeline.
  if ! grep -qF -- "$SMOKE_NAME" <<<"$req_block"; then
