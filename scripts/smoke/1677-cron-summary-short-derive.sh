...
# Build the helper line-by-line via printf — no heredoc / no here-string.
{
  ...
  printf '%s\n' ''
  printf '%s\n' '# Tooth 1c: an ABSENT summary_short KEY on a non-silent intent takes the same'
  printf '%s\n' '# derive path as null/empty — it must NOT fail the required-fields check and'
  printf '%s\n' '# drop the whole payload to the runner-error envelope. Cover both'
  printf '%s\n' '# main_session_only and forward_to_user.'
  printf '%s\n' 'p = base(delivery_intent="main_session_only")'
  printf '%s\n' 'del p["summary_short"]'
  printf '%s\n' 'r = validate_result(p)'
  printf '%s\n' 'if r.get("summary_short") != "First real digest line.":'
  printf '%s\n' '    errors.append("1c: absent key (main) not derived, got {0!r}".format(r.get("summary_short")))'
  printf '%s\n' 'if r.get("findings") != ["f1"] or r.get("actions_taken") != ["a1"]:'
  printf '%s\n' '    errors.append("1c: absent key (main) dropped the payload")'
  printf '%s\n' 'if not r.get("summary_short_normalized"):'
  printf '%s\n' '    errors.append("1c: absent key (main) missing normalization note")'
  printf '%s\n' 'p = base(delivery_intent="forward_to_user", forward_target=FT)'
  printf '%s\n' 'del p["summary_short"]'
  printf '%s\n' 'r = validate_result(p)'
  printf '%s\n' 'if r.get("summary_short") != "First real digest line.":'
  printf '%s\n' '    errors.append("1c: absent key (forward) not derived, got {0!r}".format(r.get("summary_short")))'
  printf '%s\n' 'if r.get("forward_target") != FT:'
  printf '%s\n' '    errors.append("1c: absent key (forward) dropped forward_target")'
  printf '%s\n' ''
  printf '%s\n' '# Tooth 1d: an absent summary_short key combined with an empty `summary` stays'
  printf '%s\n' '# fatal — we only seed the derive when `summary` is a usable source, so a'
  printf '%s\n' '# genuinely empty result is NOT rescued.'
  printf '%s\n' 'p = base(delivery_intent="main_session_only", summary="   ")'
  printf '%s\n' 'del p["summary_short"]'
  printf '%s\n' 'try:'
  printf '%s\n' '    validate_result(p)'
  printf '%s\n' '    errors.append("1d: absent key + empty summary should have raised ValueError")'
  printf '%s\n' 'except ValueError:'
  printf '%s\n' '    pass'
  printf '%s\n' ''
