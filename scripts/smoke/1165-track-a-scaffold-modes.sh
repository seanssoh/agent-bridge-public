...
: >"$T5_HARNESS"
{
  ...
  printf '%s\n' '    # #1175: kwargs accepts arbitrary keys (timeout=5 from the new'
  printf '%s\n' '    # safe_path_check in _isolation_aware_mkdir, plus check / capture_output /'
  printf '%s\n' '    # text from the existing sudo-mkdir path). Test does not need to'
  printf '%s\n' '    # inspect kwargs — only argv shape matters for the chgrp assertion.'
  ...
  printf '%s\n' '    # #1175: the new safe_path_check sudo `test -e` probe also reaches'
  printf '%s\n' '    # this stub. For the existence probe to report ABSENT (so the'
  printf '%s\n' '    # _isolation_aware_mkdir chgrp branch still runs and the argv'
  printf '%s\n' '    # assertion is exercised), return rc=1 with empty stderr — the'
  printf '%s\n' '    # authoritative "absent" signal that does NOT fall through to'
  printf '%s\n' '    # direct pathlib. Pre-#1175 this branch was not exercised because'
  printf '%s\n' '    # _isolation_aware_mkdir used a raw `path.exists()` directly.'
  printf '%s\n' '    if argv[:3] == ["sudo", "-n", "-u"] and len(argv) >= 5 and argv[4] == "test":'
  printf '%s\n' '        return FakeCompletedProcess(argv, returncode=1)'
  printf '%s\n' '    if argv[:3] == ["sudo", "-n", "-u"]:'
