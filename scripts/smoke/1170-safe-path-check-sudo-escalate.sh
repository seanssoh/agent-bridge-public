...
# shellcheck disable=SC2129  # per-line emit keeps footgun #11 off the table
{
  ...
  printf '%s\n' '# After #1175, the canonical `_safe_path_check` implementation lives in'
  printf '%s\n' '# `lib/bridge_iso_paths.py` and is imported into bridge-setup.py as'
  printf '%s\n' '# `_safe_path_check`. Stubs must apply on the shared module (`iso`)'
  printf '%s\n' '# because the safe-path-check function resolves `subprocess.run` and'
  printf '%s\n' '# `resolve_isolated_owner_for_path` from its own module namespace, not'
  printf '%s\n' '# bridge-setup.py s. Pre-#1175 the function was defined in bridge-setup.py'
  printf '%s\n' '# itself, so stubbing on `mod` (the bridge-setup.py module) reached the'
  printf '%s\n' '# same names; after the consolidation, stubs must target the shared module.'
  printf '%s\n' 'import sys, time, importlib.util, subprocess'
  printf '%s\n' 'from pathlib import Path'
  printf '%s\n' 'repo = sys.argv[1]'
  printf '%s\n' 'tmp = Path(sys.argv[2])'
  printf '%s\n' '# Insert repo lib/ on sys.path so the shared module is importable.'
  ...
  printf '%s\n' '# `mod._safe_path_check` is an alias of `iso.safe_path_check`. The stubs'
  printf '%s\n' '# below patch the shared module so the canonical function actually picks'
  printf '%s\n' '# them up.'
  printf '%s\n' ''
  ...
  printf '%s\n' ''
  printf '%s\n' '    #1175: stubs target the shared `iso` module because the canonical'
  printf '%s\n' '    `safe_path_check` function looks up `subprocess.run`,'
  printf '%s\n' '    `_sudo_run_as`, and `resolve_isolated_owner_for_path` in its own'
  printf '%s\n' '    module namespace. Aliases on `mod` (the bridge-setup.py module)'
  printf '%s\n' '    are bind-time references and would not be picked up.'
  printf '%s\n' '    """'
  printf '%s\n' '    sudo = SudoStub(sudo_rc if sudo_rc is not None else 0)'
  printf '%s\n' '    iso._sudo_run_as = sudo'
  printf '%s\n' '    iso.sudo_run_as_capture = sudo  # PermissionError-fallback branch uses this'
  printf '%s\n' '    # Replace subprocess.run on the shared module-level `subprocess`'
  printf '%s\n' '    # reference so the proactive direct-subprocess.run path in'
  printf '%s\n' '    # safe_path_check is captured.'
  printf '%s\n' '    subproc_stub = SubprocessRunStub(rc=subproc_rc, stderr=subproc_stderr, raises=subproc_raises)'
  printf '%s\n' '    iso.subprocess.run = subproc_stub'
  printf '%s\n' '    walker_calls = []'
  printf '%s\n' '    def walker(p):'
  printf '%s\n' '        walker_calls.append(p)'
  printf '%s\n' '        return walker_result'
  printf '%s\n' '    iso.resolve_isolated_owner_for_path = walker'
  printf '%s\n' '    iso._resolve_isolated_owner_for_path = walker  # back-compat alias too'
