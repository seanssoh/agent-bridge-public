...
- **Extract `bridge_extract_development_channels_from_command` Python heredoc to helper file — Wave A' of issue #835** (refs #835 Wave A'). PR #845 (Wave A of #835) closed the 6 launch-command builders in `lib/bridge-state.sh` but flagged in its own changelog that `lib/bridge-agents.sh::bridge_extract_development_channels_from_command` — reached via the channel-state-dirs chain when `bridge_agent_launch_cmd` rebuilds a static admin's command — still inlined an equivalent `python3 - "$command" <<'PY'` heredoc-stdin form and was upstream of the launch-cmd hot path. On Homebrew Bash 5.3.9 that bash-read form can wedge in `heredoc_write` when the wrapper is invoked inside a command substitution from an absolute-path-sourced shell — same class that closed #800 / #815 / #827 / #840 for the daemon, CLI, status, and session-id hot paths, and that Wave A of #835 closed for the launch-cmd builders. The python body moves to `scripts/python-helpers/extract-dev-channels-from-command.py`, invoked via `python3 "$BRIDGE_SCRIPT_DIR/scripts/python-helpers/extract-dev-channels-from-command.py" "$command"` instead. Behavior is preserved byte-for-byte (15-case parity test covers whitespace vs `=` forms, multi-value absorption, intra-chunk + cross-chunk dedup, ValueError shlex path, env-prefix tokens — all return the same CSV as the pre-extraction body). 14 other `python3 - <<` sites in `lib/bridge-agents.sh` (lines 831/1001/1149/1250/1378/1570/1860/1895/1973/4170/4659/4834/4940/5234) stay in place — those are cold paths or unrelated to the `bridge_agent_launch_cmd patch` wedge chain and are out of scope for Wave A'. Wave B (`agb status` tmux-without-engine detection so the surface no longer over-reports `active` on a started-but-not-yet-spawned admin) and Wave C (regression smoke fixture) stay pending — issue #835 remains open until both land.
