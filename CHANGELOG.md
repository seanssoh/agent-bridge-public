...
- **Channel agents silently lost inbound delivery when `enabledPlugins[<channel>]=false` (#1453).** A channel that was *launched* with the channel but carried a sticky-false `enabledPlugins` entry (preserved-key trap) was silently dropped, and the `--channels` alias wasn't parsed into the effective set. The settings renderer now repairs a sticky-false enable **only for a launched channel** (a genuinely operator-disabled, non-launched channel is preserved — no over-reach; correction warns to stderr, stdout stays pure JSON) and parses the `--channels` alias.
