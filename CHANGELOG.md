...
## [0.16.0-beta5] — 2026-06-04

A **beta** on `0.16.0-beta4` bundling a **seven-fix backlog wave** — portability, robustness, and Queue/daemon/A2A hardening, each codex pair-reviewed and CI-green. No new features, no schema/contract change; same `agb upgrade` path. `v0.15.4` remains the recommended **stable**.

### Fixed

- **`bootstrap-memory-system.sh` aborted under `set -euo pipefail` on an admin-less roster (#1399).** The admin-id extraction `grep | head | sed` had its no-match (empty admin) treated as a fatal pipeline failure. The grep is now wrapped `{ grep … || true; }` so a no-admin roster yields an empty admin non-fatally, while `head`/`sed` failures still surface.
- **`bridge-state.sh` read file modes BSD-first on Linux (#1402).** `_bridge_rewrite_session_id_in_file` used `stat -f '%Lp' || stat -c '%a'`; on GNU coreutils `stat -f` returns a *filesystem* (statvfs) blob, not the file mode, so the polluted value was fed to `chmod` before the atomic `mv`, dropping the file's original mode. Reordered to the canonical GNU-first `stat -c '%a' || stat -f '%Lp'` (matching the rest of the tree).
- **Shared-mode `agent delete --purge-home` + the iso plugin probe fatally `die`d without sudo on Linux (#1400).** Both paths routed through the fatal `bridge_linux_sudo_root`, which `bridge_die`s when sudo is absent — even though a shared-mode home is controller-owned (plain `rm` works) and the iso plugin probe has an ab-shared cache fallback. A new non-fatal `bridge_linux_sudo_root_best_effort` (degrades to direct invocation, mirroring the #620 precedent) now backs only those non-privileged sites; hard-escalation sites (useradd, root chown, manifest writers) are unchanged and the iso boundary is not weakened.
- **Channel agents silently lost inbound delivery when `enabledPlugins[<channel>]=false` (#1453).** A channel that was *launched* with the channel but carried a sticky-false `enabledPlugins` entry (preserved-key trap) was silently dropped, and the `--channels` alias wasn't parsed into the effective set. The settings renderer now repairs a sticky-false enable **only for a launched channel** (a genuinely operator-disabled, non-launched channel is preserved — no over-reach; the correction warning goes to stderr, leaving the rendered settings payload on stdout warning-free) and parses the `--channels` alias.
- **`BRIDGE_CRON_DISPATCH_MAX_PARALLEL` defaulted to 1 and was hard to override (#1461).** A new precedence-resolving helper (env > runtime config > host-profile default, bounded — server 3, dev/unknown 1) reaches the cron-dispatch worker-slot gate, so the override actually takes effect; documented in `OPERATIONS.md`.
- **A2A inbound handoff to a momentarily-stopped local agent 422'd instead of queueing (#1398).** An inbound cross-bridge message to a transiently-stopped local target hit the #1318 stopped-target reader guard and was rejected rather than durably queued. The inbound enqueue now passes `--force` to bypass **only** that liveness guard — the receiver's HMAC / `remote_addr` / allowlist / companion-validate / dedupe gates all remain enforced upstream (`--skip-companion-validate` is never passed).
- **macOS launchd KeepAlive thrashed against the singleton lock on out-of-band supervisor restart (#1463).** A supervisor restarting the daemon out-of-band took the singleton lock outside launchd's process tree, so launchd's KeepAlive job could never reacquire it and thrashed every `ThrottleInterval`; a secondary bug then made `status` report `stopped pid=-` falsely. The restart path is now launchd-aware (cycles launchd's **own** job via `launchctl kickstart -k`); the split guard fails closed when launchd cannot prove the live recorded pid is its supervised job; `cmd_restart` enforces the same #314 active-agent guard before the launchd fast-path (a bare `restart` with active agents is refused without `--force`); and the on-exit cleanup removes the pid-file only when it still records its own pid. Linux/non-launchd installs fall through to the existing stop+start unchanged; the singleton lock is not weakened.
