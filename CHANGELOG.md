...
- **Default-OFF is load-bearing for this cut.** The marquee capability ships reachable but inert: upgrading to v0.16.18 changes **no** fleet's token behavior until an operator explicitly runs `global-auth-sync enable`. Conservative / production installs tracking the `lts` channel advance to this head with zero rotation-behavior delta.
