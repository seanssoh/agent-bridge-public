...
### Auth / credential — token auto-rotation (M4 fleet-down core fix, #2171)

- **auth: `rotate` selected-candidate **live preflight** + daemon enablement (#2171 PR-B1 #2172 / PR-B2 Part1 #2175 / #19460).** `cmd_rotate --preflight` live-probes the *selected* rotation candidate before committing the active pointer — using the `cmd_check` split (locked snapshot → **UNLOCKED** probe → locked revalidate → commit; never probes under `registry_lock`). Authorizes **only** on a parseable JSON `available`; quota / auth / `403` / timeout / parse-unknown all fail-closed and the ring advances one bounded pass, then revalidates active-id + fingerprint + enabled + still-available before committing (a raced/replaced candidate is deterministically discarded). PR-B1 shipped the capability **opt-in (default-off)**; **PR-B2 Part1 enables it on the daemon's usage-monitor rotate** (`--preflight --preflight-budget 15 --preflight-timeout 6` under a raised 30s ceiling) — un-dormanting the live check for the fleet auto-rotation path, with a **global preflight budget** that caps summed ring-probe time and **fails closed** (budget-exhausted unprobed candidates are excluded as `preflight_budget_exhausted`, never committed on stale registry availability; a fully-exhausted pass keeps the existing `skipped:all_tokens_limited` + #1789 envelope). `--preflight` OFF = byte-identical legacy. New smokes `2171-rotate-live-preflight` + `2171-rotate-preflight-budget`.
- **auth: candidate **measured-usage prefilter** + wrong-home cache-split diagnostic (#2171 PR-B3 #2174 / #19460).** `rotation_candidate_availability` (the single eligibility gate) now also skips a candidate whose **own measured `used_percent`** (from the `.usage-cache.json` telemetry, keyed by `_token_signal_digest`) is near-limit (`measured_near_limit`), via a token-free index built **outside** `registry_lock` and injected into the predicate — complementary to the live preflight (cheap local pre-filter vs authoritative live check). Additive / fail-OPEN: absent / stale / malformed / no-digest-match → existing registry-stamp behavior (no-cache / LTS = no-op). Part 2 adds a wrong-home cache-split **diagnostic** (no-signal only; no alternate-home search). New smoke `2171-prb3-measured-usage-prefilter`.
- **auth: Claude Code **keychain-shadow self-heal** — hashed service name reconcile (#2171 PR-D #2173 / #19460).** macOS-only `agb auth claude-token reconcile-keychain` full-enumerates `Claude Code-credentials*` (base **and** the hashed `Claude Code-credentials-<hash>` variant Claude Code v2.1.195 writes) via `dump-keychain`, fingerprint-compares each against the expected active pool token, and reports a stale shadow (a divergent keychain token that makes a session ignore its rotating `.credentials.json` — the confirmed sean-m4 RCA). **Default fail-closed diagnostic** (delete nothing, non-zero exit so a health gate notices); `--apply` is the operator-approved cleanup that deletes **only** stale entries. Fingerprint-only — the raw secret never reaches stdout / the audit log. The `operator_keychain_credentials_present` sync-global guard now also sees the hashed variant. Four-round review closed a fail-open class: enumeration failure, per-entry read failure, and mixed stale+unreadable sets all now fail closed (a discovered-but-uninspectable entry never reads as clean, never mutates, exits non-zero). New smoke `2171-keychain-shadow-selfheal` (T1–T12).

## [0.17.0-beta6] — 2026-06-28 (mainline · beta · 토큰 auto-rotation 결정론적-복구 groundwork + #19146 fixer-escape 가드)

**v0.17.0-beta6 — the deterministic active-dead *primitive* for token auto-rotation, plus the fixer-escape Bash guard**, on top of beta5. The marquee is the **`claude-token mark-adverse` registry primitive (#2168 / #19460 Fix 1 PR-A)** — the **foundation** for deterministic active-dead recovery, NOT the recovery behavior itself yet. It deterministically stamps a real observed `429`/quota or `401`/`403` onto the token row **without a network probe**, so the cascade (`rotation_candidate_availability`) skips a known-dead token *before* the next selection instead of blindly re-picking it: `quota_limited` disables + stamps the reset window (`mark-quota` parity), `auth_failed` adverse-stamps only (**never permanently disabled** → the cascade skips it for the bounded adverse-freshness TTL, then fails OPEN), and a token-id + fingerprint guard drops a stale capture from a since-replaced token. **The daemon wiring that calls it — the nudge-path `active_dead_recover` (persist adverse → rotate `--sync` → converge operator-global → only then nudge, park on pool-exhaustion) — is PR-B and is NOT in this beta**; `mark-adverse` ships here as the inert, fully-tested CLI primitive that PR-B will build on. Bundled with the **#19146 PreToolUse Bash guard (#2157)** — a fail-closed DENY of destructive `git` (reset/checkout/clean/branch-D/…) escaping a dispatched-fixer worktree into the operator's primary checkout — and a README sync for the v0.16.18 LTS cut (#2166). This is a **mainline beta (prerelease)**: the **`v0.16.18` LTS stays the `lts` / Latest head**; production installs tracking `lts` are unaffected. Each PR carried its own codex / patch-dev review; the full Linux CI gate is green.

### Auth / credential — token auto-rotation (deterministic active-dead *primitive*)

- **auth: `claude-token mark-adverse` — deterministic active-dead registry primitive, no network probe (#2168 / #19460 Fix 1 PR-A).** `agb auth claude-token mark-adverse <id> --status quota_limited|auth_failed [--reset-at <iso>] [--fingerprint <fp>] [--api-error-status <s>] [--source <text>] [--json]` stamps a real observed failure onto the token row so the rotation cascade skips a known-dead token before the next selection (the M4 fleet-down root shape: rotation re-picked / blind-nudged a dead active). `quota_limited` mirrors `mark-quota` (disable + `limited_until`/`disabled_until`/`next_check_at` reset window so `recover-due` re-enables on reset); `auth_failed` adverse-stamps only (`last_check_status=auth_failed` + `last_checked_at` + a bounded `next_check_at`) and **never permanently disables**, so the candidate is skipped for the adverse-freshness window then fails OPEN. Token-id + fingerprint guarded (a stale capture from a since-replaced token is skipped as `token_replaced`, never written); fingerprint-only logging. Routed through the `bridge-auth.sh` wrapper for daemon/automation callers. New smoke `19460-mark-adverse` (6 cases incl. wrapper plumbing + cascade-skip integration). **Groundwork only** — the daemon `active_dead_recover` that consumes it is PR-B (deferred).

### Security

- **hooks: PreToolUse Bash guard — fail-closed DENY of destructive git escaping a fixer worktree into the operator's primary checkout (#19146 / #2157).** A dispatched-fixer session is confined to its own `.claude/worktrees/...` worktree, but nothing structurally stopped a `git reset` / `checkout` / `switch` / `restore` / `clean` / `branch -D` / `worktree remove` / `reflog expire` / `gc --prune` aimed at the operator's primary checkout — via `-C`, `--git-dir`, an env wrapper, a `VAR=` prefix, or a `cd` that realpath-escapes the worktree. `hooks/tool-policy.py` now **fail-closed DENIES** that shape (only the bare `git <verb>` inside the session's own worktree is permitted); operator sessions (cwd at the repo root) are structurally exempt, and the hook fails OPEN on its own crash so it stays defense-in-depth.

### Docs

- **docs: README top-block sync for the v0.16.18 LTS cut (#2166).** Current stable / LTS → v0.16.18, recommended-target + leap path bumped, CHANGELOG [0.16.18] mirrored to main so the README anchor resolves.

## [0.17.0-beta5] — 2026-06-27 (mainline · beta · 동적 토큰 auto-rotation enable + #17927 P2 + A2A/teams/discord/plugin 묶음)

**v0.17.0-beta5 — the #18849 dynamic-token auto-rotation feature is now ENABLE-able on the v0.17 line**, on top of beta4. `v0.16.17` shipped the rotation machinery (Part 1 / Part 1b) **gated default-OFF / INERT**, but the only documented opt-in — `config set-env BRIDGE_CLAUDE_GLOBAL_AUTH_SYNC=1` — was a **dead path** (the key was never on the `set-env` allowlist), so the gate could never actually be turned on. beta5 closes that gap (**#2155**) and lands the **Part 2** daemon threshold-gate + cascade, making seamless mid-session token rotation a real, operator-enableable capability — bundled with the #17927 P2 statusLine usage feed and an A2A / teams / discord / plugin reliability set. This is a **mainline beta (prerelease)**: the **`v0.16.17` LTS stays the `lts` / Latest head** and production installs tracking `lts` are unaffected — this is the v0.17 feature line. Each PR carried its own internal codex review; the full Linux CI gate is green.

### Auth / credential — dynamic token auto-rotation (#18849, now enable-able)

- **★ auth/config: a sanctioned, headless-safe enable path for global auth-sync — the dead opt-in is closed (#19140 / #2155).** Adds `auth claude-token global-auth-sync enable|disable|status [--json]` (mirrors the keychain-free verb): a **persisted, audited, daemon-inherited** runtime-config bool, so the gate is now `effective = persisted OR (BRIDGE_CLAUDE_GLOBAL_AUTH_SYNC == "1")` and the daemon follows the **effective** state. `status --json` reports `enabled / persisted_enabled / env_override_enabled / effective_source ∈ {runtime-config, env, both, default}`; `disable` clears the persisted bool but **warns** when an env override still wins (never a false full-disable). The previously-missing symmetric **`config unset-env KEY`** verb lands behind the same `resolve_config_caller` trust gate as `set-env` (denylist-first → allowlist-required, sibling-preserving, idempotent, distinct audit; exit codes mirror `set-env`). `BRIDGE_CLAUDE_GLOBAL_AUTH_SYNC` is added to the env allowlist as a strict **`flag_one`** literal (canonical `"1"`-only — `0`/`true`/empty/whitespace/control rejected), and the shared-UID caller-writable deny message is **platform-aware** (drops the iso-v2 migration suggestion off Linux). The admin-verb hook allowlist admits **only** `global-auth-sync` via a standalone validator — the token-bearing `add/activate/sync/rotate/receive` validators and the credential read-carveout are **byte-unchanged**. This is what makes #18849 actually turn-on-able.
- **auth: daemon threshold-gate + cascade rotation — #18849 Part 2 (#2147, #2148).** The Stage-2 work `v0.16.17` deferred: the daemon now **gates rotation on token attribution** (#2147) and **cascades to the next-available token with warn-on-exhaustion** when a token is spent (#2148) — the consumer side that makes the Part 1 seamless-rotation primitive useful across the fleet.
- **auth: identity-sync for global auth rotation — Part 1b / 1b-v2 (#2144, #2150 closes #2145).** Converges the displayed identity with the verified account after a rotation; #2150 adds an operator-provided `account_email` identity source so convergence no longer blocks on a `user:profile`-scoped token (the prior INERT no-op path).
- **claude-token: daemon-core statusLine usage feed — #17927 P2 (#2132).** Feeds the per-token usage/limit signal (E5/E6/E8/E10) into the daemon-core statusLine so quota state is observable.

(The Part 1 seamless-rotation writer (#2143) shipped in `v0.16.17` INERT and is carried forward on this line. The `v0.16.17` keychain-free auth-hijack hardening (#2137 / #2138 / #2141) is likewise present on the v0.17 mainline.)

### Channels

- **teams: advance the conversation reference only after confirmed delivery (#2134).** The bot's stored conversation reference (used to proactively reach a chat) is now advanced **only after a send is confirmed delivered**, so an unconfirmed / failed send no longer poisons the reference — a reliability follow-on to the #2112 reply delivery-proof.
- **discord/thread-session: one-time thread-created awareness signal to the main session (#14577 / #2133).** When a Discord thread (mapped to an agent sub-session) is first created, a single awareness signal is delivered to the main session so it knows the thread/sub-session exists — emitted exactly once, not on every message.
- **channels: suppress singleton channel plugins on a disposable `claude -p` — telegram poller self-theft (#17957 / #2128).** A disposable `claude -p` invocation no longer spins up the singleton channel plugins (e.g. the telegram poller), which had let the throwaway process **steal updates** from the real long-running channel session.
- **plugins: `marketplace.json` ↔ `plugin.json` version sync + a version-sync guard (#18177 / #2135).** The beta4 packaging slip (`marketplace.json` pinned teams `0.1.1` while `plugin.json` was `0.1.8`) made `agb plugins seed` a no-op; `marketplace.json` is bumped to `0.1.8` to match and a new **version-sync guard** prevents the two from drifting apart again.

### A2A / reliability

- **a2a: re-exec the receiver after detach to fix the macOS NAT64 `getaddrinfo` segfault / receiver crash-loop (#2140 / #2146, fleet-down).** `serve --detach` double-forked **without** `exec`, so the reconcile loop's `resolve_bind` → `getaddrinfo` ran in a fork-polluted grandchild → `SIGSEGV` on macOS NAT64 (a receiver crash-loop that took the A2A leg down on an affected host). The launcher now binds + proves the listener synchronously, then **re-execs a fresh interpreter** that adopts the proven listener instead of re-binding; the untrusted-remote surface (HMAC / source-addr / allowlist / dedupe) is unchanged and the fd adoption is fail-closed. (Same fix that shipped as the `v0.16.17` LTS backport.)
- **picker-sweep: don't misclassify the codex 'Update available' picker as a cwd-confirm (#2117 / #2136).** The picker-sweep no longer treats the codex "Update available" picker as a cwd-confirm prompt, so it is dismissed correctly instead of being answered as the wrong picker kind.

### CI / docs

- **ci: add an `lts-line-ancestry-check` gate for `release/0.16-*` drift (#2153).** A new CI gate verifies `release/0.16-*` branch ancestry so an un-verified force-push or bad merge that breaks the LTS-line ancestry is caught at PR time (the recurring branch-hygiene class).
- **docs(CLAUDE): require reading the actual codex completion note as a separate step before acting (#2154).** Codifies the hard rule that a reviewer's completion note must be read from its real `completion_note_file:` path — never inferred or pattern-matched — before recording or acting on the verdict.
...
## [0.16.20] — 2026-06-29 (LTS)

**v0.16.20 — the #2182 fleet-critical plugin-cache fresh-build race is closed on the LTS line**, the new `lts` / Latest head (supersedes `v0.16.19`). On a `v0.16.19` upgrade that bumped a channel plugin's version (teams `0.1.1→0.1.2`), an agent that did **not** already have that cached version fresh-built its per-agent plugin cache on launch — and a race partially copied the `@azure` dependency (8-11/12, non-deterministic). The required-contract check then found `node_modules/@azure/core-client/package.json` missing, aborted the launch with `criticality=channel-required`, and the verify-retry loop **reused the partial cache** instead of rebuilding it → the agent was left `stopped` (a **permanent wedge**). This is a straight cherry-pick of the `main` fix (PR #2184); the smoke (T1-T6) and the called helpers were re-verified green on the v0.16 line.

### Plugins — per-agent cache build (fleet-down hotfix, #2182)

- **plugins: atomic temp-build + winner-guarded retire for the per-agent plugin cache (#2182 / PR #2184 backport).** `bridge-dev-plugin-cache.py` no longer assembles a fresh cache **in place** (which exposed a half-built `node_modules` to a concurrent agent that then skipped it). `_atomic_build_cache_version` builds the whole tree into a pid-private temp (`.<ver>.tmp.<pid>`, same-filesystem), verifies the required contract on the temp, and only then publishes with `os.rename` — so the live path only ever transitions to a **complete** cache (the temp is cleaned on success, concurrent-winner, and failure). An **incomplete existing** cache is no longer reused: `_retire_cache_if_incomplete` re-verifies immediately before acting (never deleting a complete cache another agent has since published), retires only the exact version path via a single `os.rename` to a pid-private sibling, then rebuilds atomically — breaking the permanent verify-retry wedge. The required-contract is deliberately **not** narrowed (that would hide genuine incomplete copies); the fix guarantees a *complete* copy instead. macOS bash 5.3 + Linux bash 5.2 smoke `2182-plugin-cache-atomic-build` (T1-T6) green; carried a `main` codex `implement-ok` + green Linux CI, byte-identical (shared patch-id) to the `main` fix.

> Note: this is the mainline-mirrored copy of the v0.16.20 LTS entry (short heading); the canonical release entry lives on the `release/0.16-lts` line. The v0.16.20 release itself (VERSION + tag) shipped from `release/0.16-lts` (merge `25b8f70d`), not `main`.

## [0.16.19] — 2026-06-28 (LTS)

**v0.16.19 — the #2171 / #19460 M4 fleet-down rotation regression is closed on the LTS line**, the new `lts` / Latest head (supersedes `v0.16.18`). The M4 incident: reactive Claude-token rotation re-picked a stale/limited candidate when the usage probe degraded (403 / edge-blocked), and the whole fleet went dark. This backports the **core** of the hybrid fix (the same four PRs shipped on `v0.17.0-beta7`) — the daemon now live-probes a rotation candidate before committing it, prefers a measured-fresh candidate, and stops a macOS keychain shadow from silently re-authing onto a dead personal token. **The remaining PR-B2 Part2 (daemon `active_dead_recover` for the 401/403 auth-dead marker + blind-nudge suppression) is a tracked fast-follow.** Each PR carried a patch-dev `implement-ok` + green Linux CI on mainline; this LTS backport re-verified all four M4 smokes green on the v0.16 line, plus a small classifier backport (below) the cherry-picks needed.

### Auth / credential — token auto-rotation (M4 fleet-down core fix, #2171)

- **auth: `rotate` selected-candidate live preflight + daemon enablement (#2171 PR-B1 / PR-B2 Part1 / #19460).** `cmd_rotate --preflight` live-probes the *selected* candidate before committing the active pointer (locked snapshot → UNLOCKED probe → locked revalidate → commit; never under `registry_lock`); authorizes only on a parseable JSON `available`, fail-closed on quota / auth / 403 / timeout / parse-unknown, bounded one ring pass, revalidate-at-commit. PR-B2 Part1 enables it on the daemon's usage-monitor rotate with a **global preflight budget** (caps summed ring-probe time, fail-closed on exhaustion → existing `skipped:all_tokens_limited` + #1789 envelope). `--preflight` OFF = byte-identical legacy.
- **auth: candidate measured-usage prefilter + wrong-home cache-split diagnostic (#2171 PR-B3 / #19460).** The single eligibility gate also skips a candidate whose own measured `used_percent` (keyed by `_token_signal_digest`) is near-limit (`measured_near_limit`), via a token-free index built outside `registry_lock`. Additive / fail-open (absent / stale / malformed / no-match → existing registry-stamp behavior). Part 2 cache-split diagnostic only.
- **auth: Claude Code keychain-shadow self-heal — hashed service name reconcile (#2171 PR-D / #19460).** macOS-only `reconcile-keychain` full-enumerates `Claude Code-credentials*` (base + hashed variant), fingerprint-compares against the active pool token, reports a stale shadow. Default fail-closed diagnostic; `--apply` is operator-gated. Four-round review closed a fail-open class (enumeration / per-entry read / mixed unreadable all fail closed). Fingerprint-only.
- **auth: backport `classify_token_kind` / `active_registry_token_kind` to the v0.16 line (#18696 / #2141 helpers).** The PR-B1 live-preflight gates OAT-native probing on the token kind; the classifier shipped on mainline (#2141) but was not on the v0.16 line, so the cherry-picked preflight needed it. Two self-contained prefix-classifier helpers + their kind constants, no new dependencies.

> Note: this is the mainline-mirrored copy of the v0.16.19 LTS entry (short heading); the canonical release entry lives on the `release/0.16-lts` line. The v0.16.19 release itself (VERSION + tag) shipped from `release/0.16-lts` (merge `d7c7f799`), not `main`.

## [0.16.18] — 2026-06-27 (LTS)

**v0.16.18 — token auto-rotation becomes *enable-able* on the v0.16 LTS line (the fleet-delivery cut).** The operator fleet is pinned to the `lts` channel, so this backport is the **only path that delivers seamless token rotation to the fleet at all** — v0.16.17 carried the Stage-1 machinery but the actual enable path was dead (INERT, no persisted operator switch) and Part 2 (the daemon-driven rotation) was absent. This cut closes that deployability gap: it backports the **`global-auth-sync` enable verb (#19140)** — a first-class, persisted, headless-safe operator switch for the operator-global seamless credential propagation (so a rotated token reaches a dynamic-vanilla Claude agent without a restart) — plus **#18849 Part 2 (daemon threshold-gate + cascade)** so the daemon actually rotates an `auto_rotate`-enabled token before exhaustion. **It is backported DEFAULT-OFF for fresh installs via two independent gates: the daemon registry rotation (threshold/cascade) runs only for a token whose per-token `auto_rotate` is enabled (off by default), and the operator-global seamless propagation is additionally gated on `global-auth-sync` — so a fresh fleet that has enabled neither sees exactly zero rotation and zero live token-behavior change. (An install that had already enabled `auto_rotate` will see registry rotation on that pre-existing opt-in; `global-auth-sync enable` governs only whether a rotated token seamlessly propagates to the operator-global creds.)** Bundled with the **#19146 PreToolUse Bash guard** (security class — structurally blocks a dispatched-fixer from escaping its worktree into the operator's primary checkout with a destructive git command). This supersedes `v0.16.17` as the v0.16.x line head; the `lts` / `stable` / Latest markers advance here, and because the new rotation is default-OFF the upgrade is safe for every existing fleet install. Each bundled lane carried its own codex / patch-dev adversarial review, sits as a verified descendant of `v0.16.17`, and the full Linux CI gate is green.

### Auth — fleet token rotation (Stage 2, backported DEFAULT-OFF)

- **auth: first-class `global-auth-sync enable|disable|status` verb — close the dead enable path (#19140 / #2155 backport, marquee).** v0.16.17 shipped the Stage-1 seamless-rotation writer but left it reachable only through the `BRIDGE_CLAUDE_GLOBAL_AUTH_SYNC` env gate + an `auto_rotate` config flag — there was no operator-facing, persisted way to turn it on, so on the LTS line the feature was effectively un-deployable. `agb auth claude-token global-auth-sync enable|disable|status` now does a persisted, atomic gate write (headless-safe, no longer dependent on a live `set-env` allowlist), so an operator can actually opt a fleet into the seamless propagation of a rotated credential. This is the operator-facing switch for the operator-global seamless credential propagation (so a rotated token reaches a dynamic-vanilla Claude agent without a restart); the daemon registry rotation itself is gated separately, per-token, on `auto_rotate`.
- **auth/daemon: Part 2 — daemon threshold-gate + cascade-to-next-available + warn-on-exhaustion (#18849 Part 2 / #2147 / #2148 / #17927-P2 backport).** Backports the daemon-driven half of the rotation design: the daemon proactively rotates the active credential once usage crosses the **95% threshold** (so the swap happens before a hard exhaustion stalls the session), **cascades to the next available token** when the current one is spent, and surfaces a **warn-on-exhaustion** signal when the pool has nothing left to rotate to. **Backported DEFAULT-OFF for fresh installs** — the daemon attempts a rotation only for a token whose per-token registry `auto_rotate` is enabled (off by default), so a fresh fleet sees no threshold gating, no cascade, and no behavior change. (`global-auth-sync enable` is the separate gate governing whether a rotated token seamlessly propagates to the operator-global creds — not whether the rotation itself runs.) Pairs the Part-1 dirfd/`flock` writer hardening already on the LTS line.
- **auth: operator-email identity source for Part 1b — converge LTS identity-sync from probe-inert to operator-email (#2150 / Part 1b-v2).** The Stage-1 identity-sync follow-up converged the displayed identity with the verified account, but on the LTS line it was a probe-only no-op pending a `user:profile`-scoped token. This switches the LTS identity source to the **operator-email identity**, so a rotation converges the displayed identity without depending on the un-backported profile scope. Still gated under the same default-OFF rotation switch.

### Security

- **hooks: PreToolUse Bash guard — fail-closed DENY of destructive git escaping a fixer worktree into the operator's primary checkout (#19146 / #2157 backport).** A dispatched-fixer session is meant to be confined to its own `.claude/worktrees/...` worktree, but nothing structurally stopped a `git reset` / `checkout` / `switch` / `restore` / `clean` / `branch -D` / `worktree remove` / `reflog expire` / `gc --prune` aimed at the operator's primary checkout — via `-C`, `--git-dir`, an env wrapper, a `VAR=` prefix, or a `cd` that realpath-escapes the worktree — which is how WIP loss / branch-state collisions have happened. `hooks/tool-policy.py` now **fail-closed DENIES** that shape (allowlist: only the bare `git <verb>` inside the session's own worktree is permitted); operator sessions (cwd at the repo root, not under `.claude/worktrees`) are structurally exempt, and the hook fails OPEN on its own crash so it stays defense-in-depth. Structural prevention of the incident class.

### Notes

- **Default-OFF is load-bearing for this cut.** The marquee capability ships reachable but inert behind two independent gates: the daemon registry rotation is gated per-token on `auto_rotate` (off by default), and the operator-global seamless propagation is additionally gated on `global-auth-sync`. Upgrading to v0.16.18 changes **no** fresh fleet's token behavior; an install that had already enabled `auto_rotate` keeps rotating on that pre-existing opt-in, and `global-auth-sync enable` governs only the seamless propagation of a rotated token. Conservative / production installs tracking the `lts` channel with `auto_rotate` off advance to this head with zero rotation-behavior delta.
- **Known follow-up #2164 (non-blocking, scoped to the post-opt-in path).** Once `global-auth-sync` is enabled, a daemon rotation does not yet *synchronously* converge the operator-global creds — a quality gap in the seamless-propagation timing for opted-in installs, not a default-off risk for fresh / un-enabled fleets.
- Ancestry verified as a descendant of `v0.16.17`; each lane (#2155 / #2160 verb, #2147 / #2148 / #2162 Part 2, #2150, #2157 / #2161 guard) passed its codex / patch-dev gate, and the full Linux CI gate is green on the assembled head.

## [0.16.17] — 2026-06-27 (LTS)

**v0.16.17 — promotion of the `v0.16.17-rc1` security line to an official LTS release, bundled with the Stage-1 auth-rotation + A2A fleet-reliability fixes that landed during the rc1 soak.** This makes the **#2137 keychain-free auth-hijack root-fix** official on the v0.16.x LTS line (soaked as rc1 since 2026-06-26), and folds in a fleet-down A2A receiver crash-fix (#2146) plus the **Stage-1 (Part 1) seamless token-rotation machinery (#18849)**, which ships **gated default-OFF / INERT** so it is safe to land on the LTS line without changing live behavior. Part 2 of the auth-rotation work (daemon threshold-gate + cascade) is **Stage 2 and is NOT in this cut**. This supersedes `v0.16.17-rc1` as the v0.16.x line head; conservative / production installs tracking the `lts` channel advance here. Each bundled PR carried its own codex / patch-dev adversarial review; the full Linux CI gate is green.

### Security

- **auth: scope-guard the keychain-free backfill + fail-closed `keychain-free enable` — promoted to official (#2137 / #2138).** The single-fix security patch that shipped as `v0.16.17-rc1` is promoted to the official `v0.16.17` LTS release after the sean-mac soak + joint-test. Every broad-scope keychain-free write (`sync` / `backfill-settings` / daemon backfill) now consults `bridge_auth_apikeyhelper_allowed_for` and **excludes the admin / attached-interactive agent** from the managed `apiKeyHelper` (so a live admin can't be silently moved onto API-key billing → an "Invalid API key" break) while still syncing its OAuth credential; the admin opts in only via an explicit `--agents <admin>`. `--help` no longer mutates, unknown flags fail closed, and a sanctioned `claude-token keychain-free enable|disable|status` verb does an atomic gate write with a fail-closed enable-time preflight + a settings-write audit row. Acceptance smoke `2137-keychain-free-scope-guard` (A1–A8); `1855-keychain-free-backfill` no-regression. (No change from the rc1 base — promotion only.)

### A2A / fleet reliability

- **a2a: re-exec the receiver after detach to fix the macOS NAT64 `getaddrinfo` segfault / receiver crash-loop (#2146 / #2140, fleet-down).** `serve --detach` double-forked **without** `exec`, so the reconcile loop's `resolve_bind` → `getaddrinfo` ran in a fork-polluted grandchild → `SIGSEGV` on macOS NAT64 (a receiver crash-loop that took the A2A leg down on an affected host). The launcher now binds + proves the listener synchronously, then **re-execs a fresh interpreter** (`--already-detached` + `--inherited-listen-fd`) that adopts the proven listener instead of re-binding — only interpreter provenance changed; the per-tick bind re-prove still runs every reconcile. The untrusted-remote surface (HMAC / source-addr / allowlist / dedupe) is unchanged and the fd adoption is fail-closed. New smoke `2140-handoffd-detach-reexec` (passes here, fails against the pre-fix base — a real discriminator). (patch-dev A2A-bar adversarial review: implement-ok.)

### Auth (Stage 1 — gated default-OFF / INERT)

- **auth: gated file-based seamless token rotation for dynamic-vanilla Claude — Stage 1 / Part 1 (#18849 / #2143).** Lands the machinery for a seamless mid-session credential rotation (rotate-while-running → one prompt picks up the new token, no restart) for a dynamic-vanilla Claude agent on the operator-global `~/.claude.json`. **Double-gated default-OFF** (requires BOTH `auto_rotate` AND `BRIDGE_CLAUDE_GLOBAL_AUTH_SYNC`); the writer is a dirfd-pinned no-follow + `flock`'d PATCH (preserves `refreshToken` / unknown keys), root fail-closed, invalid-JSON / no-oauth fail-closed, fingerprint-only output, identity detection-only. Three rounds of codex security review each caught + closed a real TOCTOU (`.lock`-through-symlink, parent-swap-after-lock, lock-dir≠write-dir) → a single pinned `dir_fd` threaded through the whole read→check→write; smoke 14/14. Real auth is unaffected while the gate is off.
- **auth: identity-sync for global auth rotation — Stage 1 / Part 1b (#18849 / #2144).** The identity-sync follow-up that converges the displayed identity with the verified account after a rotation, shipped **INERT** (operator-approved dark-launch, default-OFF). Convergence is gated on a `user:profile`-scoped token (tracked by #2145); until that scope is present it is a documented, diagnosable no-op (the `no_scope` reason is surfaced in `doctor` / `global-auth-status`), and the fail-safe path was canary-verified (a `403 no_scope` → no write, no corruption). The identity writer reuses the Part 1 dirfd hardening; an r2 token-replace race was closed by recheck + persist + `~/.claude.json` write all under a single held `registry_lock`.
- **Note:** Part 2 of #18849 (daemon threshold-gate + cascade rotation) is **Stage 2 and is not in this cut.**
...
### Daemon / launchd reliability

- **Recover an enabled-but-unloaded launchd/systemd daemon (#2040).** A daemon that exited graceful-TERM with its launchd job left **enabled-but-unloaded** (KeepAlive moot) stayed silently down for hours — outside the #2030 sleep-wake and #1563 in-tick-wedge paths that rc2 covered. **Part A** hardens the upgrade restore (`_bridge_upgrade_launchd_restart_daemon`): poll-until-not-loaded before `bootstrap` (defeats the async-`bootout` race), retry the transient launchd errors, capture stderr, **verify the job actually loaded**, and on failure emit a loud non-swallowed WARN + exact remediation + a `load_state` in the upgrade summary (systemd parity verifies `is-active` for the daemon service + liveness timer). **Part B** gives the independent `bridge-daemon-liveness` watcher a standing recovery: when the daemon is gone AND the job is **enabled-but-unloaded**, it re-bootstraps it (cooldown-gated + audited) — tri-state **fail-closed** on a `disabled` job so it never fights an operator `agb daemon stop`. launchd + `systemctl --user` parity. (cm-prod `patch` adversarial review: implement-ok-for-scope; user-scope systemd confirmed.)
- **Refuse an admin self-restart (split-brain guard) (#2051).** When the agent *issuing* a restart was also the target (an admin restarting its own session), the controller died mid-flight — the kill/launch ran inside the process being killed — so the kill→confirm-dead→launch handoff had no surviving supervisor and could leave **two live instances of one identity** (double-claimed tasks, conflicting state writes, the `ensure_singleton ec=1` restart-storm bursts seen in the #2040 RCA). `run_restart` now refuses the caller-identity self-restart (`BRIDGE_AGENT_ID == target`) with a redirect to the configured `restart_peer` (new `declare -Ag BRIDGE_AGENT_RESTART_PEER` roster map) or a "restart manually" fallback, and exits non-zero before any kill. This is scoped strictly to the caller-identity case: restarting a *different* agent is fully unchanged, and the #1853 detached-survival path for the TMUX-session-only self-restart (where `BRIDGE_AGENT_ID` is unset) is preserved. A foot-gun guard, not a security boundary.

### Picker / cron / session

- **picker-sweep cron registers on macOS/non-iso instead of failing every upgrade (#2041, #2042).** The default picker-sweep registration unconditionally used `--kind shell --run-as-agent`, which a non-iso/macOS install can't resolve (iso v2 is Linux-only) → silent failure + a `failed` line re-logged on every upgrade. Registration is now platform-aware (reusing the exact `bridge_cron_validate_shell_run_config` gate): non-iso converges to a working text-kind job (idempotent, no perpetual re-fail), iso/Linux keeps shell-kind unchanged, and the cron-create stderr is no longer swallowed.
- **picker auto-resolve no longer flags a roster-claude ACTIVE assistant turn as unknown_stuck (#2043).** The `unknown_stuck` detector escalated an admin claude agent mid-turn (its live "Working" output mistaken for a stuck picker). It now reuses the #1409 active-work signal to exclude a live claude assistant turn; a genuinely-stuck unknown picker (stable, no live banner) still escalates.
- **self/loopback task-create wakes in seconds, not minutes (#2045, #2046).** A `task create --from X --to X` (a Discord thread sub-session enqueuing to its own main session) got no create-time wake push (a `from!=to` guard suppressed it) and no fresh-arrival marker (only the A2A receiver posted it) → ~6-minute periodic-nudge latency. The create-time push now fires for loopback (disambiguated from a self-nudge-mid-turn by the existing target-session busy gate — a busy target still spools), and the local enqueue posts the fresh-arrival marker.
- **Producer-only sub-sessions skip the consumer inbox-auto-drain Stop hook (#2047).** A producer-only leg (enqueues to its parent, no queue consumption, `agb` transport-guarded) that shared the main's `CLAUDE_CONFIG_DIR` inherited the consumer Stop hooks and looped demanding drains it couldn't perform. `compute_drain_decision` now honors a `BRIDGE_AGENT_LEG` producer marker and skips the drain (a real consumer is never mis-skipped).

### Channels / auth

- **ms365 cross-process refresh_token lock — ends the recurring ~3h M365 re-auth (#2048).** Root-caused (joint crm-dev + cm-prod `patch` runtime RCA) as a cross-process `refresh_token` double-consume: the ms365 plugin's in-process `SingleFlight` doesn't serialize across the MCP server, the `get-valid-token` CLI, and an external grant actor, so two processes rotate the same single-use AAD RT → `invalid_grant` → forced re-auth. The refresh grant is now wrapped in a cross-process lock keyed on `tokenPath(upn)` with **re-read-after-acquire** (skip the redundant grant if another holder already rotated a fresh token), bounded acquisition timeout + stale-lock reclaim, no token material in the lockfile, portable (`bun` `O_EXCL`, not `flock(1)`). Concurrent-refresh smoke proves exactly one grant / zero `invalid_grant`. Pairs with the cosmax-crm hook delegating to `get-valid-token` instead of its own grant.

### Doc-backfill

- **engine-detection trusts roster authority over filesystem codex residue (#2044).** The codex-AGENTS hygiene pass re-flagged 6 roster-declared **claude** agents as `detected=codex` (stale `AGENTS.md` residue), regenerating a hygiene task every pass (4+ recurrences). A roster-authoritative non-codex agent now takes a `hold-quiet` path (no recurring task; residue flagged report-only), idempotent across passes; real codex agents still detected. Distinct from #2016's emission gate.

### Managed-block data-loss + upgrade / A2A onboarding / daemon safety (landed after the original rc3 assembly)

- **Managed-block hardening — fleet-wide symlink-clobber data-loss guard + SSOT unify + ghost-ref gate + pointer-block completion (#2062 / #1814 #1815 #1816, DATA-LOSS).** The renderer's `sync_memory_schema_from_template` did `target.write_bytes(template_bytes)`, which **followed a `MEMORY-SCHEMA.md -> canon` symlink and would overwrite the canonical doc with the smaller template fork — fleet-wide, in a single apply pass**. `#1814` adds the data-loss fix (refuse to write through a symlink, with a TOCTOU re-check) and unifies COMMON-INSTRUCTIONS + MEMORY-SCHEMA onto their `docs/agent-runtime/*` canon body via a shared `_render_canon_doc` primitive (render-to-`shared/` + per-home symlink, retiring the divergent per-home template fork). `#1815` drops ghost references (the `HEARTBEAT.md` bullet present in 0/N homes, the dead `CHECKLIST.md`) and existence-gates `references/`. `#1816` cuts the remaining `CLAUDE.md` hardcopy down to canon pointers single-sourced from the engine render, version-stamps the `BEGIN` marker so a stale/foreign block is mechanically detectable (back-compat: `MANAGED_START_RE` matches stamped + unstamped), guards an unbalanced (orphaned-`BEGIN`) block (skip + report, never double-prepend), and makes backups changed-only. Three new mutation-proven regression smokes; end-to-end `apply` verified on throwaway homes.
- **Re-enable the daemon job if an upgrade is interrupted between quiesce-disable and restore-enable (#2064 / #2055).** The #2040 liveness watcher fail-closed **skips a *disabled* launchd job** (never fight an operator `agb daemon stop`). But the launchd upgrade quiesce `disable`+`bootout`s the job *before* the reconcile and re-`enable`+`bootstrap`s it *after* — so an upgrade **killed** (SIGKILL / power-loss) mid-window leaves the job **disabled+unloaded**, indistinguishable from an operator stop, and the watcher correctly-but-unhelpfully leaves the daemon down until the next manual start. A durable `state/upgrade/daemon-quiesce.intent` marker (atomic tmp+mv, records the upgrade pid + platform) brackets the quiesce window and is the discriminator the watcher lacked: marker + **dead** pid → interrupted → re-enable + recover; marker + **live** pid → in flight → defer; **no marker** + disabled → operator stop → **stay down** (#2040 contract preserved). Two self-heal layers (the upgrade EXIT handler for a catchable abort + the marker for the uncatchable crash). Also fixes a pre-existing latent errexit bug: under `set -euo pipefail` the bare reconcile-driver aborted on every non-zero rc, making the #1820 fail-closed `case` unreachable for the benign rc=2 "nothing to do" — captured via `|| _reconcile_rc=$?` so rc 0/2 proceed + restore, rc 1/3 fail closed. 13-scenario mutation-proven smoke; #2040/#655/#1905/#1820 smokes stay green.
- **Retire a stale crash/report.env on a launch-cmd change — stop the false crash-loop alarm on a healthy agent (#2065 / #2063).** A leftover `runtime/crash/report.env` from a prior launch era kept driving false crash-loop alarms (~200×/12h `crash_notified_origin_suppressed` + a spurious "Crash loop detected" urgent) on a **healthy** agent after its launch command changed (dynamic→static convert, `reclassify`, `update --set-launch-cmd`). The daemon now, before alarming, compares the recorded launch base against the agent's **current** roster base (`CRASH_LAUNCH_CMD_RAW` via the side-effect-free `bridge_agent_launch_cmd_raw` + a `shlex`-tokenize signature helper, no `eval`): on a positive mismatch it auto-retires the report + emits one `crash_report_retired_stale_launch_cmd` audit; a genuine ongoing crash rewrites the report with the current base so a matching base still alarms. Fail-safe bias — empty/unparseable/cross-engine/degenerate signals → incomparable → **keep alarming** (full `CRASH_*` family sanitized before sourcing, so a truncated report can't inherit a prior iteration's launch cmd). Part 2: `ack-crash` retires a valid report when the agent is healthy + a new `clear-crash` verb. 8-case mutation-proven smoke (incl. cross-iteration-leak teeth).
- **Structural live-leak guard — refuse to tick the LIVE runtime from a transient source checkout (#68).** A wave-orchestration fixer / CI checkout isolates *files* (a git worktree) but inherits the controller env, so `BRIDGE_HOME` still points at the operator's live `~/.agent-bridge`; a fixer that ran an individual daemon smoke / `bridge-daemon.sh run` directly then ticked the live runtime from a `/private/tmp` or `.claude/worktrees` scratch tree, and on cleanup of that tree the live daemon's `BRIDGE_SCRIPT_DIR` dangled (the #946 cascade) — the operator's daemon went down. A fail-closed guard now refuses a state-mutating daemon verb (`start`/`ensure`/`run`/`restart`/`sync`/`run-cron-worker`) when the source checkout is transient (mktemp / system-tmp / fixer worktree, incl. the managed `~/.agent-bridge/worktrees` root and the macOS `/private/var` canonicalization) AND `BRIDGE_HOME` is a persistent install — **before any live dir is created**. Help forms (`<verb> --help`) stay read-only (no refuse, no mkdir); normal dev/prod, upgrade self-respawn, and fully-isolated smokes all pass; explicit escape hatch `BRIDGE_ALLOW_FOREIGN_CHECKOUT=1`. (4-round codex adversarial review + reproduced/verified on a Linux container.)
- **A2A auto-join: self-heal a stale token-bootstrapped local peer + a stale-invite diagnosis instead of an opaque `bad_signature` (#2076 / #2073).** A joiner whose first contact failed while receiver autojoin was off had already written a local leader peer from that now-stale invite; a freshly-minted invite then skipped bootstrap and signed with the stale secret → an opaque `401 bad_signature` that read like a crypto/secret-mismatch and stalled onboarding. The joiner now refreshes the stale token-bootstrapped local peer from a new signed invite (bounded to the explicit new-invite path), and unknown/expired/revoked-token receiver rejects collapse to a single `stale_or_unknown_invite` remediation ("ask the room leader for a fresh invite") — no opaque `bad_signature`, no oracle (genuine tampering still rejects as `bad_signature`).
- **A2A receiver sources `agent-env.local.sh` before spawning (#2075).** The cross-bridge receiver spawn did not load the agent's local env overrides, so per-agent A2A config set there was missing on the receiver path; it is now sourced before the spawn.
- **Daemon blocked-aging reminders are exempt from the `[unclaimed-task]` admin escalation (#2070 / #2067).** Daemon-generated `[blocked-aging]` reminder rows (created_by=daemon, assigned to the owning agent) aged past the unclaimed threshold and each escalated to admin → N blocked tasks produced N×N auto-generated, non-actionable admin escalations. The unclaimed scanner now skips that daemon reminder class (keyed on BOTH `created_by=='daemon'` AND the `[blocked-aging] task #` title prefix, so a genuine work task with that literal still escalates).
- **CI: deflake `1563-pr7-tick-cadence` H1/H2/H4 (#2078).** The cadence smoke tested gating with `printf … | grep -q`; `grep -q` closes the pipe on first match so `printf` dies with SIGPIPE (141) and under `pipefail` the pipeline returns 141 even on a match → an intermittent false `[FAIL]` on Linux that turned `main` red on an unrelated merge. Rewritten to pure-bash matching (no pipe → no SIGPIPE), deterministic on every runner.

### A2A cross-server routing + DNS-hostname keying (operator-mandated onto the LTS line for the live multi-server rollout)

- **Receiver-enforced cross-server admin↔admin A2A routing authz (#2079).** On a multi-server mesh, each server's admin agent (e.g. `patch`) must be callable cross-server **only** by another server's admin — a non-admin remote agent must not reach an admin endpoint on a different node. The receiver now enforces a **symmetric admin↔admin** room-delivery predicate: if either endpoint is a configured admin, BOTH must be. The admin signal is a **tri-state `bridge_admin`** bit derived from each node's *configured admin id* (`BRIDGE_ADMIN_AGENT_ID`, **not** `name=='patch'`), with `unknown` never silently downgraded to non-admin; every admin-authz reject collapses to a **single generic 403** (reason audit-only — no oracle), and the sender-side UX guard refuses a bare ambiguous `--to` before any POST. A fail-open was closed in the same change: the receiver **launch** paths (the systemd handoffd unit, `agb a2a daemon start`, `bridge-handoff-daemon.sh start`) did not export the admin id, so a local admin classified `unknown` and `non-admin@remote → admin@local` failed OPEN; the admin id is now **resolved from the on-disk roster on every launch path** before serving via a zero-execution **pure-parse** of the roster assignment (a bash-eval pivot was rejected as an RCE surface). (patch-dev adversarial review: implement-ok after the fail-open + RCE-avoidance rounds.)
- **A2A DNS-hostname peer keying (#16247).** A 4th peer-identity leg — a peer/`listen` may key on a fixed DNS **`hostname`** (e.g. a Cloudflare-managed `<handle>.cosmaxcf.com`) that resolves via the host resolver to that node's CURRENT IP, so the live fleet can move off raw-IP keying and auto-track IP changes. Precedence **`node_id` > `tailscale_name` > `hostname` > literal `address`**; transport-agnostic (works on `tailscale` / `cloudflare-warp-mesh` / `trusted-routed` alike). Like the Tailscale identity legs it is an **identity, never a trusted address**: fail-closed (an unresolvable / empty-resolve / malformed — incl. a dot-only — name hard-errors, never falling back to `address`), **bounded** (a daemon-thread `getaddrinfo` + join-timeout so untrusted inbound can't block the receiver), and cached ~45s (a transient DNS blip inside the freshness window is absorbed; expired+unresolvable fails closed; a short negative cache stops resolver hammering before each 403). The receiver **source-address check** (all four inbound sites) now resolves the authenticated peer's hostname to its **full** current A/AAAA set and accepts **membership** (multi-homed / round-robin safe; IPv4-mapped IPv6 normalized) rather than a single-string compare. (Design codex-confirmed; patch-dev adversarial review caught + closed a dot-only-fallback bug → implement-ok.)

## [0.16.16-rc2] — 2026-06-21 (LTS · release candidate)

**v0.16.16-rc2 — the second release candidate on the v0.16.16 LTS line: it carries everything in rc1 plus the full batch of issues/PRs that arrived during the rc1 soak window (35 PRs), re-cut to also fold in 5 more that landed before any install adopted the tag (40 PRs total).** Like rc1 this is a soak vehicle, not a promotion — the `v0.16.15` tag stays the `lts` / Latest head and this prerelease is **not** marked Latest. The batch is dominated by daemon/queue reliability (the #1973 three-track stall-recovery set, the #1991 blocked-prompt safety floor, and the #2036 nudge-cadence fix), A2A room self-service onboarding, channel managed-send hardening (teams/discord/telegram), upgrade-safety guards for operator-customized files, agent onboarding/restart lifecycle, and the `agb status` fast-by-default perf work. Each PR carried its own internal codex review at park time; the rc2 cut is the integration gate. None of this batch is a daemon-down regression on top of rc1 — it is the accumulated hardening the rc-soak window was meant to collect. **Re-cut note (2026-06-21):** since the original `v0.16.16-rc2` tag had zero adopters (soak never started), it was rebuilt rather than deferring these to a later cut — folding the #2030 sleep-aware wedge supervisor, #2029 cron-child relay guard, #2021 plugin MCP-log liveness signal, #2035 ms365 token-freshness, and the #2036 nudge-cadence fix.

### Daemon & queue reliability

- **Idle re-nudge fires every cycle again — heavy maintenance steps are cadence-gated (#2036, re-cut marquee).** The daemon's `cmd_sync_cycle` is a single serial loop; `interval=5s` is only the inter-cycle sleep, but the cycle *work* had bloated to ~2 minutes (`daemon_tick_slow` rows of 106-151s), starving `nudge_agents` inside it so an idle session's re-nudge arrived minutes late instead of within seconds. The heavy un-gated steps (`bridge_sync` — now de-duplicated from two full syncs per cycle to one, the L1 roster reload, `mcp_orphan_cleanup_early`, `cron_staging_apply`, `claude_token_recovery`, `daily_backup`, `context_pressure_scan`) are now each cadence-gated via a fail-open `bridge_daemon_pass_due`, while `nudge_scan` / `nudge_agents` / `discord_relay` and every other delivery/wake/escalation path stay every-cycle. The gate is fully fail-open (a corrupt/unreadable/future cadence stamp runs the step now, never skips-forever, never aborts the daemon under `set -e`). The deeper Option-3 (decouple nudge dispatch into its own event-driven path) is the v0.17 direction.
- **Sleep-aware daemon wedge supervisor — no false self-abort on wake-from-sleep (#2030, re-cut).** On a host that sleeps (laptop / non-always-on Mac), the #1563 wedge supervisor measured in-tick "no progress" with wall-clock time; system sleep froze the heartbeat while the wall clock jumped forward, so on wake it read a huge no-progress age and self-aborted (`exit 99`) though nothing hung — a permanent outage if no restarter was loaded. The deadline is now suspend-aware (a single huge age-jump is treated as resume and re-anchored; an accumulation of poll-interval gaps — a genuine hang — still wedges), with a loud WARN when the abort path can't confirm a restarter.
- **Accept the Claude engine MCP-log as a second plugin-liveness signal (#2021, re-cut; credit @sankyul / #1770).** When the descendant-process probe is inconclusive/flaky for a plugin channel, the daemon now also consults the *current session's* engine MCP log for `Successfully connected` + `Channel notifications registered` before declaring the channel dead — cutting false-dead restart churn. The process probe stays primary (a conclusive-dead is never overridden); a stale prior-session log can't yield a false-alive (current-session-scoped, fail-closed to the probe when the session id can't be resolved).
...
- **ms365 token-freshness hardening (#2035, re-cut).** Two defensive fixes to `plugins/ms365/server.ts`: (1) a read-time normalize so a legacy ms-format (13-digit millisecond) `expires_at` left by an old plugin version no longer reads as valid-for-56,000-years → never-refresh (`expires_at > 1e12` → seconds), applied on `getAccessToken` + `get_valid_token`; (2) an optional `min_remaining_seconds` / `force` parameter on `get_valid_token` so a proactive-refresh cron can pre-renew with margin **without** changing the global 300s reactive threshold (which would pull every interactive call's rotation cadence in). Param-absent behavior is byte-identical to before; negative margins are rejected + clamped so a token is never served past expiry.
...
- **Runner-injected PreToolUse guard for disposable cron children (#2029, re-cut).** A disposable cron child ran `claude -p --permission-mode bypassPermissions` with `cwd` = the agent workdir (where channel transport creds live) and no tool restrictions, so a prompt-injected/over-eager child could read a channel token and post to a human channel directly (observed: a duplicate operator-channel delivery), or drop `BRIDGE_CRON_RUN_ID` from the queue CLI's env to mutate out-of-run tasks. The runner now injects a PreToolUse `Bash` deny hook (which fires even under `bypassPermissions`) into cron children only: it denies reading channel-credential `.env` paths + direct chat-transport/managed-send calls (Tier 1), and enforces in-run queue scope via the hook's own non-strippable inherited `BRIDGE_CRON_RUN_ID` (Tier 2), with carve-outs for legitimate in-run `cron:<run_id>` ops, `task create` delegation, and body-quoted verbs. Honest residual: a string-matching hook is not airtight under macOS "shared" isolation (no UID drop) — it mitigates the realistic prompt-injection / naive-command class; airtight containment needs OS-level isolation.
...
### Reliability / iso-permission (cm-prod RCA batch)

- **Upgrade warns (instead of silently breaking) on unmodeled Claude crons (#1943, F3).** The #1880 cron model-gate fail-closes an enabled Claude cron that resolves no model when interactive `settings.json` pins one — correct, but on upgrade it surfaced as a silent break with no migration or warning. `bridge-upgrade.sh` now appends a best-effort WARN to `[upgrade-complete]` (via a new read-only `lib/upgrade-helpers/cron-unmodeled-claude-warn.py` detector that imports the canonical `resolve_cron_child_model_effort` + `interactive_settings_model_for_request` so it fires on exactly the #1880 condition and can never drift). **WARN only** — it never auto-pins a model (auto-pinning the interactive model re-introduces the #1880 coupling; a cheaper model is a usage decision the operator owns) and can never fail the upgrade (`|| true` + internal try/except).
- **Iso bootstrap memory-index rebuild graceful-skips an unreadable iso tree (#1947, F5).** The bootstrap memory-index rebuild hard-failed (`rc=2`) when it hit a `0700` iso-owned tree it could not read, aborting the step instead of skipping the unreadable subtree. It now degrades to a graceful skip so a single unreadable iso home no longer breaks the rebuild.
- **Iso `agent restart` renders settings as the iso UID, not the controller (#1945, F7).** An iso `agent restart` rendered `settings.effective.json` as the controller UID into an iso-owned home → `PermissionError`. The settings-render now escalates to the agent's iso UID (fail-loud if it genuinely cannot escalate — never a silent controller-direct re-deny), and the deferral guard is gated **before** the render escalation so a decided-to-defer case returns cleanly. New smoke `1945-iso-restart-settings-iso-uid` gates the render; the existing `1145-option1-deferral-guard` gates the deferral decision (hardened non-vacuous after a regression that the broadened ci-select surfaced).
- **Daemon latches unclaimed-task escalation once per `(agent, task)` (#1944, F6 — Part 1).** The daemon's unclaimed-task escalation already had a per-task marker cooldown, but it re-alerted the same stuck queued task once per cooldown window (default `1800s`) rather than latching once for the queued `(agent, task)` — so a perpetually-unclaimed task kept re-alerting on a timer (the nudge-churn symptom). The per-task escalation marker is now a once-latch keyed by `(agent, task)` (cooldown default `1800 → 0`): it suppresses a repeat alert for the same assignee but **re-arms** on a handoff to a new assignee and self-heals a legacy single-line marker, so a legitimately-reassigned (possibly wedged) task is never permanently silenced. The deeper nudge-submit-loop reshaping (per-agent `submit_lost` backoff + wedged-session stop) is intentionally **deferred** — F6's root cause was already removed by the F8 queue-gateway fix in v0.16.15.

### Docs / UX

- **Headless shared-UID config-deny points at the supported flow (#1946, F4).** A headless shared-UID admin (daemon and agent on the same UID) has no config-mutation path because #1738 correctly fail-closes a self-owned binding store and there is no operator TTY — and the deny message offered no next step. This is **by design**, not a bug to code around (a daemon-mediated path on a shared UID is security theater — the daemon cannot prove non-forgeability when it shares the agent's UID). The deny `reason=` now points at the two supported flows — an attended operator TTY (`--from <admin>`) for one-time changes, or linux-user isolation (iso-v2) for unattended headless config — and `OPERATIONS.md` §"Iso v2 agent troubleshooting" documents the rationale. **Authorization-neutral**: only the deny message changed; the `#1738` deny teeth (`allowed=False` + the `agent-binding-store-writable` prefix the smoke matches) are unchanged.

## [0.16.15] — 2026-06-17 (LTS)

**v0.16.15 LTS — stability hotfix: a single poison request can no longer silently kill the whole fleet's queue gateway (#1949, cm-prod F8).** Completes the **F1/F8 daemon-stability P1 pair** from the cm-prod v0.16.13 post-upgrade RCA (F1 — the systemd-`--user` enable-linger 27h daemon-down — shipped in v0.16.14; F8 ships here). Cut directly to official (not the new RC-soak path) given the live daemon-stability impact on installs already upgraded to v0.16.13 / v0.16.14. Supersedes `v0.16.14` as the `lts` / Latest head; prior v0.16.x LTS designations remain valid.

- **A poison queue request no longer aborts `serve-once` → fleet-wide silent queue death (#1949, F8).** `bridge-queue-gateway.py`'s `cmd_serve_once` ran each request's `handle_request` with no per-request guard, and `run_queue` did `subprocess.run(cwd=<client-recorded cwd>)`. An iso agent that ran `agb` from inside a `0700` iso-owned dir (e.g. `.teams/attachments/<id>`) recorded a cwd the controller-run gateway could not `chdir` into → `PermissionError` aborted the entire batch, and the request — already promoted to `<id>.working.json` — re-crashed every tick (a permanent loop). Every other agent's queued request then timed out forever (the real root of the #1944 nudge-churn; on cm-prod a single attachment-cwd request killed the fleet queue ~26h while the daemon reported healthy). Fix: `run_queue` falls back to a controller-safe cwd when the recorded cwd is missing/inaccessible (queue ops key off `BRIDGE_*` env + the queue DB, not cwd), and `cmd_serve_once` wraps each request in a per-request try/except that dead-letters the poison out of the drain (`.poison` rename + error response) and CONTINUES the batch. New smoke `1949-gateway-poison-request` gates both.

The remaining cm-prod findings — F3 (#1943 cron model-gate), F5 (#1947 iso index-rebuild graceful-skip), F7 (#1945 iso restart settings perm), F4 (#1946 headless config path), and F6 (#1944 nudge backoff — root now removed by F8) — are **stability-unrelated** (reliability / iso-perm / UX) and are deferred to the next version, which will go through the new RC-soak release rule.

## [0.16.14] — 2026-06-17 (LTS)

**v0.16.14 LTS — hotfix: the systemd `--user` daemon now survives a login-session churn (enable-linger).** A sudo-self systemd-user install bound the always-on reconcile daemon **and** its liveness-backstop timer to the invoking login session without enabling lingering, so a single session churn (e.g. a `--replace` agent restart) made systemd leave `default.target` / `timers.target` and stop both — a silent, indefinite outage (observed on a production host: ~27h reconcile-daemon down + Claude token auto-rotation frozen → fleet quota-stall, because the liveness timer shares the daemon's `systemd --user` failure domain rather than being an independent backstop). The daemon installer now runs `loginctl enable-linger` whenever it **applies** the unit — not only on `--enable`, so `agb upgrade` (which re-applies the unit *without* `--enable`) reaches existing installs too — with a `sudo` fallback and a loud warning + exact remediation (`loginctl enable-linger <user>`) when it cannot. Found via the cm-prod v0.16.13 post-upgrade RCA (finding F1; patch-dev code-side confirmation #8756).

### Fast-follow (cm-prod RCA, tracked, locally mitigated)

- The companion cm-prod findings are tracked as fast-follow fixes (cm-prod has locally mitigated each): the **#1880 cron model-gate** breaking unmodeled Claude crons on upgrade without a migration or warning (F3 — local: per-job model pins); an **iso bootstrap memory-index rebuild** hard-failing (`rc=2`) on a `0700` iso tree instead of graceful-skip (F5); a **headless shared-UID admin** lacking any config-mutation path because `#1738` correctly fail-closes a self-owned binding store and there is no operator TTY (F4 — needs a daemon-mediated trusted path); **daemon nudge-retry churn** repeatedly nudging idle/wedged agents without per-agent backoff/dedupe (F6); and an **iso `agent restart` settings-render** `PermissionError` writing `settings.effective.json` as the controller UID into an iso-owned home (F7). The daemon self-heal that republishes `#1738` config-caller bindings for live resident panes was verified working (the F4 redefinition).
...
- **Config-set / set-env trust derives from an unspoofable pane binding, not env (#1738, #341 root).** `agb config set` / `config set-env` previously trusted `BRIDGE_AGENT_ID` / `--from` / `BRIDGE_CALLER_SOURCE` env to decide admin identity — forgeable by any same-UID caller. Trust now derives from a controller-published per-agent pane-pid binding that `bridge-config.py` matches against its own process ancestry (absolute `/bin/ps` + `/proc`); env no longer authorizes. Hardened over a multi-round security gate: an ownership-based store-writability fail-close (a caller-owned binding store can never prove non-forgeability), the tmux-liveness probe pinned to a clean **allowlist** env (no `DYLD_*`/`LD_PRELOAD`/`TMUX_TMPDIR` injection) with a kernel-boundary **pane-owner-UID** check, a fail-safe daemon prune (a transient tmux outage prunes nothing, never the whole fleet), and a defense-in-depth hook gate that denies a config mutation hidden behind `eval`/`bash -c`/`env`/`command`/`exec`/`nice` or an unresolved `$var`.
- **A smoke run can no longer down the live agent fleet (#1932).** Every bridge agent shares the host default tmux socket (a single point of failure), and the destructive smoke harness did not isolate it — a smoke run from inside an agent's tmux pane could tear that socket down and take **every** agent with it (observed twice on a live host). A new `lib/bridge-smoke-tmux-isolation.sh` guard severs an inherited `$TMUX` and forces a private socket (fail-closed if it would resolve to the shared/default one), sourced by the smoke lib + orchestrators + the tmux-touching smokes.
- **1738 liveness teeth use the probe's socket (#1936).** Follow-up to #1932 + #1738: the smoke's liveness test session is created on the same default socket the wrapper's liveness probe queries, restoring the ownership-gate teeth without reintroducing a fleet-down risk or an agent-controlled socket seam.
- **Hook command paths never resolve under a transient `/tmp` (#1934).** A settings render with a non-canonical / temp `BRIDGE_HOME` wrote `/tmp`-rooted hook command paths into LIVE agent settings, the `_template` scaffold, and Codex `.codex/hooks.json`; when the OS reaped `/tmp`, every hook vanished → `UserPromptSubmit` fail-closed-deaf + `PreToolUse *` tool-deadlock, bricking a production farm. Hook paths now resolve to a stable canonical dir for both engines, a missing hook **file** self-heals (session-start + daemon reconcile re-render, with a per-agent storm-guard) instead of bricking the agent, and the `state/isolated-plugin-grants/` shared dir is re-asserted to a durable `controller:ab-shared 0710` (never the writing agent's private group) so a grant write can't lock out another agent's launcher.

### Upgrade safety

- **Fail-safe picker-sweep migration (#1916).** The upgrade-init shell-kind picker-sweep migration deleted the legacy text-kind cron *before* creating the shell-kind one; when the create raced the daemon-restart window (observed on a v0.16.12 `--channel lts` upgrade), the host was left with **zero** picker-sweep crons. Reordered to **recreate-first / verify-before-delete**: the shell-kind cron is registered (and re-confirmed present) before the legacy row is deleted by id; on a failed/unverified create the legacy row is left in place + WARN, so there is never a window with neither.
- **De-ancestor the live-root CLAUDE.md to an operator stub (#1817).** The upgrade's `--ref` dry-run now hashes the requested ref's contract bytes (not the working tree), so a preview faithfully reflects what that ref would deploy; an operator-customized live-root CLAUDE.md is never overwritten.
- **Flag existing engine-mismatched instruction docs (report-only) (#1906).** Follow-up to #1896: the doc-backfill sweep now detects + flags an existing engine-mismatched instruction doc (a stale Codex-contract `AGENTS.md` left in a correct `claude` agent's home) in the `[hygiene]` surface — report-only, never auto-removed (operator does a reversible backup-rename).

### Agent runtime

- **Hard-ban AskUserQuestion for all bridge Claude agents (#1923).** The interactive multiple-choice `AskUserQuestion` picker blocks the input pane → a long-running agent goes silently deaf to A2A / daemon nudges / other agents while it is open. #1569 bounded it for hook-managed agents, but dynamic-vanilla agents (#1890) never got the intercept. A dedicated dependency-free PreToolUse deny hook (guaranteed under `--dangerously-skip-permissions`) now bans the tool for **every** bridge Claude agent — static, dynamic-managed, and dynamic-vanilla — routed into the vanilla `settings.local.json` overlay, plus a scoped `permissions.deny` as defense-in-depth. The operator-global `~/.claude` is never touched (the operator's own interactive sessions keep the tool).
- **Detect feedback-survey + permission/overwrite/context modals as blockers (#1181, Layer 1).** A long-running Claude agent that has the feedback-survey modal (or a per-session permission grant / overwrite-confirm / context-pressure modal) drawn into its input pane silently stops consuming nudges while `agb status` shows it healthy. The blocker detector now recognizes these modals, the status snapshot / `agent show` / wake surfaces report `wake_reason=modal_<state>` via a single shared predicate, and the daemon nudge-drop audit tags the modal class. Detection/status only — no submit-path. `CLAUDE_CODE_DISABLE_FEEDBACK_SURVEY=1` is documented as the recommended noise-reduction env.
...
**v0.16.12 LTS — upgrade respawn-race fix (systemd + launchd), plus iso-v2 transcript harvest, resume hardening, and usage edge-classification.** The marquee change closes an **upgrade-breakage footgun that hit every systemd-managed install** (and its macOS launchd analog): the #1820 layout-v2 reconcile quiesce stopped the daemon only at the script level, so the init system respawned it inside the quiesce window → the fail-closed reconcile fence saw a live pid → `rc=3` and a half-applied upgrade. The quiesce/restart is now init-system-aware on both systemd and launchd, validated on the cm-prod production host (the #1905 host-acceptance passed 5/5 — the exact footgun that required a manual unit-stop on v0.16.11 now upgrades untouched).

### Upgrade respawn-race — the marquee fix

- **systemd: #1820 reconcile quiesce is now systemd-aware (#1905).** On a systemd sudo-self install, `agb upgrade` aborted `rc=3` at the #1820 reconcile gate and half-applied, because the script-level `bridge-daemon.sh stop` is not systemd-aware — `agent-bridge-daemon.service` (`Restart=`) + `agent-bridge-daemon-liveness.timer` respawned the daemon inside the quiesce window. The upgrader now stops the liveness timer then the service via `systemctl --user` (and restores them on restart), mirroring the existing A2A-receiver systemd-aware pattern. A real-host catch from cm-prod: `systemctl --user` needs `XDG_RUNTIME_DIR` (unset in the upgrader env, Linger=no), so the fix also establishes it before every `systemctl --user` call — without which the stop would silently no-op (swallowed by `|| true`) and the bug would recur. Graceful fallback + loud WARN when no user bus is reachable but the unit is installed.
- **launchd: the macOS analog (#655).** The same respawn race on macOS: a `KeepAlive=true` LaunchAgent respawns the daemon during the quiesce window. The upgrader now `launchctl disable`→`bootout`s the job before the stop and `enable`→`bootstrap`→`kickstart`s it on restore, gated behind a launchd-active check and mutually exclusive with the systemd path (a host is systemd or launchd, never both). Non-macOS / plain-bash installs are byte-for-byte unchanged.

### iso-v2 + resume + usage

- **iso transcript harvest crosses the iso boundary via run-as-iso (#1894).** The controller-UID memory harvester couldn't read iso agents' `<iso-home>/.claude/projects` (runtime-created `2700`, iso-owned; the prepare-isolation ACL the comment promised is absent), so every iso agent fell to `--skipped-permission` permanently. The bounded transcript scan now runs as the iso UID via the existing sudoers `bash` allowlist and marshals results back to the controller, which keeps owning all queue-DB / aggregate writes. The prepare-isolation path is untouched (preserves the `index.sqlite` `0600` carve-out + publisher ordering).
- **resume config-dir resolver realpath-hardened against symlinked-to-operator-HOME (#1893, #1889 fast-follow).** Two non-blocking hardening items: the dynamic config-dir resolver now realpath-compares the operator-HOME exclusion (so an agent `.claude` symlinked to the operator's global falls through to scaffold), and the quarantine-archive refuses to move a transcript that realpaths into the operator's own projects root. The shipped #1890 dynamic-vanilla-Claude early-returns are preserved.
- **usage probe classifies CDN-edge blocks vs account quota (#1824).** A usage-probe failure that is a CDN-edge block (Cloudflare 429/403 with no anthropic origin headers) is now classified `edge-blocked` and never fabricates a synthetic 100% near-limit cache (which would poison rotation); a genuine origin `account-rate-limit` still fires the near-limit signal. Adds per-token probe backoff + alert isolation, and `Retry-After` now honors both the delta-seconds and RFC 7231 HTTP-date forms.

### Soak / acceptance (verified before the cut)

- **Local OrbStack real-Linux VM-verify** (Ubuntu noble, iso-v2): `oss-preflight` PASS, all five lane smokes + the ci-select shard self-test + the upgrade fail-closed smoke PASS on Linux; no macOS-only-smoke-not-Linux-portable regression.
- **cm-prod production acceptance (systemd iso, 8 live customer Teams bots):** the #1905 host-acceptance passed 5/5 with decisive evidence the XDG fix fired (single-pass reconcile + daemon PID replacement); customer bots unaffected.
- **macOS launchd acceptance:** the #655 launchd quiesce/restart validated on a real macOS launchd rig (disable→bootout brings the daemon down and keeps it down vs. a 4s respawn without the fix; enable→bootstrap→kickstart restores it). The full destructive `agb upgrade` end-to-end is tracked as a v0.16.13 CI/VM follow-up; the fix is fail-closed (worst case = the documented pre-existing `rc=3`, never new breakage).
- **CI:** the full required smoke battery (3-way sharded) was green on Linux for every constituent lane PR; the assembled tree was additionally verified by the OrbStack real-Linux VM-verify above (oss-preflight + all five lane smokes + the ci-select shard self-test). The chronic #1699 `daemon.sh` cold-boot flake recurred a few times across the session and cleared on re-run each time (deterministic-fixture hardening is tracked for v0.16.13).

## [0.16.11] — 2026-06-15 (LTS)

**v0.16.11 LTS — dynamic agents become vanilla-equivalent, plus iso-v2 create-path / doc-backfill hardening.** Promotes `v0.16.11-rc1` to the blessed LTS tag for the v0.16 line after a real-Linux soak. The marquee change is operational convergence for **dynamic agents**: a dynamic Claude or Codex agent now runs like vanilla Claude Code / Codex CLI against the operator's global config (settings, model, session history, installed plugins, MCP, credentials) with native resume (`claude -c` / `codex resume --last`), and the bridge layers only its comms/queue hooks via project-local settings — superseding the private-config-dir isolation that was destroying operator sessions. The full item list is in the `0.16.11-rc1` entry below.

### Soak (what was verified before promotion)

- **Local OrbStack real-Linux (Ubuntu noble, iso-v2 active) soak.** Upgrade from `0.16.4` → `0.16.11-rc1` applied cleanly (246 files, 12 agents migrated, eligible agents restarted, one expected `settings.json` upgrade-conflict backed up for review); the install came up healthy on rc1 (daemon running, 0 health warnings). The six release-critical smokes ran on real Linux (where macOS skips the iso paths) and passed: `1891` (iso create-path), `1892` (doc-backfill fail-closed), `1890` (dynamic vanilla Claude), `1899` (dynamic vanilla Codex), `1900-refresh-noop`, `1899-v2-secret-env-repin`. The dynamic-vanilla Claude launch builder was confirmed live to emit a vanilla `claude` command with no `--resume <id>`, no private `CLAUDE_CONFIG_DIR`, and no model/effort override, with the session-detect/persist chokepoints gated off for the dynamic class.
- **CI.** The full required smoke battery (3-way sharded) was green on Linux for every constituent PR and for the rc1 release PR.
- **Pending production acceptance.** Downstream cm-prod (real-Linux iso-v2 production) consumes this LTS via the `lts` channel and will run the contracted post-upgrade acceptance (later-created iso agents' `memory/` auto-normalized to iso-owned `2770`, `agent-meta.env` auto-created, roster-first engine detection, and reconciliation of the one historical mis-detected entrypoint). Note: live `isolate --reapply` was not exercised on the soak machine because that install is in a pre-existing markerless layout state (no `layout-version` marker); the iso create-path change is covered by the `1891` functional smoke + CI + the cm-prod post-upgrade acceptance above.
...
- **Idle dynamic-agent reaper no longer kills operator-created or `loop=1` dynamics (#1795).** `reap_idle_dynamic_agents` killed *any* detached + queue-empty + idle dynamic agent past `BRIDGE_DYNAMIC_IDLE_REAP_SECONDS` and deleted its roster entry — correct GC for auto-spawned throwaways, but it also destroyed operator-created long-lived dynamics (live evidence: the same operator codex pair reaped 4×). The reap predicate now requires, on top of the existing idle checks, `ephemeral == "1"` (exact) **and** `loop != 1`: disposability is tagged at creation (wave-fixer dispatch + the `agent-bridge --ephemeral` flag), interactive operator spawns stay non-ephemeral, and a `loop=1` agent is unconditionally exempt (reaping it would void the relaunch contract). Migration fail-safe: a roster entry with no flag reads as non-ephemeral ⇒ never reaped (uniform across all five assignment sites). A one-line keep-audit shows when the reaper considered-and-kept an idle agent. `BRIDGE_DYNAMIC_IDLE_REAP_SECONDS` is now documented (OPERATIONS.md) and on the `config set-env` allowlist as a validated non-negative int.
- **Upgrade doc-migration no longer silently rolls back agent memory (#1781).** The upgrade-time migrate-agents pass synced the agent-home `MEMORY.md` over the live `workdir/MEMORY.md` whenever they differed — rolling agent memory back to a stale copy on every upgrade (13 of 22 agents on one host). `MEMORY.md` / `users/<id>/MEMORY.md` are now treated as agent-written state: never overwritten when present, still first-materialized from the profile seed when absent, with each preservation audited and kept in the targeted backup set.
- **No-LLM picker auto-resolve stopped false-escalating every idle session (#1783).** Enabling the v0.16.8 picker stage flagged ordinary idle composer screens as `UNKNOWN`-stuck — 19 of 22 sessions escalated within minutes, recurring every ~5 min. Both engines' idle composers are now cataloged `non_picker` (empty-composer-anchored, foreground-tail-scoped so a real picker sharing the capture still escalates), the prompt-like heuristic requires an actual option/affordance, and a per-pass storm fuse caps unknown escalations.
- **Post-upgrade `verify-tasks-db` works from the admin agent, and the "unable to open database file" mystery is root-caused (#1786).** The upgrade-complete checklist told the admin agent to run a command the v0.16.8 hook blocks; it now routes through a policy-blessed `agent-bridge doctor --detectors tasks-db`. The reported open failure was not the guard stack — a WAL `mode=ro` open without the `-shm` sidecar raises `SQLITE_CANTOPEN` on a healthy db; fixed with an `immutable=1` fallback gated so an unreadable non-empty `-wal` reports `unverifiable`, never a stale `ok`. (Review bonus: the env-assignment prefix form `DB=<path> sqlite3 "$DB"` that could slip past the queue-DB guard is now also denied — a #1709-class static-decode extension.)
- **`doctor` no longer guides quarantining a live agent, and stops false-flagging the intended settings layout (#1787, #1788).** On case-insensitive filesystems `orphan-agent-dir` + `agent retire` compared names case-sensitively, so a live agent's directory under alias casing was reported an unregistered orphan *with a quarantine recipe* — executing it would dangle the agent's settings. Identity is now inode-based with a three-state contract (match / no-match / indeterminate→keep). Separately, `settings-two-tree-drift` flagged the renderer's intended dual-render for every Claude agent and suggested a non-existent entry point; both detectors are now aligned to the renderer-canonical per-layout shape with working recipes.
- **Cron-dispatched child sessions are fenced to their dispatched job, and task-create origin is attributable (#1792).** A cron child inheriting full admin context would "helpfully" act on unrelated in-flight work (creating queue tasks under the admin's name). The dispatch prompt now carries a hard scope fence (control-char-sanitized so a job field can't inject a forged directive), and task-create records an `origin` (`cron:<run_id>` | `session:<id>`) that the server validates against the cron runtime's own trusted anchor — a non-cron process can't spoof a cron origin.

### Operational notes

- The us↔cm-prod production A2A pair was restored end-to-end on 2026-06-11 using v0.16.7's `trusted-routed` transport (root cause: Cloudflare WARP corporate forwarding blackhole) — bidirectional delivery verified, existing mesh peers unaffected.
- Follow-ups filed from this train: #1797 (reaper keep-audit log volume + ci-select coverage + minor edges), plus the carried external-contrib PRs (#1770 MCP engine-log liveness, #1790 #1789 D1/D2 rotation) and #1789 D3–D5 / #1784 picker-judge design, all for a later train.

## [0.16.8] — 2026-06-11

**Session-resume + settings/CI reliability sweep, plus no-LLM picker auto-resolve** — a patch on the **v0.16 LTS line** closing seven issues that all surfaced from live fleet operation: agent restarts silently discarding live sessions (two distinct mechanisms), the operator-global settings symlink turning self-referential, `agent update --model/--effort` being a silent no-op on static agents, iso v2 agents unable to read their own settings (a production startup-blocking picker), fresh agents rendering an abbreviated HUD, and a CI ratchet whose substring matching cost three false-red rounds in one day. Every lane went through the queue pair-review gate (agb-dev-codex), with the restart/iso/settings lanes additionally gated by the independent `patch` live/VM verification; the review rounds caught real defects in five of seven lanes — including a path-traversal escape, a quarantine-bypass in the trusted-resume design, and a fail-open parser edge. **Every v0.16.x release is an LTS release** — v0.16.8 supersedes v0.16.7 as the line head. Same `agb upgrade` path; A2A wire protocol unchanged; all changes are fix-class or guard-tightening and degrade safe.

### Added

- **No-LLM picker auto-resolve — daemon fingerprint catalog + policy keystrokes + admin escalation skill (#1762).** The daemon now detects interactive pickers (codex update prompts, Claude resume/compact prompts, trust dialogs) by capturing tmux pane content and matching it against a shipped fingerprint catalog (`runtime-templates/shared/picker-catalog.json`), then resolves them with cataloged policy keystrokes — no LLM sweep required. Three safety rails: post-resolve re-capture verification, anti-loop guard, and a destructive-option guard (any option containing ASCII `>` pointing at destructive verbs escalates instead). Unknown pickers escalate to the admin via the new `picker-resolve` skill with a verbatim capture for catalog promotion. The blocked agent returns to a state where it can resume its previous session and accept user prompts.
- **Operator plugin display config seeds into new agent homes (#1753).** Fresh Claude agents no longer render an abbreviated HUD: at agent create/start, allowlisted per-plugin display config (default exactly `claude-hud`, extendable via `BRIDGE_SEED_PLUGIN_CONFIG_ALLOWLIST`) seeds **if-absent** from the operator's `~/.claude/plugins/<plugin>/config.json` into the agent home — never overwriting a diverged agent-local config, with plugin-id validation + resolved-path containment on both layers (a review round probe-proved and closed a `../../` traversal escape) and a Claude-only engine gate. On the shared/non-iso path the no-overwrite contract is exclusive-create race-closed (`open(…, "xb")`); on iso v2 the copy goes through the controller-publish pattern (`chgrp ab-agent-<a>` 0660) as seed-if-absent on the supported create/start flows, with a documented residual stat-then-publish window (an atomic `publish-if-absent` iso primitive is a noted follow-up, not claimed here).

### Fixed

- **Agent restart no longer silently discards a live session — both mechanisms of #1769 closed.**
  - *Mechanism 1 — post-kill resolver gate (#1774):* `run_restart`'s #981 session re-inject was defeated for fresh/idle sessions (live-session record only, no eligible transcript): the resolver's #827 shortcut needs an alive pid, so the just-killed id was wiped and the agent relaunched fresh. `run_restart` now validates the live session it is about to kill (record presence, pid alive, cwd match, **not quarantined**) and writes a one-shot trusted-resume marker (`state/agents/<a>/resume.trusted`, 0600, TTL 300s, rewritten per verify-retry kill, Claude-only); the resolver accepts that exact id once with a dead pid — **after** quarantine checks, which the #827 live-session shortcut now also enforces (a quarantined id never resolves, live or not; the review caught this bypass in two successive rounds). Marker consume is decision-site-gated so hydration probes can't burn it; terminal clears at every restart exit path.
  - *Mechanism 2 — setup-freshness gate (#1777):* `bridge-start.sh`'s `FORCE_FRESH_SESSION` discarded a resolvable resume id **silently** whenever any setup-freshness check tripped — the actual cause of a live 18-session mass-fresh event (1.3–38.8 MB transcripts). Every fresh-forcing trip now logs one named line, and on the normal restart path a resolvable resume id plus only re-ensurable trips keeps `--continue` (the five hook-status checks re-ensure unconditionally before exec and die on failure — no stale-hook resume path exists). First-boot, `--skip-project-skill`, explicit `--continue`, and Codex paths are byte-identical.
- **`agent update --model/--effort` now reaches static Claude agents (#1763).** The static launch builder never read the roster `BRIDGE_AGENT_MODEL`/`BRIDGE_AGENT_EFFORT` that `agent update`/`materialize-fields` wrote — a silent no-op behind "changed: yes". The builder now emits `--model`/`--effort` with roster-wins / preserve-when-empty precedence, deduping both space-form and joined-form baked flags, consuming a follower token as the value slot only when it is a plain value (an option-shaped follower is preserved — a review round caught the earlier rule eating an unrelated `--settings` flag). The documented `--set-launch-cmd` workaround keeps working byte-for-byte.
- **iso v2 agents can read their own `workdir/.claude/settings.json` (#1766).** The per-agent `settings.effective.json` rendered controller-owned 0600 under a 0700 parent, so every iso agent hit EACCES on its own project settings → a blocking "Settings Error" picker on every (re)start that became load-bearing during a cm-prod restart storm. Render/prepare/reapply now publish the effective file `controller:ab-agent-<a>` **0640** (group-read only, never group-write, never `ab-shared`) with a 0750 traversable parent — re-applied after every rerender so the fix can't flap; the `publish_content_tree` planted-redirect guard accepts exactly the canonical self-target symlink (realpath target-validated, dir_fd TOCTOU-safe) while all other symlinks stay refused; and the Settings Error picker is cataloged for auto-resolve (option 3) as a belt-and-braces mitigation. Verified on a real iso v2 VM with kernel-enforced UID boundary tests.
- **Operator-global settings self-reference loop broken (#1759).** On shared-admin layouts where the operator's `~/.claude/settings.json` is a symlink to an agent's own `settings.effective.json`, the #11901 base-read became self-referential for that one agent — self-sustaining benign keys, surviving accidental hand-edits, and letting drift-apply rewrite the operator global wholesale. One detection function now classifies the operator-global target (realpath equality → `os.path.samefile` inode equivalence for APFS case-variants → output-shaped classification) with fail-safe direction: indeterminate output-shaped targets break the loop and degrade to the bridge base, while another agent's output reached one-directionally — including the first-render case — keeps the verified #11901 inherit. Render, drift-plan, and drift-apply all delegate to the one function; drift-apply additionally refuses to write *through* a symlink doubling as the operator global without `--force-operator-global` (naming the file); preserved user keys survive the loop-break via the #1756 pass.
- **CI iso-helper-ratchet substring false-positives eliminated (#1764).** The boundary-site pattern matched `\.env` inside `os.environ` and flagged test-fixture `settings.effective.json` paths — three false-red CI rounds in one day, each costing a diagnose+noqa+push cycle. The pattern is now boundary-anchored (`(…)([^A-Za-z0-9_]|$)`, POSIX-safe, rg-portable), the baseline dropped 1561→871 across 125 files with a mechanically-audited no-per-file-increase guarantee, and a self-test smoke (live-pattern extraction, the three false-positive shapes as negatives, nine genuine positives, noqa + fail-closed behavior) locks the anchoring against regression.

### Operational notes

- The us↔cm-prod production A2A pair was restored end-to-end on 2026-06-11 using v0.16.7's `trusted-routed` transport (root cause confirmed: Cloudflare WARP corporate forwarding blackhole) — bidirectional delivery verified 6/6 with the existing mesh peers unaffected.
- Known cosmetic: `a2a net-status` displays a per-peer `trusted-routed` override under the node-level substrate label; the override applies at send time and functions correctly.
- Earlier deferrals carry forward (#1738, #1699 — the daemon.sh cold-boot flake cost ~1 rerun per PR this train, #1711/#1722, #1709 brace numeric-range over-block). PR #1770 (external, MCP engine-log liveness) is in review for a later train.

## [0.16.7] — 2026-06-10

**`trusted-routed` A2A transport + a hooks/settings reliability pair** — a patch on the **v0.16 LTS line** whose marquee is an operator-directed capability-add (allowed on the LTS line per `docs/release-lines.md` when operator-directed; shipped here rather than a v0.17 beta so `lts`-channel production installs can actually receive it): a new A2A transport kind for corporate-routed trusted private networks, restoring a down production cross-bridge pair. Every lane went through the dual gate (agb-dev-codex + patch); the A2A lane converged over three rounds with each reviewer catching a distinct real blocker. **Every v0.16.x release is an LTS release** — v0.16.7 supersedes v0.16.6 as the line head.

### Added

- **`transport.kind: trusted-routed` — direct private-IP bind on a trusted routed network, no WARP/Tailscale required (#1758).** For hosts on a corporate-routed private network (the operator's trust call), the receiver's bind proof is "the configured `listen.address` is assigned on a real local interface" — the WARP/Tailscale enrollment half is dropped for this kind ONLY. Everything else stays fail-closed: loopback/wildcard binds refused, an **explicit `listen.address` is required** (empty hard-fails `bind_unresolved` — no Tailscale auto-select fallback), unknown kinds hard-fail, and HMAC / `inbound_allowlist` / source-addr check / dedupe are byte-untouched. **Per-peer transport override**: a peer entry may carry its own `transport.kind`, and the sender now picks the egress source per destination — mesh peers keep the pinned mesh source, routed-marked peers get the OS-routed source (fixes the measured "Network is unreachable" when a mesh-pinned source can't reach a routed peer). A typo'd per-peer kind degrades that one row to retry (never halts the whole outbox runner); a typo'd node-level kind still fail-closes the receiver bind. Docs: `docs/a2a-cross-bridge.md` trust model + rollout recipe.

### Fixed

- **Dynamic-agent every-prompt "UserPromptSubmit hook error" eliminated (#1755).** Two-layer fix: (1) the bridge hook was registered in global + workdir scopes with different interpreter spellings, so Claude Code's exact-string dedupe ran both concurrently — all hook ensure paths now record the pinned interpreter and converge stale-spelling duplicates in place on the next ensure pass; (2) `save_timestamp_state` (and same-class call sites) move to unique per-instance temp files with **no error swallow** — all rename errors propagate to the existing #1205 iso-fail-open/controller-re-raise policy (a review round caught that swallowing rename errors after the unique-temp change would mask real write failures; a forced-failure regression pins it).
- **Operator-pinned session preferences survive settings re-renders (#1756).** `model`, `alwaysThinkingEnabled`, `agentPushNotifEnabled` join `PRESERVED_USER_KEYS` (the #1689 statusLine data-loss class — live-confirmed: a pinned model silently reverted after an upgrade re-render). The `link-shared-settings` adoption step now folds existing user keys into the render input at symlink takeover, runs the launched-channel sticky-false repair in the same position as the render paths (a review round caught that the fold could otherwise re-disable a launched channel plugin, #1453 class), and a global `model` inherits through the #11901 safety filter.

### Known issues

- **#1759** — on shared-admin layouts where the operator global is itself a render-output symlink, the #11901 base-read is self-referential for that one agent (loop guard + operator-input/output split designed, not yet implemented).
- **#1762** — no-LLM picker auto-resolve (daemon fingerprint catalog + policy keystrokes + admin escalation skill) is in implementation; next train.
- Earlier v0.16.6 deferrals carry forward (#1738, #1699, #1711/#1722, #1709 brace numeric-range over-block).

## [0.16.6] — 2026-06-10

**Mesh-robustness + read-guard hardening, plus an upgrade/settings/daemon reliability sweep** — a patch on the **v0.16 LTS line** that makes the A2A mesh resilient to transient peer churn, gives admins a durable install env-override path, comprehensively closes the static-decodable bypass class in the shared-secret read-guard, and folds in a reliability batch (queue-gateway crash-loop fix, upgrade-time orphan reaper, settings-render conflict collapse, shared-global settings inheritance, A2A receiver-supervisor robustness, and a fresh-install admin-identity scaffold fix). Every lane was gated by per-PR + full-branch agb-dev-codex review; the security/settings/daemon-critical lanes (read-guard #1709, test-bind state-path #1728, shared-global settings #11901) additionally went through the independent `patch` write-gate, and the WARP bounce-gate (#1733) was verified on a real Cloudflare-WARP mesh host. The dual-gate review repeatedly earned its keep — it caught a series of real read-guard bypasses on #1709 (converged over many rounds) and an all-denied-global silent-drop on #11901 that the per-lane review missed. **Every v0.16.x release is an LTS release** — v0.16.6 supersedes v0.16.5 as the line head and does not revoke the prior LTS designation. Same `agb upgrade` path; A2A wire protocol unchanged (`PROTOCOL_VERSION` `a2a-enqueue-v1`); all changes are additive or guard-tightening and degrade safe.

### Added

- **Audited durable install env-override — `config set-env` (#1734).** Admins now have a durable, write-gated path to set install-level environment overrides without hand-editing tracked files: a JSON-only `config set-env` surface behind the existing write-gate ∩ config-set contract, so the override survives upgrades and never leaks into the source tree.
- **Shared static agents inherit the operator's global `~/.claude/settings.json` (#11901).** A shared (non-isolated) static Claude agent now inherits the operator's system-global `~/.claude/settings.json` as the bottom-most render layer, so operator-global changes (e.g. `agentPushNotifEnabled`) propagate to shared agents — while per-class defaults (static `autoCompactWindow` stays 400k), bridge hooks, and per-agent keys still win on top. A safety denylist filters credential/machine-specific keys (`apiKeyHelper`, `env`, `permissions`, `statusLine`, `awsAuthRefresh`, credential-shaped names) before inheritance, emitting one `[info]` line naming any dropped key; a missing/unreadable/malformed global fail-safe-degrades to the existing bridge base. The operator home is resolved via the controller-home path (not the per-agent `CLAUDE_CONFIG_DIR`), and isolated (iso-v2) agents are explicitly unaffected.

### Fixed

- **WARP bounce gate honors the transient-peer class — cross-lane #1732 × #1733 (integration-review catch, #1742).** The full-branch integration review caught a cross-lane regression no per-lane review could see: Lane B (#1732) made transient peers a distinct quiet/no-alarm class, but Lane A's (#1733) WARP bounce classifier still treated every configured peer as bounce-relevant — so a transient (expected-disconnect) peer going down could bounce the tunnel, and a transient peer's stale state could even suppress a real persistent-loss bounce. The bounce gate now builds its loss proof from bounce-relevant (`peer_alarm_on_unreachable`, i.e. persistent default + explicit override) peers only; transient peers are excluded from the proof.
- **A2A transient-peer resilience — quiet-pend, no-drop, seamless resume (#1732).** A peer that briefly disappears (transient tailnet/WARP blip) is no longer hard-dropped from the mesh: the reconcile loop quiet-pends a transient-suspect peer, holds its roster entry, and resumes delivery seamlessly when it reappears — instead of tearing the peer down and forcing a full re-join. Composes with the #1733 bounce-gate so a transient loss neither drops the peer nor bounces the tunnel.
- **WARP tunnel-bounce gated on real peer-loss, not handshake-idle (#1733).** The WARP tunnel-health auto-bounce was too aggressive — it fired on handshake *idle* (no traffic on a healthy mesh), severing a live tunnel. A full bounce now requires actual peer-loss signal: stale + N≥2 consecutive streak + ≥1 fresh suspect/down peer + soft-refresh-first; all-fresh-up or stale-unknown is a hard no-bounce. The connection is threaded to tunnel-health which reads the prior-tick peer FSM. New env knob `BRIDGE_A2A_WARP_HANDSHAKE_STALE_SECONDS` tunes the stale threshold. Verified on a real Cloudflare-WARP mesh host (no idle self-bounce; recovers on genuine peer loss).
- **Shared-secret / peer-home read-guard — static-decodable bypass class comprehensively closed (#1709).** `hooks/tool-policy.py`'s read-guard for `shared/secrets`, `shared/private`, and other agents' homes resolved a read's *spelling* but not the path Bash actually resolves, so a range of static spellings bypassed it. The guard now MODELS the resolved path across the full static-decodable surface and denies on resolved-containment: `cd`/`pushd` cwd grammar (positional, subshell, `pushd`/`popd`/`cd -`), `&&`/`||`/`;` conditional-execution chains (short-circuit precedence, a cd-OUT-fail "sticky" fail-branch, `!`-negated cd exit), reserved-word/quote-removal tokenization, `$BRIDGE_HOME`/`~`/`$HOME` prefix expansion, `..`/`//`/`.` folding, glob enumeration (`ls sec*ets`, `cat */token`), ANSI-C `$'…'` decode (incl. hex/octal/unicode), and bash brace expansion (`{secrets,wiki}`, ranges, nested/cartesian — with a cap-truncation fail-close and quote-aware expansion so quoted/escaped braces stay literal). Runtime-only indirection (`eval`, command-substitution, `$var` read-prefix, symlink-through) is explicitly out of the static-analysis scope — covered by filesystem permissions (iso v2) + #1738/#341 — and this hook remains defense-in-depth over the static-path class. Closes the v0.16.5 read-bypass class.
- **Test-bind state-path guard — no live `rooms.db` clobber (#1728).** A local test mesh that overrides only `BRIDGE_HOME` could write into the live `state/handoff/rooms.db` because `handoff_dir()` honors `BRIDGE_STATE_DIR` over `BRIDGE_HOME`. A `BRIDGE_A2A_ALLOW_TEST_BIND`-gated guard now refuses to bind when the resolved state path points outside the (test) home, preventing a test run from clobbering live mesh state. Production installs are unaffected (the flag is test-only). Resolves the v0.16.5 known issue.
- **Queue-gateway socket-listener crash-loop stopped (#1652).** The queue-gateway socket listener could enter a crash-loop: it unlinked a socket that still had a live pid (premature `clean_stale`), an unguarded `FileNotFoundError` in the socket group/mode refresh hard-failed on a transiently-missing socket path, and an empty connect-probe `recv` churned `invalid_payload`/`send_failed` log noise. The daemon now requires N consecutive probe failures before unlinking a socket whose pid is alive (`bridge_queue_gateway_socket_probe_persistently_dead`), narrowly catches the missing-socket `FileNotFoundError` to degrade instead of crash, and routes an empty probe through a quiet `_ProbeClose` path. Live socket-transport verified in an isolated `BRIDGE_HOME`.
- **Upgrade-time reaper for leaked codex-broker + queue-gateway orphans (#1567).** Upgrades now reap the accumulated backlog of already-leaked orphans — `ppid==1` codex `app-server-broker` processes whose disposable worktree `--cwd` is gone, and stale `bridge-queue-gateway` procs whose `--bridge-home` no longer resolves. Conservative by default: a dry-run report + a high-priority admin cleanup task with the safe-kill recipe; nothing is killed inside the upgrade unless `AGENT_BRIDGE_REAP_CODEX_ORPHANS` is set. Guarded by a one-shot migration marker (runs once, skips new installs), an `lstart`-anchored PID-reuse guard, and positive disposable-worktree provenance so it can never match a live owned broker.
- **Upgrade no longer prompts on cosmetic settings.json differences (#1675/#1694).** A Homebrew `/opt/homebrew/bin/python3` interpreter path and rendered absolute `.agent-bridge/hooks` paths now canonicalize to the same form as the tracked template, so a purely cosmetic interpreter/path difference no longer triggers a settings.json merge-conflict prompt during upgrade. Real edits (different hook basename, bare/venv interpreter, unrelated hooks directory) still fall through as genuine conflicts.
- **Fresh-install admin workdir identity no longer clobbered by the codex-pair template (#1750).** On a managed-project install the admin's codex pair shares the admin's workdir (#1492); the start-time identity reconcile (#1417) carried neither create-time shared-workspace guard, so the pair's first start copied *its* home identity (SOUL/SESSION-TYPE/CLAUDE) over the admin's workdir — and since the runtime resolves identity from the cwd copy, a fresh admin booted believing it was the codex pair-programmer, mis-applying onboarding. A roster-aware ownership guard now decides by the identity the workdir actually carries (whole-token canonical-position name match — `patch` structurally cannot match `patch-dev`; longest-match tie-break; a stale-but-mine copy still refreshes from home per #1417; indeterminate ownership fail-safes to keeping per-agent identity in `agent_home`, never rendering a sibling template).
- **A2A receiver supervisor no longer floods the admin queue on macOS tailnet conditions (#1679, #1680).** On a macOS A2A host the receiver supervisor misclassified *environmental* tailnet conditions as receiver faults: a self-route flap (the host transiently can't reach its own tailnet IP) produced bursts of false `healthz_timeout` → 51 bogus crash-loop tasks over 2 days (#1679), and a tailnet-IP loss (Tailscale down/wedged) crash-looped the fail-closed bind, permanently stopped auto-restart after 5 tries, and emitted ~100 alert tasks over a 53h outage with no auto-recovery (#1680). The supervisor now discriminates environment from fault: on a `/healthz` timeout it first probes self-reachability (an outbound connect to its own already-bind-proven address) — a self-unreachable host HOLDS the healthy receiver (`healthz_unreachable_self`, no restart-budget burn, no crash-loop task) while a reachable host still restarts a genuinely-wedged serve loop (the #1405 path is preserved); a `bind_fail` from an absent tailnet IP is treated as a retryable environmental state (`env_bind_ip_absent`) that re-probes and auto-rebinds when the IP returns instead of burning the restart budget; and the per-cycle alert flood collapses to a single auto-clearing "receiver environmental incident" task. The fail-closed bind proof, HMAC, allowlist, and dedupe are untouched — the new discriminators are read-only.

### Internal

- **CI: `lint-heredoc-ban` job timeout 5m → 10m (#1747).** The heredoc-ban baseline ratchet runs a genuinely CPU-heavy audit (`audit-footgun-11.sh --tsv`, ~3.5min) that was being killed at the 5-minute wall on congested runners, cancelling otherwise-green PRs. Raised to 10m (no scan-logic change).
- **CI: `unit/static smoke` job timeout 20m → 25m (#1752).** The required-smoke battery grows with every release wave and legitimately runs ~17-18m healthy by v0.16.6; the 20m cap cancelled green runs twice in one day under normal runner congestion (the same false-red mode as the original #1509 10m cap). The comment now carries a split-don't-bump guideline if the healthy baseline crosses ~20m.

### Known issues

- **#1709 brace numeric-range truncation** — a brace whose *numeric range* overflows the expansion cap under a bridge cwd currently fail-closes (a safe-direction over-block, never a leak); precise exemption of pure-numeric-range truncation is deferred to a v0.16.x follow-up.
- **#1738** — env-trust wrapper redesign for the class-(b) runtime-indirection tail of the read-guard (eval/`$()`/`$var`-cd/symlink); covered today by FS perms, deferred.
- **#1699** — de-flake two chronic-slow required smokes (the `unit/static smoke` battery runs long); deferred.
- **#1711 / #1722** — deferred follow-ups.

## [Unreleased]

## [0.16.5] — 2026-06-09

**Zero-touch A2A Rooms mesh** — a feature release on the **v0.16 LTS line** that turns multi-node A2A Rooms into a self-healing mesh: a k8s-controller-style reconcile loop runs on the handoff daemon tick and continuously converges each node's *actual* network state (stable address, tunnel health, peer reachability, applied roster epoch) toward the only human input — *desired* room membership + transport config. New nodes join from a room token, a leader relays member↔member traffic, the roster anti-entropies itself, and `agb a2a net-status` becomes the control-loop's status window. The mesh is **transport-agnostic** (identical on `cloudflare-warp-mesh` and `tailscale`; only stable-address acquisition and tunnel-health probing live behind per-transport adapters) and preserves every receiver security invariant (fail-closed bind proof, HMAC, allowlist, dedupe, `remote_addr`, two-factor admission). Delivered as a 7-lane wave (L0 reconcile framework → L1–L3 self-heal adapters → L4 token-join → L5 leader-relay + roster → L6 net-status v2), each lane gated by per-PR + full-branch agb-dev-codex review **and** the receiver lanes additionally by an independent adversarial skeptic sweep and a patch receiver review; the integration branch was VM-verified on a Linux iso v2 install (fresh `agb upgrade` + 7/7 lane smokes) and exercised by a real-daemon 5-node churn run that confirmed zero-touch convergence of peer-down→rejoin and kick→converge→retire. The full-branch review caught a real cross-lane config-path seam (fixed pre-cut). **Every v0.16.x release is an LTS release** — v0.16.5 does not supersede the prior LTS designation. Same `agb upgrade` path; A2A wire protocol unchanged (`PROTOCOL_VERSION` `a2a-enqueue-v1`); all new mesh state is additive and degrades safe on installs with no rooms/mesh config.

### Added

- **Reconcile control-loop framework (#1705-#1708, L0).** A durable, idempotent `reconcile_once` sequence (`bridge_reconcile_common.py`) layered onto the existing handoff daemon tick: each step is eligibility-gated by a per-step durable backoff (cap + exponential + jitter in `reconcile.db`), event-driven AND periodic anti-entropy, and fail-safe (a raising/erroring step degrades to a no-op + audit, never crashes the tick). The store is created only by the daemon's own attempt path — viewer/status reads never materialize it.
- **Self-heal adapters — stable address, tunnel health, peer reachability (#1705/#1706/#1707, L1–L3).** Per-transport adapters that (L1) detect and converge the node's stable listen address (Tailscale identity-keyed `tailscale ip`; WARP a hardened utun/Mesh address — never a guessed CIDR) and rebind through the single `resolve_bind()` oracle on IP drift; (L2) probe tunnel handshake freshness and bounce a stale WARP tunnel within a bounded budget; (L3) advance a per-peer `UP→SUSPECT→DOWN→(recovery)→UP` hysteretic state machine and record a desired LAN→WARP rebind when the local listen address vanishes. All fail-closed and active-transport-only.
- **Token-bootstrap room join (#1695 P1, L4).** A new node joins a room from a room token: the wire carries only `sha256(token)`, the controller derives a per-pair key via HKDF domain separation (`a2a-room-pair-seed-v1` → `a2a-room-pair-key-v1`) and persists the *derived* key, and auto-pair registers the unknown peer only **after** a valid token + `remote_addr` + HMAC (two-factor admission, all mutation deferred until post-HMAC). Auto-join is env-gated with the default unchanged.
- **Leader-relay + roster anti-entropy (#1695 P2, L5).** A room leader relays member↔member traffic by re-signing with the leader↔target pair key (never reusing the origin signature), with a `relayed_via` loop-guard and a deterministic forward-id for dedupe; `relayed_from` provenance is trusted only after the receiver confirms the authed peer is the cached leader. The roster anti-entropies via a durable `room_roster_outbox` (kick/leave/deny broadcast + reconcile-tick re-broadcast), a monotonic epoch split-brain tiebreaker, and bounded retirement — a kicked member loses access and the mesh converges without operator commands.
- **`agb a2a net-status` v2 — control-loop status window (#1708, L6).** Additive, read-only enrichment of net-status so a human can confirm the daemon is already self-healing: own stable address, room leader + IP, allowed agents, room roster + epoch + last-sync, tunnel freshness, per-peer `UP/SUSPECT/DOWN`, last reconcile tick, auto-recovery counts, and roster-epoch convergence. Strictly additive to the #1697 v1 schema (every v1 field byte-identical), read-only/non-creating (never materializes reconcile.db), no secrets, active-transport-only.

### Fixed

- **Reconcile desired-config writers honor the active `serve --config <path>` (#11573, cross-lane).** The full-branch integration review caught a config-path seam: the stable-address reconcile writer persisted desired listen-address drift to the default config (`a2a.config_path()`) regardless of the custom config a `serve --config <path>` receiver had loaded, so a custom-config daemon drifted the wrong file forever and never converged. The active `config_path` is now threaded into `stable_local_addr` and `peer_reachability_step`, which write `config_path or a2a.config_path()`; default-path behavior is byte-identical.

### Known issues

- **#1722 — relay reachability over-claim** (relay-resistance hardening of the leader-relay path); deferred to a v0.16.x follow-up.
- **#1728 — test-bind state-path guard.** A local test mesh that overrides only `BRIDGE_HOME` can write into the live `state/handoff/rooms.db` because `handoff_dir()` honors `BRIDGE_STATE_DIR` over `BRIDGE_HOME`; a `BRIDGE_A2A_ALLOW_TEST_BIND`-gated guard (refuse-when-state-points-outside-home) + doc is deferred to v0.16.6. Production installs are unaffected (the flag is test-only); the workaround is to also override `BRIDGE_STATE_DIR` (and the rooms/reconcile/inbox/outbox DB paths) under the test home.
- **#1729 — extend the mesh verify matrix** with S2 rebind-*success* and S4/S5 relay live legs on an orbstack registered-agent VM (the loopback churn harness proved detect+fail-closed but couldn't bind a second substrate address or terminate a relay at a real inbox); deferred to v0.16.6.

## [0.16.4] — 2026-06-09

Guard over-block hardening patch over **0.16.3** on the **v0.16 LTS line** — a wave that relaxed several false-deny friction points in the `hooks/tool-policy.py` containment/audit guard, fixed a WARP-mesh receiver crash-loop, and added A2A transport observability. Every lane was gated by per-PR + full-branch agb-dev-codex review **and** independent local adversarial review; the integration branch was VM-verified on a Linux iso v2 install. The dual-review gate caught real read-gate bypasses on the higher-risk lanes (see Known issues for the one lane that was descoped). v0.16.4 is a v0.16.x LTS-line patch — it does **not** supersede the v0.16.3 LTS designation; **every v0.16.x release is an LTS release**. Same `agb upgrade` path; A2A wire protocol unchanged (`PROTOCOL_VERSION` `a2a-enqueue-v1`).

### Added

- **`agb a2a net-status` — active-transport observability (#1697).** A new read-only verb (alias `agb a2a status`) reports the install's `bridge_id` / transport / listen address / receiver (pid + healthz) / substrate health / configured peers / room membership, so a co-resident agent can answer "what transport is this install actually on, and is it healthy?" without acting on a stale Tailscale assumption after a transport switch. **Active-transport-only**: it probes only the configured substrate (a `cloudflare-warp-mesh` install never shells `tailscale`). Zero state mutation, stable JSON shape (`--json`), no secrets in output. Receiver-liveness delegates to the same healthz path as the daemon (so it inherits the #1701 WARP fix below rather than re-reporting a healthy WARP receiver as down).

### Fixed

- **WARP-mesh receiver no longer crash-loops a healthy `/32`-bound receiver (#1701).** On a `cloudflare-warp-mesh` install the A2A receiver binds a point-to-point /32 WARP tunnel IP that cannot hairpin a TCP connection to itself, so the `GET /healthz` self-probe always timed out and the daemon supervisor crash-looped a receiver that was actually serving peers (bricking the node's A2A/rooms). `cmd_healthz` now falls back, **for that transport only**, to a dependency-free socket-held liveness check (a plain `bind()` of the same address:port returns `EADDRINUSE` ⇒ the listen socket is held ⇒ healthy). Tailscale/LAN keep full HTTP wedge-detection (their self-probe hairpins normally).
- **`state/tasks.db` is readable, writes stay denied, and the read-intent classifier is hardened fail-closed (#1690).** The queue DB gained a read-intent carve-out (a `cat`/`stat`/etc. read is allowed; the "use `agb` queue commands" contract still blocks all writes/redirects/mutations, with no admin bypass). Closing that carve-out exposed pre-existing fail-open gaps in the shared read-intent classifier, which were hardened: dynamic-loader/preprocessor env prefixes (`LD_PRELOAD`/`LD_AUDIT`/`GCONV_PATH`/`DYLD_*`/…) are treated as exec surface, Bash 5.3 `${ … }` function-substitution and other shell embeddings fail closed, `awk` is scanned only in its program word, `uniq`/`view`/`file`/`yq` write/exec flag surfaces are gated, and an unresolved path-expansion (`$VAR`/`${VAR}`/`~`/brace) blocks the protected-path carve-out so a variable-spelled forbidden sibling can't slip through. Sibling protected-path gates fire even when the same command also names `tasks.db`.
- **Admin Bash reads of a peer agent's home are no longer falsely denied (#1692).** An admin (`is_admin_agent`) read-intent Bash read of a peer-home file (`cat <peer>/MEMORY.md`) is now allowed, matching the non-Bash Read-tool behavior — but **only** reads, **only** for admin, and **only** peer-home: admin writes/mutations stay denied, non-admin peer-home reads stay denied, the system-class carve-out is preserved, and the team-shared private/secret subtrees stay denied even for admin (least-privilege).
- **Read-only viewer commands are classified read-intent + a prose over-fire is fixed (#1693).** `strings`, `hexdump`, `comm`, `fold`, `expand`, `paste`, and `csvlook` are now read-intent so a benign `strings <roster>` / `hexdump -C <file>` is no longer false-denied by the roster/system-config/queue gates (a write/redirect through a viewer stays denied). The shlex-failure short-needle fallback was narrowed to a redirect/assignment/path **position** test, so a prose mention of `hooks/`/`state/cron/` inside an unbalanced-quote command body (e.g. a `git commit -m` with an apostrophe) is no longer treated as a protected-path reference — while a real redirect/assignment to a protected path stays denied.

### Known issues

- **#1691 — intent-aware credential/shared-alias *mention* gates (descoped from this release).** The attempt to let a non-admin command merely *list/search/mention* a credential filename or protected-path name (while still denying an actual file *open*) hit the same fundamental fragility as the substring-based protected-path gates: distinguishing "opens the file" from "mentions the name" via substring analysis is structurally vulnerable, and the dual-review gate found two distinct real bypasses in the candidate (a broadened peer-home gate and a `find -exec` credential read). It is deferred to be redone as an architecture-level change **bundled with #1709**. Today the affected gates stay conservatively over-blocking, which is the safe default.
- **#1709 — substring protected-path gates miss brace/`$BRIDGE_HOME` path spellings** (pre-existing). To be addressed by the same suffix-matching + obfuscation-fail-close redesign as #1691.
- **#1711 — non-Bash admin Read can access the shared secret/private subtrees** (over-permission vs the Bash least-privilege behavior added in #1692); to be tightened for parity.

## [0.16.3] — 2026-06-08

Stabilization patch over **0.16.2** on the **v0.16 LTS line** — nine fixes surfaced by the v0.16.2 fleet rollout plus an operator-directed `lts` upgrade channel, processed as a 9-lane wave-orchestration (per-lane internal codex + per-PR agb-dev-codex Phase-4 + a full-branch integration review) and VM-verified on a Linux iso v2 install. v0.16.3 is a v0.16.x LTS-line patch — it does **not** supersede the v0.16.2 LTS designation. Same `agb upgrade` path; A2A wire protocol unchanged (`PROTOCOL_VERSION` `a2a-enqueue-v1`).

### Added

- **`lts` upgrade channel — version-line pin for conservative installs (#1687).** `agb upgrade --channel lts` pins an install to the highest non-prerelease tag *within the LTS series* (root `LTS_SERIES` pointer, currently `0.16`) so it doesn't auto-jump off the LTS line when a higher minor ships. A dedicated sticky `state/upgrade/channel` records the choice (precedence: explicit CLI `--channel`/`--version`/`--ref` > special-cases > recorded sticky > legacy `stable`); `AGENT_BRIDGE_UPGRADE_CHANNEL` is transient and `--version`/`--ref` are one-shot (neither overwrites the sticky). The series resolver is full-match (not prefix), skips prereleases, and **fails closed** (clear remediation, non-zero rc) if `LTS_SERIES` is missing/malformed or the sticky write can't be verified — never a silent fallback to `stable` that would slip an LTS install off-line. Default behavior is unchanged for non-`lts` installs.

### Fixed

- **`upgrade --apply` self-restart no longer reads as a failure to the invoking session (#1662).** An upgrade that restarts its own session was observed exiting 137 (SIGKILL of the parent during agent restart), which an invoking wrapper read as failure even though the upgrade had completed. An upgrade-complete marker is now written after the work and before the restart, promoted post-restart, and surfaced in `--json` (atomic tmp+rename, dry-run suppressed) — the marker is the source of truth, so a self-restart exit is expected-success rather than a spurious error.
- **`agb upgrade`/`rollback` lock is held through the full critical section (#1667).** The daemon-control lock token was captured in a `$()` subshell whose fd closed immediately, releasing the lock early and re-opening the concurrent-upgrade race that #1661 was meant to close. The token now lives in a shell global (no subshell fd-close) and the refuse-fast path uses `flock -n` (not the invalid `flock -w 0`); a serialization smoke proves the fd is held across the critical section.
- **cron-runner no longer discards a whole child payload over a missing `summary_short` (#1677).** A `null`/empty/absent/over-long `summary_short` previously produced a fatal error-envelope that threw away the entire cron child result (silent data loss in `state/cron/runs/`). It now derives `summary_short` from the first non-empty line of `summary` with visible truncation (197 chars + `…`). An empty `summary` and a bad `forward_target` stay fatal; the error-envelope path was not generalized.
- **Settings rerender preserves an operator-added `statusLine` (HUD) (#1689).** The isolated-home settings rerender that runs on every upgrade/restart/relink carried forward only an allowlist of user keys, which did not include `statusLine` — so an operator-configured statusLine (e.g. a claude-hud HUD) silently vanished after a bridge upgrade. `statusLine` is now in `PRESERVED_USER_KEYS`, so an operator-added (or `hud_usage_tap`-patched) statusLine survives every rerender; an unrelated user setting still does not leak through.
- **`rematerialize-agent-identity --dry-run` no longer blanks the agent in its JSON (#1670).** The helper ran `shift 5` before sourcing `bridge-lib.sh`, which re-execs Bash 3.2→4 and replayed the original `$@` — blanking the agent argument on the stock-shebang path (dry-run reported `agent=""`). It now slices positionals via `"${@:6}"` and adds a Python-side agent-preserved guard.
- **`link-shared-settings` is idempotent on an iso-owned existing symlink (#1672).** A re-run hit `FileExistsError` when the target symlink already existed and was owned by an iso UID. The link step now catches the idempotent case (resolving the existing link's real path via a fail-closed `sudo readlink -f`); a wrong-target or non-symlink at the path still re-raises.
- **A2A receiver self-heals after an upgrade left it on stale code (#1685).** A pre-v0.16.1 → v0.16.x first upgrade never restarted the tailnet receiver, so it kept running stale code and silently 429'd inbound A2A. A daemon-tick detector (receiver boot marker `state/handoff/receiver-boot.json` vs `last-upgrade.json.updated_at`) now performs a guarded one-shot auto-restart (preflight-before-stop, systemd-aware, fully fail-safe on a malformed marker). Ships with an operator upgrade-guide note for the manual receiver restart on a pre-v0.16.1 first upgrade.
- **Teams `server.ts` emits a clear `EADDRINUSE` diagnostic (#1671).** A port collision now reports `HOST:PORT` + the `TEAMS_WEBHOOK_PORT` knob instead of an opaque stack trace (diagnostic-only — no kill primitive, since cross-UID provenance of the holder is unprovable; the watchdog still self-heals).
- **Deterministic `1659` multijob smoke fixture (#1683).** The cron cadence-health smoke used a wall-clock-relative fixture that failed inside a Sunday-22:00→Monday-09:00 UTC window, intermittently red-failing the required static smoke on unrelated PRs. The fixture is now weekday-independent (fixed `hourly − 4d` offset), verified across 672 synthetic times/week with zero ordering failures, and still catches a latest-run-per-agent regression.

## [0.16.2] — 2026-06-08

Stabilization patch over **0.16.1** — four robustness/perf fixes surfaced by the v0.16.1 fleet rollout (macbook / SYRS mac-mini / cm-prod, the last a Linux iso v2 production host). Every fix was codex pair-reviewed (per-PR + a full-branch integration review) and the integration branch was VM-verified on a Linux iso v2 install with the **flock backend** — the path that exposed two real production bugs the macOS (mkdir-backend) tests could not (see #1661). Same `agb upgrade` path; A2A wire protocol unchanged (`PROTOCOL_VERSION` `a2a-enqueue-v1`).

### Fixed

- **`agb status` is no longer O(cron-run-records × window-minutes) (#1659).** On a host with a large `state/cron/runs/` backlog the dashboard's cron cadence-health walk re-ran the cron matcher tens of millions of times (≈84s CPU on a 5,632-record host — a timeout risk for any `cron`/monitor that shells `agb status`). The pure cron matcher is now memoized (`allowed_values(expr,min,max) → frozenset` behind a bounded `lru_cache`; `field_matches` is a membership test), and `last_cron_run_by_agent` reduces to the latest run per distinct `(agent, job/schedule)` before the cadence check — preserving the in-cadence latch + most-recent-timestamp aggregation bit-for-bit (a multi-job agent with an older-healthy + newer-stale schedule still classifies correctly). Render now scales with distinct schedules, not total historical run rows.
- **`upgrade --apply` preserves its intended exit code when stdout is closed (#1660).** A `BrokenPipeError` at a `migrate-agents` (or any other) JSON emit no longer surfaces as a misleading non-zero exit (observed: exit 144 on a *successful* migration when a concurrent run closed the capture pipe). A new `emit_json(payload, rc)` helper flushes inside `try`, redirects stdout to devnull on `BrokenPipeError`, and returns the command's intended rc — applied to all JSON emit sites. No global `SIGPIPE` handler (a completed migration that only broke at emit exits 0, not 141). Commands that intentionally return non-zero (e.g. `cmd_verify_tasks_db`) keep their code.
- **`agb upgrade`/`rollback` now hold a singleton lock — concurrent runs on the same install refuse fast (#1661).** Previously two `upgrade --apply` against one `BRIDGE_HOME` raced the daemon + agent restart (observed: a 5-process thrash on a real host). A new shared `lib/bridge-lock.sh` (flock-first, `mkdir` fallback) acquires `state/locks/upgrade.lock` for the mutating `upgrade --apply` and non-dry-run `rollback` paths only; the second concurrent run refuses fast with a clear `already running (pid …, started …)` diagnostic (opt-in bounded `--wait`). **This fix also closed two bugs that made the lock non-functional on every Linux (flock) host: `flock -w 0` is rejected by real `flock(1)` (the refuse-fast path now uses `flock -n`), and a bare `exec {fd}>>file 2>/dev/null` had permanently muted the shell's stderr.** Lock release is integrated into the existing upgrade exit-handler; child spawns (daemon/receiver/agent restart) drop the inherited lock fd.
- **An upgrade-conflict sidecar can no longer cascade-fail iso agent launches (#1663).** If an upgrade left a `*.upgrade-conflict` backup (mode 0600, owner-only) inside a dev-plugin source dir, the iso plugin-cache build hit `PermissionError` reading it and aborted the *entire* cache — taking down every iso agent on that plugin (observed as a real outage on a cm-prod iso v2 host). The cache overlay now skips upgrade/VCS sidecars (`*.upgrade-conflict`, `*.orig`, `*.rej`, merge-tool sidecars, `.git/.hg/.svn`) by pattern, and guards each entry so a single unreadable/unknown file is skipped with a warning instead of aborting — **except** required plugin-contract material (`plugin.json`, `server.ts`/`server.js`, `package.json`, `mcp.json`), which now fails loud (`install-failed`) rather than silently shipping a broken plugin.

## [0.16.1] — 2026-06-08

Stabilization + hardening patch over **0.16.0**. A post-0.16.0 fix wave (handed via the SYRS mac-mini and local patches) plus an A2A audit follow-up, iso-upgrade robustness, an MS365 cross-plugin token-refresh path, and a complete `set -u` hardening of the roster-map accessors. Every PR was codex pair-reviewed (the gate caught real defects across the wave — a cross-agent scaffold-write escape, a CLI no-upn exit-0, and the full nounset accessor set); the candidate was verified on a Linux VM (`agb upgrade --channel current` clean on an iso v2 install — 12 agents incl an iso agent migrated with no abort, daemon healthy — plus the per-fix Linux smoke battery). Same `agb upgrade` path; A2A wire protocol unchanged (`PROTOCOL_VERSION` `a2a-enqueue-v1`).

### Added

- **MS365 `get_valid_token` — guaranteed-valid token for trusted in-fleet callers (#1650).** A new ms365 MCP tool **and** a `bun server.ts get-valid-token [upn]` CLI subcommand return a currently-valid Graph `access_token` (transparently refreshing via the stored `refresh_token` when expired/near-expiry, reusing the existing pre-call expiry check + `SingleFlight` coordination) so a sibling caller (e.g. the CRM proxy) that previously read the token file directly — and so used a stale token — gets a fresh one. Returns `{ upn, access_token, expires_at, expires_in_seconds }`; **never** returns the `refresh_token`; emits a redacted `ms365_token_issued` audit. Live-verified end-to-end on real M365 (expired token → auto-refresh → fresh token, no `Authentication required`).
- **Post-restart auto-wake for auto-restarted sessions (#1639).** After an upgrade/daemon/watchdog auto-restart, the fresh Claude session no longer sits idle: `bridge-start.sh` propagates `BRIDGE_AUTO_RESTART_WAKE=1` on non-attach launches and `bridge-run.sh` fires a first-turn inbox-bootstrap wake, scoped to non-cron claimed/blocked work (a cron-dispatch-only queue never spuriously wakes). An operator's interactive `--attach` launch never auto-wakes; a pending `NEXT-SESSION.md` handoff suppresses it; an empty queue on a long-lived agent injects nothing.
- **`agb urgent --from <sender>` sender-attribution override (#1640).** `agb urgent` accepts `--from` to set the sender on the urgent interrupt (wired through `bridge-send.sh`).
- **`agb list` marks controller-blind iso agents as `[iso]` (#1637).** An iso v2 agent whose workdir the controller cannot stat now renders as `[iso]` instead of a blank/error column.

### Fixed

- **iso-agent upgrades no longer abort on the backup scan (#1635).** The upgrade backup scan graceful-skips iso-owned files the controller cannot stat (per-UID isolated tree), instead of aborting the whole upgrade with a `PermissionError` traceback — the v0.16.0 upgrader aborted here on an iso v2 install. The `remember()`/`exists()` probe and the copy path are both tolerant.
- **Agent `_template` scaffolding propagates to the workdir on upgrade (#1636).** `rematerialize-agent-identity.sh` now also add-missing-only copies the non-identity `_template` scaffolding (`.claude/commands/`, `raw/captures/*/.gitkeep`, `session-type-files/`, `codex/` except `AGENTS.md`) to the agent workdir — previously only the identity files propagated, so existing agents silently missed new scaffolding across upgrades. Skip-existing (never clobbers a user-customized file), reuses the iso-UID write path, graceful-skips on PermissionError, and a per-agent `target_dir` containment guard refuses any symlink ancestor that would escape the agent's own workdir (cross-agent write protection).
- **Complete `set -u` hardening of roster-map accessors (#1627).** Every `BRIDGE_AGENT_*` associative-map access on the launch hot path — per-agent reads, `[[ -v … ]]` predicates, key-iterations (`${!MAP[@]}`), and existence probes — is now guarded by `bridge_var_is_assoc`, so an undeclared or scalar-clobbered map can no longer abort a fresh agent launch/restart under `nounset` (`<agent>: unbound variable`). Closes the partial #1626 fix (which covered only `source`/`provenance`/`precompact_notify`); now also `loop`, `continue`, `start_policy`, `model`, `effort`, `permission_mode`, the class/action key-iterations, and the `exists`/`session_id`/`created_at` probes (which previously used a bare `declare -p` / unanchored grep that false-positives on a scalar).
- **settings.json cosmetic-only diff no longer forces a spurious `.upgrade-conflict` (#1638).** An upgrade where `.claude/settings.json` differs only by hook-event ORDER or `python3` vs `/usr/bin/python3` is treated as semantically identical (preserve live, no conflict); a genuine hook-set change still conflicts. Tight interpreter allowlist (`python3`/`/usr/bin/python3` only).
- **A2A inbound backpressure fails OPEN on a count-read error (#1623).** A WAL/read error in the open-task count no longer fail-closes inbound A2A with a 503 (which silently blocked a peer); it fails open.
- **A2A sender deliver-loop per-row guard + outbox retry/backoff (#1628, #1618).** One malformed outbox row can no longer abort the whole deliver batch; a manual retry resets the attempt counter so backoff restarts.
- **A2A fresh-arrival nudge + read-only nudge DB (#1633, #1634).** An inbound handoff posts a fresh-arrival nudge marker so the daemon wakes the target; the daemon nudge helpers open the queue DB read-only (no empty-DB fabrication).
- **A2A receiver cycles on `--restart-daemon` (#1614); `GET /healthz` exempt from the request semaphore (#1645).**
- **Deferred pending-attention staleness — one unified task-ref gate (#1619).** A deferred tmux inject for a task that has since been done is dropped at flush (the gate now covers arrival / `task #N` / completion headers, not just `[task-complete]`); read-failure/missing-task fail-safe to replay.
- **Daemon socket loop-restart CI flake hardened (#1644).** The `scripts/smoke/daemon.sh` loop-restart no longer aborts with a cryptic `first_pid: unbound variable` on a slow cold-boot — `first_pid` is initialized and the initial-listener wait budget widened.
- **Runtime/cron/wiki:** shared-mode agents use the operator HOME (keep `CLAUDE_CONFIG_DIR` per-agent) + upgrader credential consolidation (#1622); migrate-agents roster-restricted, skips non-roster orphans (#1611); cron capture-runner hotfixes folded in (#1625); upgrade-complete wiki report step + indented-code wikilink false-positives fixed (#1616).

## [0.16.0] — 2026-06-07

Official **0.16.0 stable**. Built on the rc1-rc3 foundation (A2A Rooms P1-P4, the daemon-singleton supervision redesign, and A2A inbound backpressure + the B1-B8 receiver security audit — see the rc sections below) plus a post-rc3 feature/fix wave. Every PR was codex pair-reviewed; the wave was verified on a Linux VM (`agb upgrade` dry-run clean + the full per-feature smoke battery green). Same `agb upgrade` path; A2A wire protocol unchanged (`PROTOCOL_VERSION` `a2a-enqueue-v1`).

### Added

- **Bounded AskUserQuestion for autonomous agents (#1569).** An autonomous bridge agent that calls the interactive multiple-choice `AskUserQuestion` tool no longer hangs on the picker: a PreToolUse intercept denies the tool and routes the question to the agent's channel with a bounded wait (`BRIDGE_ASKUSERQUESTION_WAIT_SECONDS`, default 30s), resolving a channel reply (numeric → option label) or taking an autonomous fallback (reversible → proceed-with-note, high-stakes → blocked). `BRIDGE_ASKUSERQUESTION_BOUND=0` restores the raw picker. Live-verified in a real session: no picker renders, no hang, other tools unaffected.
- **A2A whole-room fan-out — `agent-bridge a2a send --room` (#1594).** Send one message to every eligible member of an A2A room (aliases `room send`, `room talk --fanout`). Sender membership is OS-actor anchored from this node's roster cache / `rooms.db` (a non-member is refused before anything leaves either leg); same-node members deliver via the durable queue, remote members via the existing room-scoped A2A path (HMAC + room epoch); per-recipient partial-failure reporting. The receiver fail-closed `room_scoped_check` is untouched — the sender-side gate is purely additive.
- **A2A Cloudflare One / WARP-Mesh transport (#1595).** A2A is now transport-pluggable: alongside the default Tailscale transport, a `cloudflare-warp-mesh` kind binds the receiver to a WARP Mesh device IP only after proving the IP is assigned to a real local interface AND WARP is connected + registered/enrolled (a nonzero `warp-cli` registration probe can only confirm a negative, never prove enrollment — fail-closed on any uncertainty; CIDR shape is never proof). Tailscale + raw-IP back-compat preserved exactly; HMAC / dedupe / `remote_addr` / allowlist / `room_scoped_check` unchanged.
- **Agent Bridge operating-manual index skill (#1604).** A pointers-only "operating manual" index skill (situation → SSOT routing table + absolute-rules checklist + SSOT document map) installed for all Claude session types, with a Codex project-`CLAUDE.md` pointer — one entry point to the operating canon with zero doctrine duplication. SSOT pointers resolve across engines/session-types (the isolated-home renderer normalizes `~/.agent-bridge/` → the absolute `BRIDGE_HOME`).

### Fixed

- **Stop-hook drain skips daemon-owned `[cron-dispatch]` work (#1596).** The Claude/Codex Stop-hook inbox-drain no longer re-enters the model for daemon-owned cron-dispatch rows the daemon itself closes (a wasted LLM turn). One shared actionable predicate filters them (title-prefix as SSOT; a `cron:`-actor fallback only for untitled rows; a `[cron-followup]` carve-out), iterates the queued/claimed lists so a real task sitting behind a cron row still blocks, and re-checks/fails-open as late as practical. Queued-first, claimed anti-abandonment, loop-guard, and fail-open invariants intact; a nonzero `find-open` exit fails open even when its stdout is parseable.
- **`agb upgrade conflicts adopt` guards against marker/unparseable sidecars (#1601).** `conflicts adopt` now scans for unresolved conflict markers and syntax-checks the `.upgrade-conflict` sidecar before overwriting the live file, and refuses (controlled nonzero, no traceback, sidecar preserved) when the prior-live `.pre-adopt` backup cannot be created — so adopting a marker-laden or backup-less sidecar can no longer brick the upgrader's own backend. `--force` escape hatch retained.
- **`agb upgrade --ref <tag> --dry-run` previews the requested ref (#1602).** The dry-run preview now reads the requested ref's tree via `git ls-tree` / `git show` (no checkout, no working-tree mutation) instead of previewing against whatever ref the source checkout happened to sit on. The `--apply` path is unchanged (it still checks out the ref) and dry-run/apply parity is enforced.
- **migrate-agents re-materializes stale workdir identity copies (#1598).** `agb upgrade` migrate now re-materializes each agent's workdir identity copy (not just the home managed-block), through the iso safe-writer with a shared-workspace guard and a backup-before-overwrite, so a copy-model agent's workdir no longer drifts stale behind its home.

### Deferred — live validation pending infra

- **2-node A2A rooms fan-out live test (#1594)** and **2-node WARP-Mesh transport live test (#1595)** are deferred until the multi-node / WARP-enrolled infrastructure exists. Both are smoke-verified (single-node + the full fail-closed battery) and adversarially reviewed; the rooms fan-out reuses the previously-validated room-scoped wire path. Only the cross-node / WARP *live* leg is pending — the code is shipped and gated fail-closed.

## [0.16.0-rc3] — 2026-06-06

Third release candidate for **0.16.0**. The marquee is **A2A inbound backpressure + security-receiver stabilization** — a live incident where "A2A that used to work stopped working" root-caused to a backpressure ratchet bug, fixed alongside an 8-finding adversarial audit of the A2A receiver — bundled with six accumulated fixes / hotpatch-upstreams. Each PR codex pair-reviewed; the A2A audit went through two adversarial NO-GO rounds (a `Retry-After: inf` remote-DoS among the catches) before GO, and the cross-PR `_schedule_retry` reconciliation was verified on both smokes. Same `agb upgrade` path; no wire-protocol/schema change vs rc2 (`PROTOCOL_VERSION` `a2a-enqueue-v1` unchanged).

### Added / Fixed

- **A2A inbound audit — B1-B8 delivery defects (Track A, #1589).** A live `crm-dev → patch` outage where inbound A2A silently stopped was root-caused to a **backpressure all-time-count bug**: the open-task count counted the entire dedupe history instead of currently-OPEN tasks, so any peer past the cumulative cap got **503-rejected forever** (cm-prod permanently blocked at 200). The fix counts only OPEN tasks via a separate read-only connection — the original ran `ATTACH`/`DETACH` inside an open `BEGIN IMMEDIATE`, which fails deterministically on SQLite 3.51 and 503-rejected every capped peer, a remote-triggered DoS the CI smoke missed because it only exercised the autocommit path. An adversarial multi-agent audit surfaced seven more (B2-B8): large-body reaper corruption on a symlinked home (resolved-vs-unresolved path compare), crash-consistency (no task duplication on a mid-deliver crash), a stale-pending backpressure + room-gate bypass, a recover `created_by` mismatch, and a slow-loris pre-auth header DoS (absolute request deadline). The fail-closed receiver bind / HMAC / `remote_addr` / allowlist / dedupe boundary is untouched.
- **Two-cap Retry-After (Track A reconciliation with #1575-B).** A sender's untrusted peer `Retry-After` is honored as a hard delivery floor (respecting receiver backpressure) **bounded by a separate `delivery_max_retry_after_seconds` cap (default 600s)**, while the exponential backoff keeps its own `delivery_backoff_ceiling_seconds` cap (default 120s). The two caps coexist because they defend different threats — backoff-ceiling protects us (DoS-by-sleep), the Retry-After cap protects the peer (backpressure honor). The opt-in trust gate requires an exact boolean `True` (a string `"false"` no longer fails open), a non-finite `Retry-After` (`inf`/`nan`/`1e400`) can no longer crash the deliver loop, and a trusted `Retry-After` is sanity-capped at 3600s.
- **A2A sender outbox backoff ceiling cap (#1575-B).** The outbox retry backoff default ceiling dropped 3600s → 120s with a configurable `delivery_backoff_ceiling_seconds` (env `BRIDGE_A2A_BACKOFF_CEILING_SECONDS`), so a recovered peer's high-attempt retry rows no longer idle dormant for 16-60 minutes.
- **smoke-test LIVE-state isolation (Track C, #1588).** `scripts/smoke-test.sh` no longer leaks into the operator's live `~/.agent-bridge` state — it clears smoke-owned controller path vars and exports temp-rooted state / daemon-log / task-db, asserting every controller path stays under a temp prefix (fixes the recurring `#365 / SCRIPTDIR_STARTUP_OK` false-failure on clean checkouts).
- **Daemon routine idle-nudge gates on real interaction, not bare attach (#1568).** The periodic queued-task idle-nudge previously skipped whenever any tmux client was attached, so under a persistent-client multiplexer (every tab attached) the proactive nudge never fired. It now skips only when the session is genuinely busy (`bridge_tmux_session_inject_busy`), matching the urgent path; an attached-but-idle session is nudged. (#1568 stays open for the fuller recent-keypress/focus design.)
- **`lib/bridge-core.sh` de-heredoc + `lib/bridge-path-utils.py` (#1466).** Upstreams a lost footgun-#11 fix — the remaining heredoc-stdin / here-string sites in `bridge-core.sh` (a Bash 5.3.9 `read_comsub`/`heredoc_write` deadlock class) are converted to `python3 -c` one-liners / process-substitution / a new file-as-argv path helper.
- **agent-bridge dispatcher skips roster load for pure queue verbs (#848).** `show`/`claim`/`done`/`cancel`/`update`/`handoff`/`summary`/`create` no longer pay the `bridge_load_roster` cold-start cost — they exec `bridge-task.sh`, which owns its own lazy roster loading (`handoff` target validation preserved).
- **PreToolUse hook hardening (#1574).** Fixes a Python 3.12+ `SyntaxWarning`, pins the hook interpreter, de-duplicates hook registration, and narrows a #341 config-path false-positive that over-blocked legitimate `shared/` report writes — scoped to a single decidable inert quoted-heredoc-write shape (three adversarial bypass classes verified still denied; the credential guard is not weakened).

## [0.16.0-rc2] — 2026-06-06

Second release candidate for **0.16.0**, landing the **#1563 daemon supervision redesign** — a crash-only supervisor that self-aborts a genuinely wedged scheduler tick (and never a healthy-but-busy one), an admin-liveness escalation path that escalates instead of self-healing, and an A2A-receiver health gate that ends the ~9-minute bind-fail crash-loop — plus a confirmed watchdog-scan daemon wedge, the slow-tick per-agent-pass cost, and A2A sender-side diagnosis/recovery hardening surfaced from a live tailnet incident. Eight PRs (PR-1..8), each codex pair-reviewed; the Phase-4 gate caught a real defect on nearly every code PR. Same `agb upgrade` path; no schema/contract change vs rc1.

### Added / Fixed

- **Daemon singleton hardening — owner-record with start-time-proof eviction (PR-1, #1564).** The daemon singleton now records its owner with an absolute process start-time proof, so a stale lock from a dead PID (or a recycled PID) is evicted deterministically instead of either blocking a legitimate start or letting two daemons race. Preserves the #1388 fd-cloexec guard and the #1463 launchd-keepalive no-thrash invariant.
- **Daemon audit false-positive fix (PR-1b, #9882).** Two audit-shape bugs: (A) `self_check` escalation is now gated on a positive proof of a second live daemon rather than the absence of a negative, and (B) `daemon_started` is emitted exactly once per boot (single-emit invariant) — so the audit trail no longer shows phantom double-starts or self-check escalations against a healthy singleton.
- **T1 runner-process self-abort + T0 OS-init crash-only restart (PR-2, #1572).** Each scheduler tick runs under a runner-process supervisor with a deadline of **max-step-budget + grace** (NOT nudge-latency); a genuinely wedged unbounded step self-aborts (the daemon exits non-zero and the OS init system restarts it crash-only), while a healthy long step under its own bounded timeout — heartbeat-refreshed before AND after — is never aborted (the flapping-monitor negative control). The cross-tick escalation counter is persisted to a per-key state file so it survives the supervised-child boundary instead of resetting every tick.
- **T2 admin-liveness escalation — escalate, don't self-heal (PR-3, #1576, #9819 A/B).** When the daemon's mechanical checks detect a down admin/peer it enqueues a durable admin task instead of trying to fix it. The admin-liveness predicate is conservative: `alive` for ANY live session (working/starting/picker-blocked/idle), `down` only with no live session AND a file-backed daemon heartbeat stale past `BRIDGE_DAEMON_ADMIN_DOWN_STALE_SECS` (default 900s) — a busy/long-turn admin is never classified down. A failed escalation task-create is now audited (`daemon_escalation_task_create_failed`) with retained retry state (no swallowed `|| true`, no hot-loop); the admin's codex pair (patch-dev) is the recipient only after the admin-down predicate holds; an MCP-liveness give-up escalates to an admin task, and an admin-self give-up routes to patch-dev (audit-only fallback when no pair exists).
- **A2A receiver health gate — kill the ~9-minute bind-fail crash-loop (PR-4, #1573, #9813 + #9819 C).** The receiver supervisor now classifies the exit cause (transient bind/network availability vs auth/config) and applies a bounded backoff + a circuit breaker keyed by (peer/config, error_class), escalating once per cooldown; a successful bind resets the breaker, and a real exit after a previously-healthy bind restarts promptly. A loadable-but-empty-peer-secret config is HELD as `auth_config` (not thrashed through the transient backoff) — the exit-cause override reuses the receiver's full `load_config` + `validate_config_peer_secrets` contract. The fail-closed tailnet bind proof / HMAC / remote_addr / allowlist / dedupe are byte-for-byte unchanged.
- **iso v2 urgent instant-wake for always-on isolated agents (#9981, via #1571).** `agent-bridge urgent <iso-agent>` previously failed to write the pending-attention instant-wake marker into the isolated agent's data dir (the controller is not a member of the `ab-agent-<a>` group), so urgent silently degraded to poll-latency (~1–2 min) for iso agents. The pending-attention spool is now anchored on the controller-owned `state/agents/<a>/` leaf (which the controller can write) and group-published readable for the iso agent to consume; the old 200-retry-then-give-up is replaced by a fast-fail one-warning. Durable queue delivery was always intact; this restores the instant-wake.
- **Daemon-supervision false-positive-control integration teeth + docs (PR-5, #1577).** A new `1563-pr5-fp-control-matrix.sh` integration smoke proves the combined supervisor never punishes a healthy daemon while still catching real wedges (healthy-long-step → no abort, wedged-unbounded → abort within max-step+grace, stale-timer disarmed between ticks, singleton race → one survivor, escalation task-create-fail → audit+retry, admin-self MCP-giveup → patch-dev, A2A transient → backoff vs real-crash → bounded-restart). Docs: corrected the A2A Rooms P4 status (leader/invite/join/roster-broadcast IS wired and multi-node-verified) and documented the daemon supervision contract + the rc1 sealed-paste / spool-reaper / stop-drain features.
- **Watchdog drift-scan no longer wedges the daemon (PR-6, #1580).** The daemon's watchdog gate called `bridge-watchdog.sh scan` with no timeout before the 30s-bounded `--json` call, so a scan that hung in a directory-walk loop blocked the daemon main loop forever (confirmed live with `sample` — killing the hung `bridge-watchdog.py` child resumed the tick). Both scans now route through a new `bridge_run_command_with_pgroup_timeout` (`BRIDGE_WATCHDOG_SCAN_TIMEOUT_SECONDS`, default 30s) that process-group-kills the hung scan subtree (no orphan), and the new ceiling is coupled into the PR-2 self-abort deadline so a healthy operator-raised scan is never false-aborted. (The required-smoke CI suite, which grew across these daemon PRs, also had its `unit/static` timeout raised 15m → 20m.)
- **Slow daemon ticks — cadence-gate the expensive periodic per-agent passes (PR-7, #1581, closes #1579).** `cmd_sync_cycle` ran ~33 passes serially, several iterating every agent every 5s tick, so `tick_age` drifted to 30–45s on a real roster. The expensive periodic health/housekeeping passes (channel-health, plugin-liveness, memory-refresh, stall-reports, unclaimed-queue/marker sweeps, nudge-late-success sweep, context-pressure-scan, crash-reports) are now cadence-gated to ~30s (each env-overridable) via a new `bridge_daemon_pass_due` helper, while every time-critical delivery/escalation/liveness pass (queue-gateway, attention-flush, cron-dispatch, A2A ticks, nudge, admin-liveness escalation, MCP-liveness give-up recovery, heartbeats) stays every-tick. Each gated pass refreshes the PR-2 progress heartbeat before its due-check, so a skipped tick never false-aborts.
- **A2A diagnosis + recovery hardening (PR-8, #1582).** Surfaced from a live tailnet outage where the death was environmental but the bridge's diagnosis (both sides blamed each other's receiver) and recovery (exponential backoff kept messages dormant for tens of minutes after the peer recovered) prolonged it. Adds a directional leg classifier (`agb a2a diagnose-stuck` → `peer_receiver_unreachable` / `local_tailnet_degraded` / `peer_tailnet_degraded` / `transport_dead_path_unknown`, from local healthz + peer-TCP + tailscale tx/rx); a **probe-gated, transition-gated backoff recovery reset** (when the stuck-scan sees a retry row and a TCP probe to that peer succeeds, it resets the peer's retry rows to deliver immediately — still-unreachable peers keep their backoff + dead-letter, leased rows are never touched); an actionable stuck-alert body (`next_attempt_in_seconds` + last TCP-probe result + a "TCP healthy but backoff-waiting" line); and ack history preservation (the last error is no longer NULLed on ack). TCP `peer:port` is the health oracle (never `tailscale ping`); there is **no** automatic `tailscale up`/`down`; the fail-closed receiver bind/HMAC/allowlist/dedupe boundary is untouched.
...
- **First `agent create --isolate` for a linux-user Claude agent could auto-start before its per-agent credential was seeded → authless first session / "Channels not available" (#1520b, PR #1531).** The create path now seeds the per-agent credential during isolation prepare (after `bridge_linux_prepare_agent_isolation`, before the ~20s daemon auto-start window) so the agent's first managed session starts with working auth instead of racing the daemon. Linux-user isolation only; non-fatal (the agent is held from authless auto-start until the credential is present).
- **First `agent create --isolate` left the six identity profile files (SOUL/CLAUDE/SESSION-TYPE/MEMORY/MEMORY-SCHEMA/TOOLS.md) at `iso-uid:<controller-group> 0600` instead of `ab-agent-<a>:0660`, so the controller/watchdog could not read them (#1520c, PR #1532).** The controller's stale supplementary-group cache (KNOWN_ISSUES §28) blocks the #1506 recursive normalize from traversing the freshly-`chown -R`-ed `2770 ab-agent` workdir, so the profile files were missed on first create (the workdir dir was correct; only the files were stranded). A narrow, **root-forced** publish now sets exactly those six basenames to `ab-agent-<a>:0660` after prepare's `chown -R`, independent of the controller's group cache. The publish is **TOCTOU-safe**: a standalone root helper opens the workdir as an `O_DIRECTORY|O_NOFOLLOW` directory fd, `openat()`s each basename `O_NOFOLLOW` (a planted symlink is refused via `ELOOP`, never followed), `fstat`-verifies a regular file owned by the iso UID, then `fchown`/`fchmod`s the **open fd** — no profile pathname is re-resolved after the check, closing the swap-between-check-and-mutation race. It deliberately excludes HEARTBEAT.md (controller-owned), CHANGE-POLICY.md (shared symlink), and the v3 channel-state `.env`; it is non-fatal (warn + `profile_publish_failed` audit row on any per-file failure, including a refused symlink). The watchdog's `classify_scan_error_category` now anchors the expected group to the **workdir directory's own group** (the setgid-inheritance contract) rather than string-swapping the iso username, so a long agent name (OS user truncated to 32 chars vs the hash-truncated group) is no longer misclassified as a publish gap. Because `isolate --reapply` shares `bridge_linux_prepare_agent_isolation`, it runs the same publish — giving operators a native recovery path for an already-stranded agent.

## [0.16.0-beta3] — 2026-06-04

A **beta** on `0.16.0-beta2` bundling a new-items wave: the marquee items are **per-agent Claude config isolation for shared-mode agents (#1520)** — hardened across multiple adversarial review rounds down to the BASH_ENV/allexport process-startup boundary — and a **generic plugin `requires` resolver (#1528)** so an agent created with one plugin channel auto-provisions the channels that plugin declares it needs. Also lands the multi-user **Teams/M365 OAuth callback demux (#1524)**, three isolation/upgrade fixes (#1516/#1517/#1518), and the hermetic-CI fix that ends the recurring `unit/static smoke` false-red (#1527/#1509). Every item is codex pair-reviewed and CI-green; same `agb upgrade` path, no schema/contract change vs beta2.

### Added

- **Generic plugin `requires` resolver on `agent create` (#1528).** A plugin's `.claude-plugin/plugin.json` may now declare an optional top-level `"requires"` array of canonical channel specs (`plugin:<name>@<marketplace>`); the create path transitively expands them into the resolved `--channels` set (BFS with ordered dedupe, cycle detection, depth cap, and warn-and-continue on an unresolvable dependency — create is never blocked), emitting `[info] <plugin> requires <…> — adding`. This fixes the half-provisioned-agent trap where forgetting a dependency channel yields an agent that looks created but can't authenticate or call its API. **Zero domain hardcoding in core** — it forwards only whatever the manifest declares; a no-`requires` channel set is byte-identical to before. `--dry-run` shows the expansion. Introduces create-time plugin-manifest read (previously isolation-time only) via the existing `bridge_resolve_plugin_install_path`.
- **Multi-user Teams/M365 OAuth `/auth/callback` demux (#1524).** When a single Azure bot app fronts N per-agent listeners, the OAuth callback (which carries only `?code&state`, no agent identity) couldn't be routed. The ms365/teams plugins now embed the agent id in the OAuth `state` as `<BRIDGE_AGENT_ID>.<uuid>` (validator allows `.`, rejects `/`+backslash; `AGENT_TAG` strictly `[A-Za-z0-9_-]{1,64}`), so a front router can demux `/auth/callback` by the `state` prefix while single-listener installs keep a plain uuid (no regression). Shipped to the marketplace as teams + ms365 `0.1.1` (#1526). Live-validated on a production multi-user deployment.

### Fixed

- **Shared-mode `agent create` Claude agents launched without `CLAUDE_CONFIG_DIR` → per-agent config/MCP/credential isolation silently ignored (#1520).** A shared (non-linux-user) Claude agent's final launch never set `CLAUDE_CONFIG_DIR`, so the launched `claude` read the operator-global `~/.claude.json` instead of its pre-seeded per-agent `<agent-home>/.claude`. The launch now exports `CLAUDE_CONFIG_DIR` (and **only** that — HOME stays the shared home; Codex agents excluded; the iso/v2 secret-env path untouched) for shared Claude launches. Auth-continuity is preserved across the switch: the per-agent credential is **seeded on demand** before the export (single-agent `claude-token sync`, working for static **and** dynamic agents), keychain-free (apiKeyHelper) auth is export-safe without a credential file gated on a **proven** keychain-free state (Darwin + validated settings/helper/registry — never the platform-blind enable flag), and if no credential can be seeded the export is skipped (one-time warn, operator-global fallback) so a previously-authed agent is never stranded. Hardened across adversarial review down to the startup boundary: the keychain-free **proven flag** is `unset` at true process entry (drops any inherited export attribute before the first fork/re-exec), and an inherited `BASH_ENV` running `set -p -a` can no longer leak the ambient-secret captures or the proven flag — `builtin set +a` clears inherited allexport unconditionally at entry, before STEP A, so a skipped (caller-spoofable, defense-in-depth) `bash -p` re-exec cannot reintroduce the leak. The residual `builtin`-keyword shadow is the pre-existing same-UID #1454 boundary, not a new regression.
- **`upgrade --apply` silently DOWNGRADED a beta install to the latest stable (#1516).** With no `--ref`, the `stable` channel skips pre-release tags, so a beta install would jump *backward* to the newest stable on a routine `upgrade --apply`. A semver downgrade guard (new `bridge-release.py compare` + `--allow-downgrade`) now aborts a backward move (exit 64 + JSON), scoped to the stable+`--ref` path, after capturing the installed version and before checkout/pull.
- **First `agb room create` on a fresh iso host failed closed (#1517).** With no `rooms.db` yet, the actor resolution had nothing to anchor the controller against and returned `actor_unresolved`. `agb room create`/`adopt-all` now controller-only auto-bootstrap the **canonical** `rooms.db` (derived from the task-DB home, never an env-redirectable path), idempotently, before actor resolution.
- **Dead `chmod 0755` on the legacy mirror clobbered by later normalization (#1518).** A #1513 follow-up: the layer-2 `chmod 0755` on the legacy mirror dir was overwritten by the later recursive isolation normalization (the mirror ends `2750`, still iso-traversable) — dead code. The isolate path now applies the mirror leaf mode **last** (after the grant-matrix + install-tree passes) and asserts the final iso-traversable state, covering the split-root layout the data-root passes don't reach.

### Internal / CI

- **`unit/static smoke` recurring false-red fixed — it was a chronic job timeout, not a content flake (#1527, closes #1509).** The required-smoke suite legitimately runs ~9.5–10m on `ubuntu-latest`, but the job capped at `timeout-minutes: 10`; normal runner variance (+20s) tipped it over and GitHub reported the `timeout-minutes` kill as conclusion `cancelled` — read as the "kappa flake" but actually a boundary timeout that admin-blocked several otherwise-green PRs. Raised the cap to **15m** (matching `integration-smoke`). Secondary: the `beta5-2-kappa-state-audit-reconcile` static-introspection assertions now snapshot every source-under-test once via `git show HEAD:<path>` (object store, immune to working-tree mutation), closing the source-mutation-race class.
