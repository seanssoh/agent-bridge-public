...
## [0.16.13-rc1] — 2026-06-16

**v0.16.13 LTS (RC1) — upgrade-safe picker-sweep migration + AskUserQuestion hard-ban + modal-blocker detection, plus CI cold-boot de-flake and doc/upgrade hardening.** The marquee fixes close a **picker-sweep cron that vanished on upgrade** (a working cron deleted before its replacement was confirmed) and make the interactive `AskUserQuestion` picker — which silently deafens a long-running agent to A2A/nudges — a hard ban across every bridge Claude agent including dynamic-vanilla ones. RC1 is a prerelease for the VM + macbook-patch + cm-prod upgrade-test sequence; the conservative `lts` channel stays on v0.16.12 until v0.16.13 is cut official.
