...
**v0.16.15 LTS — stability hotfix: a single poison request can no longer silently kill the whole fleet's queue gateway (#1949, cm-prod F8).** Closes the second of the two P1s from the cm-prod v0.16.13 post-upgrade RCA (F1 — the systemd-`--user` enable-linger 27h daemon-down — shipped in v0.16.14). Cut directly to official (not the new RC-soak path) given the live daemon-stability impact on installs already upgraded to v0.16.13 / v0.16.14. Supersedes `v0.16.14` as the `lts` / Latest head; prior v0.16.x LTS designations remain valid.
