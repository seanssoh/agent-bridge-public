...
- **Caller audit verdict**: all 3 production callers (`lib/bridge-state.sh:256` / `:3674`, `bridge-sync.sh:146`) already thread `_iso_sudo_user`. Added `BRIDGE_TEST_TRACE_DETECT_CALLERS=1` env var to prove this on demand (stderr `[detect-trace] fn=<helper> caller=<fn> os_user=<value>`).
