...
LOG_FILE="$LOG_DIR/call-${DATE_KST}.log"
REQUEST=""
FROM_AGENT=""
DISCORD_CHANNEL=""
...
REQUEST="${REQUEST:-일일 시스템 점검을 실행해줘. CHECKLIST.md의 모든 항목을 확인하고, 이상 있으면 수리하고, MEMORY.md에 기록하고, 결과를 stdout으로 간단히 정리해줘.}"
FROM_AGENT="${FROM_AGENT:-cron}"
...
echo "[$TIMESTAMP] Patch called (from=$FROM_AGENT): $REQUEST" >>"$LOG_FILE"
...
cleanup() {
  ...
  echo "[$finished] Patch call finished (from=$FROM_AGENT, exit=$code)" >>"$LOG_FILE"
  ...
  if [[ "$FROM_AGENT" != "direct" && "$FROM_AGENT" != "cron" && -n "$report_file" ]]; then
    send_result_task "$FROM_AGENT" "[patch] CLI result" "$report_file" || true
  fi
...
CLI_OUTPUT_FILE="$(mktemp /tmp/patch-output.XXXXXX)"
AGENT_SESSION_DIR="$PATCH_HOME/sessions/$FROM_AGENT"
mkdir -p "$AGENT_SESSION_DIR"
[[ -f "$AGENT_SESSION_DIR/CLAUDE.md" ]] || ln -sf "$PATCH_HOME/CLAUDE.md" "$AGENT_SESSION_DIR/CLAUDE.md" 2>/dev/null || true
...
if [[ -z "$_singleton_suppress_lib" ]]; then
  echo "[call-patch][fatal] #17957 singleton-channel-suppression.sh not found; refusing to launch the disposable Claude child (would risk telegram/discord poller theft)" >&2
  exit 1
fi
