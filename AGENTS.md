...
- **Every v0.16.x release is an LTS release.** The current LTS-line head is **v0.16.4** (2026-06-09). When a new v0.16.x patch ships it becomes the line head but does not revoke the prior patch's LTS status. An LTS is a blessed stable tag (release title `(LTS)` + README marker) for conservative/production installs; v0.16.2 was the first declared (2026-06-08); both stand.
- **One line until features diverge** — the v0.16.x hardening sequence (v0.16.4, …) rides `main` and *is* the LTS line; do not fork a maintenance branch until the first new *feature* lands. Fork trigger: branch `release/0.16-lts` from the latest `v0.16.x` tag and bump `main` to `v0.17.0-beta`. New features → beta first, never onto the LTS line.
...
- **`release/X.Y-lts` is protected — a force-push is gated on three requirements.** A `--force`/`--force-with-lease` push to the LTS branch (the sanctioned way to reset a drifted line to its tag) is allowed **only** when all three hold, each proven necessary in the 2026-06-27 reconcile — this absorbs and strengthens the prior CI-green force-push safeguard: **(a) candidate-branch CI is green** — push the intended history to a candidate branch/PR first and let the full matrix, including the `lts-line-ancestry-check` gate, pass before touching the live ref; **(b) the patch-dev codex gate is `implement-ok`** (§2), so an ancestry reset can't silently drop or re-order a qualified backport; **(c) `git ls-remote origin release/X.Y-lts` is re-verified immediately after the push** to confirm the remote tip equals the SHA you intended to publish. An un-verified force-push leaves the branch HEAD broken even when the tagged release is fine, shipping invisibly to the next patch cut (this class recurred 3×). See [`docs/release-lines.md`](./docs/release-lines.md) §"Branch hygiene" and §"LTS line ancestry invariant". Force-pushing **another agent's** branch remains forbidden (§4).
