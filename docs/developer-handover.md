...
**Dynamic Codex = vanilla Codex CLI + bridge comms only (#1899).** #1890의 Codex
sibling. `engine == codex && source == dynamic && !iso`인 agent는 별도 predicate
`bridge_agent_is_dynamic_vanilla_codex` 하나로만 분기한다(Claude predicate와 동일한
3-term 계약: engine/source/!iso, 다른 추론 금지). 이 분기는: (1) launch builder가
`codex resume <id>`를 절대 emit하지 않고 네이티브 `codex resume --last`(continue) /
plain `codex`(fresh)만 쓴다 — `--all` 금지, cwd 필터가 안전 경계
(`bridge_build_dynamic_launch_cmd` codex arm + `bridge_build_resume_launch_cmd`
decline), (2) launch env가 `CODEX_HOME`을 operator-global `~/.codex`로 명시
export하여 `<agent_home>/.codex`가 되지 않도록 invariant를 못박는다
(`bridge_run_export_codex_launch_env`; `CODEX_SQLITE_HOME`은 건드리지 않음), (3)
codex session detect/refresh/normalize/central-resolver/#1305-persist/daemon
`bridge-sync.sh` backfill이 모두 이 class에 대해 no-op이라 operator
`~/.codex/sessions` id를 절대 detect/persist하지 않는다, (4) bridge comms hook은
operator `~/.codex/hooks.json`이 아니라 **project-local
`<workdir>/.codex/hooks.json`**에 설치되고(`bridge_ensure_codex_dynamic_project_hooks`),
launch는 fail-closed comms를 위해 `-c features.hooks=true`만 강제할 뿐 model/profile/
auth는 강제하지 않는다. **hook-trust가 핵심**: Codex는 project trust가 서야만
project-local hook을 실행하므로, trust가 comms를 막으면 blanket
`--dangerously-bypass-hook-trust`(모든 hook bypass — 너무 광범위) 대신 operator에게
**detect + REPORT**한다(warning + `codex_dynamic_project_hooks_untrusted` /
`_trust_unknown` audit, `bridge-hooks.py status-codex-project-trust`로 read-only
탐지). (5) auth은 `$CODEX_HOME/auth.json`을 상속하고 dynamic Codex는 `codex-cred`
fleet sync에서 제외되며 ambient token scrub은 default 유지
(`BRIDGE_CODEX_UNMANAGED_AUTH=1`이 명시적 escape hatch). **static/admin Codex는 전부
기존 동작 유지** — per-agent `<agent_home>/.codex` profile + hooks + managed
`auth.json` sync + bridge `codex resume <id>`. regression smoke:
`scripts/smoke/1899-dynamic-vanilla-codex.sh` (KNOWN_ISSUES #36).
