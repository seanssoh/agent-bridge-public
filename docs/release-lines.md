...
- **v0.16.10 — current LTS-line head (2026-06-13).** GitHub release `v0.16.10` (Latest, LTS); a large reliability + migration train consolidating rc1→rc3, hardened through a fleet soak across sean-mac (macOS shared-mode) and cm-prod (first real Linux iso-v2 production host) — zero data loss / zero drift on both. Dynamic-agent restart relaunch + self-restart survival + provisioning preserve across recreate (#1852/#1853/#1857); gated layout-v2 four-writer migration with fail-closed v1→v2 reconciliation (#1820); iso-v2 reconcile graceful-skip + structured audit + cron-worker prod-mutation fence (rc3); reconcile observability (rc2); #1838 tool-policy security fix. Supersedes v0.16.9 as line head; prior v0.16.x LTS designations remain valid. Follow-up hardening (controller fresh-group preflight for the stale-shell Errno13 cosmetic, KNOWN_ISSUES §28 / #1836) lands in the next v0.16.x patch.
