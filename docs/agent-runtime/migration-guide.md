...
For Telegram agents that should use the relay daemon instead of one
`getUpdates` poller per Claude session:

1. Keep the existing Telegram state files generated by setup:

   ```sh
   agent-bridge setup telegram <agent> --token <bot-token> --allow-from <telegram-user-id> --default-chat <chat-id> --yes
   ```

2. Enable relay supervision in the bridge daemon environment:

   ```sh
   export BRIDGE_TELEGRAM_RELAY_ENABLED=1
   ```

3. Change the agent channel registration from:
...
4. Until setup lifecycle wiring automates this, either leave
   `TELEGRAM_RELAY_REGISTER_TOKEN=1` enabled for the plugin or add the token
   hash and plugin-created `.telegram/relay-token` path to:

   ```text
   ~/.agent-bridge/state/channels/telegram/tokens.list
   ```

5. If the daemon may not already be running when the plugin starts, set:

   ```sh
   export BRIDGE_TELEGRAM_RELAY_AUTOSPAWN=1
   ```

The relay plugin uses the same `TELEGRAM_STATE_DIR` and `.telegram/access.json`
shape as the current Telegram setup path. It reads `TELEGRAM_BOT_TOKEN` from
`.telegram/.env`, writes a raw `.telegram/relay-token` file with mode `0600`
for the daemon, and never puts the token value on argv. Phase 3 will move these
opt-in steps behind `bridge-setup.py telegram` and surface relay liveness in
`agent-bridge status`.
