...
4. **Rewire symlinks**: `bridge_docs.ensure_agent_shared_links(agent_dir, bridge_home, …)` installs `COMMON-INSTRUCTIONS.md`, `CHANGE-POLICY.md`, `TOOLS.md`, `ADMIN-PROTOCOL.md`, and `MEMORY-SCHEMA.md` as symlinks that **resolve to `<bridge_home>/shared/<name>`** — the canonical shared docs that `sync_shared_docs` renders. The link target is computed depth-correct via `os.path.relpath(<shared_dir>, agent_dir)` (issue #1813), so it resolves at both v1 home depth (`agents/<a>/`) and v2 home depth (`data/agents/<a>/home/`); an already-correct link (absolute or relative) is accepted by realpath equality and never clobbered. `MEMORY-SCHEMA.md` joined this family in issue #1814 — its body is now propagated from `docs/agent-runtime/memory-schema.md` (canon SSOT) into `shared/MEMORY-SCHEMA.md` and symlinked from each home, retiring the old per-home template-sync fork. The legacy `sync_memory_schema_from_template` remains only as a **symlink-clobber-guarded no-op** (it refuses to write through a symlink, defusing the latent fleet-wide canon-clobber data-loss).
...
| `MEMORY-SCHEMA.md` link | resolves to `<bridge_home>/shared/MEMORY-SCHEMA.md` (canon body; depth-correct relative; #1814) |
