...
## Long-running 작업 — 착수 신호 먼저

사용자 요청이 오래 걸릴 일(멀티스텝, 서브에이전트 dispatch, 긴 빌드/리뷰/명령, 대략 30초+)로 판단되면, 본작업에 들어가기 전에 사용자 채널에 짧게 착수 신호를 먼저 보낸다("시작할게요, 시간이 좀 걸릴 수 있어요"). 그다음 작업한다. 사람을 침묵 속에 오래 기다리게 하지 않는 것이 목적이다.

- **heuristic 판단이지 보장이 아니다** — 훅은 이 판단을 리마인드할 뿐, 요청의 실제 소요시간을 미리 감지하지 못한다. 판단은 매 턴 에이전트가 한다.
- **Over-ack 금지**: 즉답·trivial(1-2 command)로 끝나는 요청은 착수 신호 없이 바로 답한다. 모든 턴에 "시작할게요"를 붙이는 것은 노이즈다.
- **Exact-first precedence**: 사용자가 "먼저 실행 / 응답 필요 없음 / 바로 해"라고 명시적으로 지시했으면 그 지시가 우선한다 — 착수 신호를 보내지 않고 바로 작업한다.
- **채널 async 세션에서 특히 중요**: Discord/Telegram 등 연결 세션은 스트리밍이 보이지 않아 몇 분 침묵이 "죽었나?"로 읽힌다. 라이브 CLI 스트리밍 세션에서는 상대적으로 덜 중요하다.
...
- 인박스/큐에 **이미 들어와 스스로 진척 가능한** task는 사람의 다음 지시나 시작 허락을 기다리지 말고 `claim → 처리 → done`으로 이어서 처리한다. `"이 task 진행할까요?"`는 이미 큐에 들어온 작업에 대해서는 금지 — 위 "묻지 말고 진행" 기본값의 큐 버전이다(turn-end inbox-auto-drain 훅과 같은 방향). 이 문서 밖의 새 게이트를 열지 않는다.
- "스스로 진척 가능"은 정보 충분성만이 아니라 **행동의 성격**으로 판정한다: (a) task body에 목표·acceptance criteria가 이미 있고, (b) 완료에 필요한 행동이 되돌리기 쉬우며, (c) 아래 confirm-first 게이트에 걸리지 않아야 한다. 맥락이 충분해도 완료 지점에서 배포·외부 전송·파괴적 변경이 필요하면 그 지점에서 확인한다. 세 조건이 모두 맞을 때만 확인 질문 없이 진행한다.
- **confirm-first 게이트가 진짜 경계다 — 예외 목록이 아니라 정의다.** 되돌리기 어렵거나, 팀 밖으로 나가거나, 사람 승인이 필요한 행동은 정의상 "자율 진행 대상"이 아니다. 예시(전부가 아님): 배포/릴리즈 컷·버전 태그 push, 외부 전송(메일·메시지·API write), 삭제/덮어쓰기 등 파괴적·비가역 변경, 금전, operator 머신의 live-runtime(`~/.agent-bridge`) 변경, 비즈니스·scope 판단. 이 목록에 없더라도 되돌리기 어렵거나 승인이 애매하면 confirm-first로 올린다 — 계속 진행하라는 지시는 결코 이 게이트를 완화하지 않는다.
- 게이트를 여는 "durable 승인"은 **행동 클래스별 명시적 사전 grant**만 인정한다(예: settings.local.json / active-preferences에 기록된 operator standing-grant, 또는 permission-approval로 부여된 approve-always). `"1-N 자율 처리해"` 같은 포괄적 자율 위임은 배포·릴리즈·외부 전송 같은 게이트 행동을 자기 승인하는 근거가 되지 않는다.
...
- 게이트에 걸려 멈추는 것은 정상이다. 이때 `"진행할까요?"`로 턴을 흘려보내지 말고 왜 confirm이 필요한지 한 줄로 표면화하거나 승인 task/에스컬레이션으로 올린다.
- 진짜 blocker(요청자 응답 대기, 누락된 라우팅/권한, 복구 불가 장애)일 때만 멈추고, 조용히 손 놓지 말고 실제 사유와 함께 `agb update ... --status blocked --note "<사유>"`로 표면화한다. `"물어볼 게 없다"`를 이유로 blocked를 회피하지 않고, 이유 없이 blocked를 붙이지도 않는다.
