...
- 터미널만 선택한 경우: `SOUL.md`, `SESSION-TYPE.md`, 사용자 메모리를 갱신하고 onboarding을 완료한다. `Onboarding State`는 `SESSION-TYPE.md`를 직접 손으로 고치지 말고 `agb agent set-onboarding <admin-agent> complete`로 바꾼다 — managed-project(워크디렉터리≠홈) admin은 HOME 사본만 고치면 런타임이 읽는 워크디렉터리 사본에 반영되지 않아 `pending`에 묶일 수 있다(#1417). 이 verb는 모든 SESSION-TYPE 레이어(profile source / home / workdir)와 `state/agents/<admin>/onboarding-{pending,complete}` 마커를 per-agent lock 아래에서 원자적으로 함께 쓴다 — 손으로 필드만 고치면 마커가 `pending`에 남아 업그레이드가 완료 상태를 보존하지 못한다(#2004). 그 뒤 `agb status`, `agb agent create`, `agb task create`, `agb upgrade`를 자연어로 요청하면 된다고 안내한다.
...
- 채널 setup이 끝났더라도 `SESSION-TYPE.md`가 `Onboarding State: pending`이면 admin은 `exit` 후 자동 재시작하지 않는다. `exit` 안내 전 반드시 `agb agent set-onboarding <admin-agent> complete`로 `Onboarding State`를 `complete`로 갱신하고(모든 SESSION-TYPE 레이어 + `onboarding-{pending,complete}` 마커를 함께 쓴다, #1417/#2004), `agb agent show <admin-agent>`로 `onboarding_state: complete`를 다시 확인한다.
...
- 채널 검증은 `~/.agent-bridge/agent-bridge agent start <agent> --dry-run`, `~/.agent-bridge/agent-bridge status`, 그리고 에이전트별 state dir 파일 존재 여부로 한다. **Telegram 채널은 `claude mcp list`로 검증하지 않는다** — 살아 있는 Telegram 봇에 `claude mcp list`를 실행하면 같은 봇 토큰으로 두 번째 `getUpdates` 롱폴러가 떠서 Telegram의 단일 소비자 제약을 위반하고 **409 Conflict**로 실행 중인 세션의 Telegram MCP 자식이 죽는다(검증 단계가 검증 대상 채널을 망가뜨림). 추가로 Agent Bridge 밖에서 `claude mcp list`를 돌리면 전역 `~/.claude/channels/...` 기준 오류가 보일 수도 있다.
