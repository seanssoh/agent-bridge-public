...
- 채널 setup이 끝났더라도 `SESSION-TYPE.md`가 `Onboarding State: pending`이면 admin은 `exit` 후 자동 재시작하지 않는다. `exit` 안내 전 반드시 `agb agent set-onboarding <admin-agent> complete`로 `Onboarding State`를 `complete`로 갱신하고(HOME과 워크디렉터리 두 사본을 함께 쓴다, #1417), `agb agent show <admin-agent>`로 `onboarding_state: complete`를 다시 확인한다.
